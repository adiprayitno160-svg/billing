...
exports.testPaidInvoicePdfSending = testPaidInvoicePdfSending;
exports.checkUnmarkedPaidInvoices = checkUnmarkedPaidInvoices;
const pool_1 = require("../src/db/pool");
const InvoicePdfService_1 = require("../src/services/invoice/InvoicePdfService");
    ...
    try {
      ...
      `);
            ...
            for (const invoice of rows) {
                ...
                try {
                    const pdfBuffer = await InvoicePdfService_1.InvoicePdfService.generateInvoicePdf(invoice.id);
                    if (pdfBuffer) {
...
${process.env.COMPANY_NAME || 'Perusahaan'} Billing System`;
                        ...
                        const messageResult = await whatsapp_1.whatsappService.sendMessage(invoice.customer_phone, waMessage);
                        if (messageResult.success) {
                            ...
                            const result = await whatsapp_1.whatsappService.sendDocument(invoice.customer_phone, tempFilePath, `Invoice_${invoice.invoice_number}.pdf`, `Invoice ${invoice.invoice_number} - Lunas`);
                  ...
                  INSERT INTO notification_logs (customer_id, invoice_id, notification_type, channel, status, sent_at, message)
                  VALUES (?, ?, 'paid_invoice_pdf', 'whatsapp', 'sent', NOW(), ?)
                `, [invoice.customer_id, invoice.id, `Paid invoice PDF sent: ${invoice.invoice_number}`]);
                            }
                            else {
