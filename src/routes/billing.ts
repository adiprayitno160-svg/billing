...
router.get('/rekap/unpaid', async (req, res) => {
    try {
        ...
        try {
            ...
            if (odc_id) { query += " AND c.odc_id = ?"; params.push(odc_id); }
            if (search) {
                query += " AND (c.name LIKE ? OR i.invoice_number LIKE ?)";
                params.push(`%${search}%`, `%${search}%`);
            }
            query += " ORDER BY o.name ASC, c.name ASC";
            const [invoices] = await conn.query(query, params) as any;
...
router.get('/rekap/transactions', async (req, res) => {
    try {
        ...
        try {
            ...
            `;
            const params: any[] = [];
            if (status) { query += " AND p.gateway_status = ?"; params.push(status); }
            if (search) {
                query += " AND (c.name LIKE ? OR i.invoice_number LIKE ?)";
                params.push(`%${search}%`, `%${search}%`);
            }
            query += " ORDER BY p.payment_date DESC";
            const [transactions] = await conn.query(query, params) as any;
...
router.get('/customer/isolation/api/whitelist', async (req, res) => {
    try {
        ...
        const customers: any[] = []; // Temporary fix for missing method
        res.json({ success: true, data: customers });
    } catch (error: any) {
...
router.get('/customer/isolation/api/watchlist', async (req, res) => {
    try {
        ...
        const watchlist: any[] = []; // Temporary fix for missing method
...
router.get('/customer/isolation/diagnose', async (req, res) => {
    try {
        ...
        for (const customer of customers as any[]) {
            ...
            // Get unpaid invoices
            const [invoices] = await databasePool.query(
                `SELECT id, invoice_number, period, status, remaining_amount, due_date,
                ...
                FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled') ORDER BY period ASC`,
                [GRACE_PERIOD_DAYS, customer.id]
            );
            diagnosis.invoices = invoices;

            // Check active deferments
            const [deferments] = await databasePool.query(
                "SELECT id, status, deferred_until_date, reason FROM payment_deferments WHERE customer_id = ? AND status IN ('pending', 'approved') AND deferred_until_date >= CURDATE()",
                [customer.id]
            );
            diagnosis.deferments = deferments;

            // Last 5 isolation logs
            const [isoLogs] = await databasePool.query(
                "SELECT action, reason, performed_by, mikrotik_response, created_at FROM isolation_logs WHERE customer_id = ? ORDER BY created_at DESC LIMIT 5",
                [customer.id]
            );
            ...
            if (!checks.is_isolated_false.pass) failedChecks.push('SUDAH TERISOLIR (is_isolated=1)');
            if (!checks.is_deferred_false.pass) failedChecks.push('ADA JANJI BAYAR AKTIF (is_deferred=1)');
            if (!checks.status_active.pass) failedChecks.push(`STATUS BUKAN ACTIVE (status=${customer.status})`);
            if (!checks.isolation_enabled_1.pass) failedChecks.push('ISOLATION BELUM DIAKTIFKAN (isolation_enabled=0) — INI PENYEBAB PALING UMUM!');

            if ((deferments as any[]).length > 0) failedChecks.push('ADA DEFERMENT AKTIF DI DB');

            const qualifyingInvoices = (invoices as any[]).filter((inv: any) =>
                inv.status_filter === 'PASS' && inv.period_filter === 'PASS' && inv.grace_filter === 'PASS'
            );
        ...
        const [schedulerSettings] = await databasePool.query(
        ...
        res.json({
            ...
            scheduler_settings: (schedulerSettings as any[])[0] || 'NOT FOUND',
...
router.post('/customer/isolation/bulk-restore-all-silent', async (req, res) => {
    try {
        const { IsolationService } = await import('../services/billing/isolationService');
        const username = (req.session as any)?.user?.username || 'admin';
...
router.get('/tagihan/:invoiceId/pay', async (req, res) => {
    try {
        ...
        try {
            ...
            const [allPendingInvoices] = await conn.query(
                `SELECT i.*, 
            ...
            const [primaryItems] = await conn.query(
                'SELECT * FROM invoice_items WHERE invoice_id = ? ORDER BY id',
                [invoice.id]
            ) as any;
