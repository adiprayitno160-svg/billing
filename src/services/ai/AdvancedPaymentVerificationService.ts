...
export class AdvancedPaymentVerificationService {
    ...
    static async verifyPaymentAdvanced(
        ...
        try {
            ...
            let extractionResult: VerificationStageResult;
            try {
            ...
            let matchResult: VerificationStageResult;
            try {
                matchResult = await this.stageMatching(
                    customerId,
                    extractedAmount,
                    options?.invoiceId
                );
...
- Jika tidak bisa membaca, kembalikan {"amount": null}`;
    ...
    private static async stageApproval(
        ...
        const hasCriticalFraud = fraudIndicators.some((ind: any) => ind.severity === 'critical');
            ...
            if (isFullPayment) {
                ...
                const [unpaidCheck] = await connection.query<RowDataPacket[]>(
                    `SELECT COUNT(*) as count FROM invoices 
                     WHERE customer_id = ? AND id != ? AND status IN ('sent', 'partial', 'overdue') AND remaining_amount > 0`,
