...
Ketik *Menu* untuk memulai.`);
            ...
            if (keyword === 'janjibayar' || keyword === 'janji' || cleanText === 'minta perpanjangan' || cleanText === 'perpanjangan waktu') {
                ...
                try {
                    ...
                    const [unpaidInvoices] = await databasePool.query<RowDataPacket[]>(
                    ...
                    const [existingDeferment] = await databasePool.query<RowDataPacket[]>(
                        `SELECT id, deferred_until_date FROM payment_deferments 
                         WHERE customer_id = ? AND invoice_id = ? AND status IN ('pending', 'approved')`,
                    ...
                    if (result.success) {
                        ...
                        await service.sendMessage(senderJid,
                            `✅ *JANJI BAYAR DISETUJUI*\n\n` +
...
Teknisi akan segera menghubungi Kakak. Terima kasih.`;
            ...
            if (cleanText.startsWith('!bayar')) {
                // 1. Check if Admin
                const [adminRows] = await databasePool.query<RowDataPacket[]>(
                    "SELECT id, full_name, role FROM users WHERE phone LIKE ? AND role IN ('admin', 'superadmin', 'operator') AND is_active = 1 LIMIT 1",
                    [`%${senderPhone.substring(senderPhone.length - 10)}`]
                );
...
_Catatan: Spasi di nominal akan dibuang otomatis._`);
                ...
                try {
                    ...
                    if (targetIdentifier.startsWith('0')) formats.push('62' + targetIdentifier.substring(1));
                    if (targetIdentifier.startsWith('62')) formats.push('0' + targetIdentifier.substring(2));

                    const [cRows] = await databasePool.query<RowDataPacket[]>(
                        `SELECT id, name, phone, customer_code FROM customers 
                         WHERE customer_code = ? OR phone IN (?) OR phone LIKE ? LIMIT 1`,
                        [targetIdentifier, formats, `%${targetIdentifier}`]
                    );
                    ...
                    const [invRows] = await databasePool.query<RowDataPacket[]>(
                        `SELECT id, invoice_number, remaining_amount FROM invoices 
                    ...
                    try {
                        ...
                        // Get current values
                        const currentPaid = parseFloat(invoice.remaining_amount) <= 0 ? 0 : (await conn.query<RowDataPacket[]>('SELECT paid_amount, total_amount FROM invoices WHERE id = ?', [invoice.id]))[0][0].paid_amount;
                        // wait, I can just recalculate
                        const [fullInv] = await conn.query<RowDataPacket[]>('SELECT paid_amount, total_amount, remaining_amount FROM invoices WHERE id = ?', [invoice.id]);
                        ...
                        const [pResult] = await conn.execute(
                            `INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, created_at) 
                             VALUES (?, 'whatsapp_admin', ?, NOW(), ?, NOW())`,
                            [invoice.id, amount, `Recorded via WhatsApp by ${admin.full_name}`]
                        );

                        // Update invoice
                        await conn.execute(
                            `UPDATE invoices SET paid_amount = ?, remaining_amount = ?, status = ?, 
                        ...
                        if (newStatus === 'paid') {
                            const [unpaidCheck] = await conn.query<RowDataPacket[]>(
                                "SELECT COUNT(*) as count FROM invoices WHERE customer_id = ? AND id != ? AND status != 'paid' AND remaining_amount > 0",
                                [targetCustomer.id, invoice.id]
                            );
                        ...
                        // 6. Notify Customer
                        const { UnifiedNotificationService } = await import('../notification/UnifiedNotificationService');
                        await UnifiedNotificationService.notifyPaymentReceived((pResult as any).insertId).catch(() => {});
...
Ketik *YA* untuk konfirmasi.`;
            ...
            if (/^(beli|paket|internet)/.test(keyword)) {
                ...
                if (resp) {
                    await WhatsAppSessionService.setSession(senderPhone, { step: 'waiting_prepaid_selection', data: {} });
