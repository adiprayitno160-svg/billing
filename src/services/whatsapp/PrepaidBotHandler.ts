    ...
    static async handleBuyCommand(customerPhone: string, customer: any): Promise<string> {
        try {
            ...
            const [unpaidInvoices] = await databasePool.query<RowDataPacket[]>(
                `SELECT SUM(remaining_amount) as total_arrears, COUNT(id) as invoice_count, GROUP_CONCAT(invoice_number SEPARATOR ', ') as invoice_list
                 FROM invoices 
            ...
            if (customer.connection_type === 'static_ip') {
                ...
                [packages] = await databasePool.query<RowDataPacket[]>(
                    `SELECT sip.id, sip.name, sip.price, sip.duration_days
                ...
                if (packages && packages.length > 0) pkg = packages[0];
            }
        ...
        try {
            ...
            if (customer.connection_type === 'static_ip') {
                const [packages] = await databasePool.query<RowDataPacket[]>(
                    `SELECT sip.id, sip.name, sip.price as price_30_days, sip.duration_days
                     FROM static_ip_clients sic
                     JOIN static_ip_packages sip ON sic.package_id = sip.id
                     WHERE sic.customer_id = ?
                     LIMIT 1`,
            ...
            try {
                const [companyRows] = await databasePool.query<RowDataPacket[]>(
                    'SELECT qris_image_url FROM company_settings ORDER BY updated_at DESC LIMIT 1'
                );
                
                let qrisFile: string | null = null;
                const company = companyRows[0];
                
                if (company && company.qris_image_url) {
                    // Check if it's a local path or URL
                    if (company.qris_image_url.startsWith('/')) {
                        qrisFile = path.join(process.cwd(), 'public', company.qris_image_url);
                ...
                if (!qrisFile) {
                    const defaultQris = path.join(process.cwd(), 'public', 'images', 'payments', 'qris.png');
                    if (fs.existsSync(defaultQris)) qrisFile = defaultQris;
                }

                if (qrisFile && fs.existsSync(qrisFile)) {
