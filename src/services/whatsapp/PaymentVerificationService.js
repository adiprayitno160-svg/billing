...
class PaymentVerificationService {
    ...
    static async verifyPaymentProofAuto(media, customerId) {
        try {
            ...
            const [customerRows] = await pool_1.databasePool.query('SELECT billing_mode FROM customers WHERE id = ?', [customerId]);
    ...
    static async verifyPrepaidPayment(customerId, media, transferAmount, extractedData) {
        ...
        try {
            ...
            const [paymentRequests] = await pool_1.databasePool.query(`SELECT * FROM prepaid_payment_requests
                 WHERE customer_id = ?
                 AND status = 'pending'
                 AND expires_at > NOW()
                 AND unique_code = ?
            ...
            if (paymentRequests.length === 0) {
                ...
                const [requestsByAmount] = await pool_1.databasePool.query(`SELECT * FROM prepaid_payment_requests
                     WHERE customer_id = ?
                     AND status = 'pending'
                     AND expires_at > NOW()
