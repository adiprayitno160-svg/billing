...
export class WhatsAppService extends EventEmitter {
  ...
  private readonly MAX_QR_RETRIES = 300; // Increased to give user 10 minutes to scan
  ...
  private readonly MAX_MESSAGES_PER_HOUR = 500; // Increased from 30 to 500
  private readonly MAX_MESSAGES_PER_DAY = 5000; // Increased from 200 to 5000
  ...
  // Paths
  private readonly AUTH_DIR = process.env.WA_SESSION_PATH 
    ? (path.isAbsolute(process.env.WA_SESSION_PATH) ? process.env.WA_SESSION_PATH : path.join(process.cwd(), process.env.WA_SESSION_PATH))
  ...
  private log(level: 'info' | 'warn' | 'error' | 'debug', message: string, data?: any): void {
    ...
    if (level === 'error') {
      ...
      try {
        ...
        fs.promises.appendFile(errorLogFile, errorMessage).catch(() => {});
      } catch (e) {
    ...
    try {
      ...
      fs.promises.appendFile(logFile, `${logMessage} ${data ? JSON.stringify(data) : ''}\n`).catch(() => {});
    } catch (e) {
  ...
  private queueMessage(to: string, content: AnyMessageContent, options?: SendMessageOptions): Promise<MessageResult> {
    return new Promise((resolve, reject) => {
      ...
      const existingInQueue = this.messageQueue.find(item => 
        this.getMessageHash(item.to, item.content) === msgHash
      );
      if (existingInQueue) {
