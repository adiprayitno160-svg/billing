...
export class AnalyticsService {
  ...
  async getRevenueAnalytics(): Promise<RevenueAnalytics> {
    ...
    // Total revenue
    const [totalRows] = await databasePool.query<RowDataPacket[]>(
    ...
    // Today revenue
    const [todayRows] = await databasePool.query<RowDataPacket[]>(
      `SELECT SUM(final_price) as today_revenue 
       FROM prepaid_subscriptions_v2 
    ...
    // This month revenue
    const [thisMonthRows] = await databasePool.query<RowDataPacket[]>(
      `SELECT SUM(final_price) as this_month_revenue 
       FROM prepaid_subscriptions_v2 
    ...
    // Last month revenue
    const [lastMonthRows] = await databasePool.query<RowDataPacket[]>(
      `SELECT SUM(final_price) as last_month_revenue 
       FROM prepaid_subscriptions_v2 
       WHERE activation_date >= ? AND activation_date <= ?`,
    ...
    const [avgRows] = await databasePool.query<RowDataPacket[]>(
  ...
  async getUsageAnalytics(): Promise<UsageAnalytics> {
    ...
    const [activeRows] = await databasePool.query<RowDataPacket[]>(
  ...
  async getCustomerAnalytics(): Promise<CustomerAnalytics> {
    ...
    const [monthRows] = await databasePool.query<RowDataPacket[]>(
      `SELECT COUNT(*) as new_month 
       FROM customers 
       WHERE connection_type = 'prepaid' AND created_at >= ?`,
      [thisMonthStart]
    );
    const newCustomersThisMonth = parseInt(monthRows[0]?.new_month || 0);
    
    // Churn rate (customers with expired subscriptions vs total)
    const [churnRows] = await databasePool.query<RowDataPacket[]>(
      `SELECT 
    ...
    const [ltvRows] = await databasePool.query<RowDataPacket[]>(
   ...
   */
  async getPackagePerformance(limit: number = 10): Promise<PackagePerformance[]> {
  ...
  async updateDailyAnalytics(date: Date = new Date()): Promise<void> {
    ...
    await databasePool.query(
      `INSERT INTO prepaid_analytics_daily (
        analytics_date,
