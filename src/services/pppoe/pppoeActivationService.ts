        ...
        try {
            ...
            for (const sub of subscriptions as any[]) {
                try {
                    ...
                    const [unpaidInvoices] = await connection.execute(
                        `SELECT period, total_amount, remaining_amount FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled') ORDER BY period ASC`,
                        [sub.customer_id]
                    );
                    const periods = (unpaidInvoices as any[]).map(inv => InvoiceService.getMonthName(inv.period));
                    ...
                    await UnifiedNotificationService.broadcastToAdmins(adminMsg).catch(e => console.error('Failed to notify admin about block:', e));
