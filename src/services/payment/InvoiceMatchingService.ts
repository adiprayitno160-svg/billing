...
export class InvoiceMatchingService {
     ...
     */
    static async matchPaymentWithInvoices(
        customerId: number,
        ...
        try {
            ...
            // Score each invoice
            const scoredInvoices: MatchedInvoice[] = invoices.map(invoice => {
     ...
     * Get pending invoices for customer
     */
    private static async getPendingInvoices(customerId: number): Promise<any[]> {
        const query = `
            SELECT 
                i.*,
        ...
        const [result] = await databasePool.query<RowDataPacket[]>(query, [customerId]);
     ...
     */
    private static scoreInvoiceMatch(invoice: any, extractedData: ExtractedPaymentData): {
        score: number;
        reasons: string[];
    } {
        let score = 0;
     ...
     */
    static async findInvoiceByNumber(customerId: number, invoiceNumber: string): Promise<any | null> {
        try {
            const query = `
                SELECT 
                    i.*,
            ...
            const [result] = await databasePool.query<RowDataPacket[]>(query, [customerId, invoiceNumber]);
