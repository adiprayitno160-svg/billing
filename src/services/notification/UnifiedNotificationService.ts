...
export class UnifiedNotificationService {
   ...
   */
  public static async sendNotificationById(notificationId: number): Promise<boolean> {
    ...
    try {
      ...
      const [rows] = await connection.query<RowDataPacket[]>(
        "SELECT * FROM unified_notifications_queue WHERE id = ? AND status IN ('pending', 'failed')", 
        [notificationId]
      );
      ...
      await connection.query(
        "UPDATE unified_notifications_queue SET status = 'processing', updated_at = NOW() WHERE id = ? AND status IN ('pending', 'failed')",
        [notificationId]
      );
      ...
      // 5. Mark as sent
      await connection.query(
        "UPDATE unified_notifications_queue SET status = 'sent', sent_at = NOW() WHERE id = ?",
        [notificationId]
      );
      ...
      await connection.query(
        "UPDATE unified_notifications_queue SET status = 'failed', error_message = ? WHERE id = ?",
    ...
    try {
      ...
      for (const notif of notifications) {
        ...
        try {
          ...
          if (notif.invoice_id) {
            const [invCheck] = await connection.query<RowDataPacket[]>(
              'SELECT id, status, remaining_amount FROM invoices WHERE id = ?',
              [notif.invoice_id]
            );
   ...
   */
  static async notifyInvoiceCreated(invoiceId: number, sendImmediately: boolean = true): Promise<number[]> {
    const connection = await databasePool.getConnection();
   ...
   */
  static async notifyPaymentReceived(paymentId: number, sendImmediately: boolean = true, isManualVerification: boolean = false): Promise<number[]> {
   ...
   */
  static async notifyPaymentDebt(invoiceId: number, sendImmediately: boolean = true): Promise<number[]> {
    ...
    try {
      ...
      const [invoiceRows] = await connection.query<RowDataPacket[]>(
        `SELECT i.*, c.name as customer_name, c.customer_code, c.phone
         FROM invoices i
         JOIN customers c ON i.customer_id = c.id
         WHERE i.id = ?`,
        [invoiceId]
      ...
      try {
        const ids = await this.queueNotification({
          ...
          variables: {
            ...
            total_amount: NotificationTemplateService.formatCurrency(parseFloat(invoice.total_amount)),
            remaining_amount: NotificationTemplateService.formatCurrency(parseFloat(invoice.remaining_amount)),
            due_date: invoice.due_date ? NotificationTemplateService.formatDate(new Date(invoice.due_date)) : '-',
            notes: invoice.notes || ''
          },
          send_immediately: sendImmediately
        });
        ...
        try {
          ...
          const adminSummary = `ðŸ“Œ *INFORMASI ${typeLabel} (SISTEM)*\n\n` +
            ...
            `ðŸ’° *Sisa Tagihan:* Rp ${NotificationTemplateService.formatCurrency(parseFloat(invoice.remaining_amount))}\n` +
            (invoice.due_date ? `ðŸ“† *Tgl Janji:* ${NotificationTemplateService.formatDate(new Date(invoice.due_date))}\n` : '') +
   ...
   */
  static async notifyJanjiBayar(invoiceId: number, sendImmediately: boolean = true): Promise<number[]> {
    const connection = await databasePool.getConnection();
    try {
      const { getBillingMonth } = await import('../../utils/periodHelper');
      const [invoiceRows] = await connection.query<RowDataPacket[]>(
        `SELECT i.*, c.name as customer_name, c.customer_code FROM invoices i JOIN customers c ON i.customer_id = c.id WHERE i.id = ?`,
        [invoiceId]
      );
      ...
      const ids = await this.queueNotification({
        customer_id: invoice.customer_id,
        invoice_id: invoiceId,
        notification_type: 'janji_bayar' as any,
        variables: {
          customer_name: invoice.customer_name,
          customer_code: invoice.customer_code,
          invoice_number: invoice.invoice_number,
