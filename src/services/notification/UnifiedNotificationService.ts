      ...
      try {
        ...
        try {
          ...
          const adminSummary = `📌 *INFORMASI ${typeLabel} (SISTEM)*\n\n` +
            ...
            `💰 *Sisa Tagihan:* Rp ${NotificationTemplateService.formatCurrency(parseFloat(invoice.remaining_amount))}\n` +
            (invoice.due_date ? `📆 *Tgl Janji:* ${NotificationTemplateService.formatDate(new Date(invoice.due_date))}\n` : '') +
            `📅 *Bulan:* ${billingMonth}\n\n` +
