...
export class PrepaidService {
    ...
    static async generatePaymentRequest(customerId: number, packageId: number, durationDays: number, options: any = {}): Promise<any> {
        ...
        try {
            ...
            const [customers] = await connection.query<RowDataPacket[]>(
                'SELECT connection_type FROM customers WHERE id = ?',
                [customerId]
            );
            ...
            if (connectionType === 'static_ip') {
                ...
                if (!packages || packages.length === 0) throw new Error('Static IP Package not found');
                pkg = packages[0];
                // For static IP, we usually use the fixed duration from package if not specified
                if (!durationDays) durationDays = pkg.duration_days || 30;
            } else {
                // Default to PPPoE
                const [packages]: any = await connection.query(
                    `SELECT id, name, 
                ...
                if (!packages || packages.length === 0) throw new Error('PPPoE Package not found');
                pkg = packages[0];
            }
