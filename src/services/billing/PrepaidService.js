...
class PrepaidService {
    ...
    static async switchToPrepaid(customerId, initialDays = 1, sendNotification = true) {
        ...
        try {
            ...
            // Get customer info
            const [customers] = await conn.query('SELECT id, name, phone, billing_mode, pppoe_profile_id FROM customers WHERE id = ?', [customerId]);
            ...
            await conn.query(`UPDATE customers 
                 SET billing_mode = 'prepaid', expiry_date = ?
                 WHERE id = ?`, [expiryDate, customerId]);
            // Get package info for notification
            let packageName = 'Paket Internet';
            if (customer.pppoe_profile_id) {
                const [profiles] = await conn.query('SELECT name FROM pppoe_profiles WHERE id = ?', [customer.pppoe_profile_id]);
...
Terima kasih atas pengertiannya! 🙏`;
    ...
    static async generatePaymentRequest(customerId, packageId, durationDays, options = {}) {
        ...
        try {
            await conn.beginTransaction();
            // Dynamic imports
            const { VoucherService } = await Promise.resolve().then(() => __importStar(require('./VoucherService')));
            // Get package price
            const [packages] = await conn.query(`SELECT id, name, 
                 CASE 
            ...
            const [custRows] = await conn.query('SELECT use_device_rental, is_taxable FROM customers WHERE id = ?', [customerId]);
            ...
            while (attempts < maxAttempts) {
                ...
                // Check collision
                const [existing] = await conn.query(`SELECT id FROM payment_requests 
                     WHERE total_amount = ? 
                     AND status = 'pending' 
                     AND expires_at > NOW()`, [finalTotal.toFixed(2)] // Assuming total_amount is DECIMAL
                );
                if (!existing || existing.length === 0) {
                    // Code is unique, create payment request
                    const expiresAt = new Date();
                    expiresAt.setHours(expiresAt.getHours() + 1); // Valid for 1 hour
                    const [result] = await conn.query(`INSERT INTO payment_requests 
                    ...
                    const [created] = await conn.query(`SELECT pr.*, pp.name as package_name
                         FROM payment_requests pr
                         LEFT JOIN pppoe_packages pp ON pr.package_id = pp.id
                         WHERE pr.id = ?`, [result.insertId]);
                    return {
                        success: true,
    ...
    static async confirmPayment(paymentRequestId, verifiedBy = null, paymentMethod = 'transfer') {
        ...
        try {
            await conn.beginTransaction();
            // Get payment request
            const [requests] = await conn.query(`SELECT pr.*, c.expiry_date as current_expiry
                 FROM payment_requests pr
            ...
            await conn.query(`UPDATE payment_requests 
                 SET status = 'paid', paid_at = NOW()
                 WHERE id = ?`, [paymentRequestId]);
            // Update customer expiry date and un-isolate
            await conn.query(`UPDATE customers 
                 SET expiry_date = ?, is_isolated = 0
                 WHERE id = ?`, [newExpiryDate, request.customer_id]);
            // Log transaction
            const [txResult] = await conn.query(`INSERT INTO prepaid_transactions 
            ...
            try {
                ...
                // Create the invoice
                const invoiceId = await InvoiceService.createInvoice(invoiceData, invoiceItems);
                // Link invoice back to transaction
                await conn.query('UPDATE prepaid_transactions SET invoice_id = ? WHERE id = ?', [invoiceId, transactionId]);
            ...
            if (request.voucher_id) {
                const { VoucherService } = await Promise.resolve().then(() => __importStar(require('./VoucherService')));
                await VoucherService.logVoucherUsage(request.voucher_id, request.customer_id, paymentRequestId, request.voucher_discount, request.base_amount, request.total_amount);
            }
            // Check Referral
            const { ReferralService } = await Promise.resolve().then(() => __importStar(require('./ReferralService')));
            ...
            try {
                if (request.customer_code) { // Assuming we can look up username if needed, or query it
                    ...
                    const [custRows] = await pool_1.databasePool.query('SELECT pppoe_username, phone, name FROM customers WHERE id = ?', [request.customer_id]);
                    if (custRows.length > 0 && custRows[0].pppoe_username) {
                        ...
                        if (custRows[0].phone) {
                            ...
                            if (device > 0)
                                msg += `Sewa Perangkat: Rp ${device.toLocaleString('id-ID')}\n`;
                            if (ppn > 0)
                                msg += `PPN: Rp ${ppn.toLocaleString('id-ID')}\n`;
    ...
    static async getExpiredCustomers() {
        const conn = await pool_1.databasePool.getConnection();
        try {
            const [customers] = await conn.query(`SELECT id, customer_code, name, phone, pppoe_username, expiry_date, is_isolated
                 FROM customers
        ...
        for (const customer of customers) {
            ...
            try {
                ...
                if (customer.phone) {
                    ...
                    await whatsappService.sendMessage(customer.phone, message).catch((err) => console.error('WA Error:', err));
                }
            }
            catch (error) {
                await conn.rollback();
     ...
     */
    static async getExpiringCustomers(daysUntilExpiry) {
        const conn = await pool_1.databasePool.getConnection();
        try {
            ...
            const [customers] = await conn.query(`SELECT id, customer_code, name, phone, pppoe_username, expiry_date
                 FROM customers
                 WHERE billing_mode = 'prepaid'
                 AND expiry_date IS NOT NULL
                 AND expiry_date BETWEEN ? AND ?
                 AND (is_isolated IS NULL OR is_isolated = 0)
                 ORDER BY expiry_date ASC`, [startOfDay, endOfDay]);
            return customers || [];
        }
        finally {
            conn.release();
