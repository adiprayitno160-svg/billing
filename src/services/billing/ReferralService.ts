...
export class ReferralService {
     ...
     */
    static async generateReferralCode(customerId: number, customerName: string): Promise<string> {
        try {
     ...
     */
    static async getCustomerReferralCode(customerId: number): Promise<string | null> {
        try {
            const [customers] = await databasePool.query<RowDataPacket[]>(
                `SELECT referral_code, name FROM customers WHERE id = ?`,
                [customerId]
            );
     ...
     */
    static async validateReferralCode(code: string, referredCustomerId: number): Promise<{
        valid: boolean;
        message: string;
        referrerId?: number;
        ...
        try {
            ...
            const [referrers] = await databasePool.query<RowDataPacket[]>(
                `SELECT id, name, billing_mode FROM customers WHERE referral_code = ?`,
                [code.toUpperCase()]
            );
            ...
            const [existing] = await databasePool.query<RowDataPacket[]>(
                `SELECT id FROM customer_referrals WHERE referrer_id = ? AND referred_id = ?`,
                [referrer.id, referredCustomerId]
            );
    ...
    static async createReferral(
        ...
        try {
            const [result] = await databasePool.query<ResultSetHeader>(
                ...
                VALUES (?, ?, ?, 'pending', 3, 10)`,
                [referrerId, referredId, referralCode.toUpperCase()]
            );
     ...
     */
    static async completeReferral(referralId: number): Promise<boolean> {
        try {
            // Get referral details
            const [referrals] = await databasePool.query<RowDataPacket[]>(
                `SELECT * FROM customer_referrals WHERE id = ? AND status = 'pending'`,
                [referralId]
            );
            ...
            const referral = referrals[0] as CustomerReferral;
            ...
            await databasePool.query(
                ...
                WHERE id = ?`,
                [rewardDays, referral.referrer_id]
            );
     ...
     */
    static async getReferralStats(customerId: number): Promise<{
        ...
        try {
            ...
            const successful = referrals.filter((r: any) => r.status === 'rewarded');
            const pending = referrals.filter((r: any) => r.status === 'pending');

            const totalDaysEarned = successful.reduce(
                (sum: number, r: any) => sum + (r.referrer_reward_days || 0),
                0
            );
     ...
     */
    static async getReferralByReferredId(referredId: number): Promise<CustomerReferral | null> {
        try {
            const [referrals] = await databasePool.query<RowDataPacket[]>(
                `SELECT * FROM customer_referrals WHERE referred_id = ? LIMIT 1`,
                [referredId]
            );

            return referrals.length > 0 ? (referrals[0] as CustomerReferral) : null;
        } catch (error) {
     ...
     */
    static async canUseReferral(customerId: number): Promise<boolean> {
        try {
            // Check if already used a referral
            const [existing] = await databasePool.query<RowDataPacket[]>(
                `SELECT id FROM customer_referrals WHERE referred_id = ?`,
                [customerId]
            );
            ...
            const daysSinceRegistration = Math.floor(
                (Date.now() - new Date(customers[0].created_at).getTime()) / (1000 * 60 * 60 * 24)
            );
