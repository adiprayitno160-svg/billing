...
class MigrationService {
  ...
  async migrateToPrepaid(customerId: number, adminId?: number): Promise<MigrationResult> {
    ...
    try {
      ...
      if (customer.billing_mode === 'prepaid') {
        ...
        const [existingPortal] = await connection.query<RowDataPacket[]>(
      ...
      if (customer.connection_type === 'static_ip') {
        const [staticIpCheck] = await connection.query<RowDataPacket[]>(
      ...
      try {
        ...
        const [mikrotikSettings] = await pool.query<RowDataPacket[]>(
          `SELECT host, port, username, password, is_active 
        ...
        if (mikrotikSettings.length > 0 && mikrotikSettings[0]) {
          ...
          if (currentHost === '192.168.5.1' || currentHost?.startsWith('192.168.5.')) {
            ...
            const settingId = allSettings.find((s: any) => s.id && String(s.host) === currentHost)?.id || (allSettings.length > 0 && allSettings[0] ? allSettings[0].id : undefined);
            
            if (settingId) {
              await pool.query('UPDATE mikrotik_settings SET host = ? WHERE id = ?', [correctIP, settingId]);
          ...
          if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
            ...
            const [staticIpRows] = await pool.query<RowDataPacket[]>(
              `SELECT ip_address FROM static_ip_clients WHERE customer_id = ? AND status = 'active' LIMIT 1`,
              [customerId]
            );
            ...
            if (!ipAddress) {
              ...
              const [allStaticIpRows] = await pool.query<RowDataPacket[]>(
              ...
              if (allStaticIpRows.length > 0 && allStaticIpRows[0].ip_address) {
                ...
                const [customerDetailRows] = await pool.query<RowDataPacket[]>(
                  `SELECT c.*, sic.ip_address as static_ip 
                   FROM customers c
            ...
            if (ipAddress) {
              ...
              if (!ipRegex.test(customerIP)) {
                ...
                if (isGatewayIP || isMikrotikHost) {
                  ...
                  try {
                    addSuccess = await addressListService.addToAddressList(
                      'prepaid-no-package',
                      customerIP,
                      `Prepaid-Migrated - ${customer.name} - Portal: ${portalId}`
                    );
                    ...
                    const errorMsg = (addError && typeof addError === 'object' && 'userFriendlyMessage' in addError 
                      ? String((addError as { userFriendlyMessage?: string }).userFriendlyMessage)
  ...
  async migrateToPostpaid(customerId: number, adminId?: number): Promise<MigrationResult> {
    ...
    try {
      ...
      try {
        ...
        const [allSettings] = await pool.query<RowDataPacket[]>(
          'SELECT id, host FROM mikrotik_settings ORDER BY id DESC'
        );
        ...
        const [finalSettings] = await pool.query<RowDataPacket[]>(
          `SELECT host, port, api_port, username, password 
           FROM mikrotik_settings 
           WHERE is_active = 1 
           ORDER BY id DESC 
           LIMIT 1`
        ...
        if (mikrotikSettings.length > 0) {
          ...
          if (customer.connection_type === 'static_ip') {
            ...
            const [lastSubscriptionRows] = await pool.query<RowDataPacket[]>(
            ...
            if (lastSubscriptionRows.length > 0 && lastSubscriptionRows[0].profile_name) {
              ...
              // Use pool after commit
              const [activeSubscriptionRows] = await pool.query<RowDataPacket[]>(
                `SELECT s.*, pp.profile_id, pprof.name as profile_name
                 FROM subscriptions s
                 LEFT JOIN pppoe_packages pp ON s.package_id = pp.id
                 LEFT JOIN pppoe_profiles pprof ON pp.profile_id = pprof.id
   ...
   */
  async fixPrepaidCustomer(customerId: number): Promise<MigrationResult> {
    const connection = await pool.getConnection();
    ...
    try {
      ...
      if (customer.connection_type === 'static_ip') {
        ...
        const [staticIpRows] = await connection.query<RowDataPacket[]>(
          `SELECT ip_address FROM static_ip_clients WHERE customer_id = ? AND status = 'active' LIMIT 1`,
          [customerId]
        );
   ...
   */
  async debugCustomerMigration(customerId: number): Promise<{
    customer: any;
    issues: string[];
   ...
   */
  async fixAllPrepaidCustomers(): Promise<{ fixed: number; failed: number; messages: string[] }> {
    const connection = await pool.getConnection();
    ...
    try {
      ...
      const [customers] = await connection.query<RowDataPacket[]>(
        `SELECT id, name, connection_type, billing_mode 
         FROM customers 
