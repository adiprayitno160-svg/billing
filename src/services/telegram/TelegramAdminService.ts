...
export class TelegramAdminService {
    ...
    private async handlePing(msg: TelegramBot.Message, customerIdStr: string): Promise<void> {
        ...
        try {
            ...
            // Get MikroTik config
            const [mkRows] = await pool.query<RowDataPacket[]>(`
            ...
            if (customer.connection_type === 'PPPoE') {
                ...
                const { mikrotikPool } = await import('../MikroTikConnectionPool');
                const rows = await mikrotikPool.execute<any[]>(cfg, '/ppp/active/print', [`?name=${username}`], `ppp_ping_${username}`, 1000);
                ...
                const { pingIpAddress } = await import('../mikrotikService');
     ...
     */
    private async handleBayarLunas(msg: TelegramBot.Message, invoiceIdStr: string): Promise<void> {
        const chatId = msg.chat.id;
