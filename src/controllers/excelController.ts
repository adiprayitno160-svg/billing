                ...
                while (!inserted && retries < maxRetries) {
                    try {
                        ...
                        const insertQuery = `
                            ...
                            ) VALUES (?, ?, ?, ?, ?, 'pppoe', 'inactive', 'postpaid', COALESCE(?, NOW()), NOW(), DATE_ADD(COALESCE(?, NOW()), INTERVAL 1 MONTH), ?, ?)
                        `;
                        ...
                        await databasePool.execute(insertQuery, [
                            ...
                            finalActivationDate,
                            finalActivationDate,
                            finalActivationDate || new Date().toISOString().split('T')[0],
                            28 // Per user request, setting all to 28
                        ]);
            ...
            try {
                ...
                const customerQuery = `
                    INSERT INTO customers (
                        ...
                        activation_date, custom_payment_deadline
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'postpaid', ?, ?, ?, NOW(), NOW(), DATE_ADD(NOW(), INTERVAL 1 MONTH), NOW(), ?)
                `;
