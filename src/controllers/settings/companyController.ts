...
export class CompanyController {
    ...
    static async showSettings(req: Request, res: Response) {
        try {
            ...
            const [companySettings] = await databasePool.query(
                'SELECT * FROM company_settings ORDER BY CASE WHEN id = 1 THEN 0 ELSE 1 END, updated_at DESC LIMIT 1'
            );
    ...
    static async saveSettings(req: Request, res: Response) {
        try {
            ...
            const [existing] = await databasePool.query('SELECT id FROM company_settings LIMIT 1');
