            ...
            if (updateFields.length > 0) {
                ...
                try {
                    ...
                    if (updateError.code === 'ER_BAD_FIELD_ERROR' || updateError.message.includes('Unknown column')) {
                        ...
                        for (const fq of fixQueries) { try { await conn.query(fq); } catch (e) {} }
                        await conn.query(query, updateValues);
            ...
            if (req.body.activation_date) {
                ...
                await conn.query(
                    `UPDATE subscriptions 
                     ...
                     WHERE customer_id = ?`,
                    [req.body.activation_date, nextBlockStr, customerId]
                );
                ...
                try {
                    ...
                    if (staticClient && staticClient.length > 0) {
                        ...
                        if (req.body.ip_address && pkgId) {
                            await conn.query(
                                'INSERT INTO static_ip_clients (customer_id, client_name, ip_address, gateway_ip, interface, package_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, "active", NOW(), NOW())',
                        ...
                        if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
                            ...
                            if (staticIpClient.ip_address) {
                                try {
                                    ...
                                    } catch (e: any) { console.warn(`[Background Cleanup] Client IP delete fail: ${e.message}`); }
                                    ...
                                    if (prefix === 30) {
                                        ...
                                        } catch (e: any) { console.warn(`[Background Cleanup] Gateway IP delete fail: ${e.message}`); }
                                ...
                                } catch (e) { console.warn(`[Background Cleanup] IP calc error:`, e); }
        ...
        try {
            ...
            for (const customerId of customerIds) {
                try {
                    ...
                    if (customer.connection_type === 'static_ip') {
                        const [sipRows] = await conn.query<RowDataPacket[]>('SELECT ip_address FROM static_ip_clients WHERE customer_id = ?', [customerId]);
                        if (sipRows && sipRows.length > 0) {
    ...
    try {
        ...
        try {
            ...
            if (status === 'inactive') {
                ...
                await conn.query(
                    'UPDATE customers SET status = "inactive", is_isolated = TRUE, updated_at = NOW() WHERE id = ?',
                    [customerId]
                );
                ...
                await conn.query(
                    'UPDATE invoices SET status = "cancelled", updated_at = NOW() WHERE customer_id = ? AND status = "draft"',
                    [customerId]
                );
            } else {
                // If activating customer:
                // 1. Check if they have unpaid invoices (debt)
                const [unpaid] = await conn.query<RowDataPacket[]>(
                    "SELECT COUNT(*) as count FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled')",
                    [customerId]
                );
                const hasDebt = (unpaid as any[])[0].count > 0;

                // 2. Set status to active, keep isolated if debt exists
                await conn.query(
                    'UPDATE customers SET status = "active", is_isolated = ?, updated_at = NOW() WHERE id = ?',
                    [hasDebt ? 1 : 0, customerId]
                );
            ...
            if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
                ...
                try {
                    const [staticClients] = await conn.query<RowDataPacket[]>(
                        'SELECT ip_address, gateway_ip, gateway_ip_id FROM static_ip_clients WHERE customer_id = ?',
                        [customerId]
                    );
                        ...
                        if (clientIp) {
                            ...
                            if (config) {
                                ...
                                if (!ipId) {
                                    ...
                                    if (!ipId && !sip.gateway_ip) ipId = await findIpAddressId(config, clientIp);
                                    if (!ipId && !sip.gateway_ip) ipId = await findIpAddressId(config, clientIp.split('/')[0]);
                                    
                                    // Extreme fallback: find by comment (Customer Code)
                                    if (!ipId) {
                                        const ips = await config ? (await import('../services/MikroTikConnectionPool')).mikrotikPool.execute(config, '/ip/address/print', []) : [];
            ...
            (async () => {
                try {
                    const [custInfo] = await databasePool.query<RowDataPacket[]>(
