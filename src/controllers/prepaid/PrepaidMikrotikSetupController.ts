...
import { getMikrotikConfig, validateMikrotikConfig } from '../../utils/mikrotikConfigHelper';
...
class PrepaidMikrotikSetupController {
  ...
  async showSetupWizard(req: Request, res: Response) {
    try {
      ...
      try {
        ...
        mikrotikConfig = await getMikrotikConfig();
        
        if (mikrotikConfig) {
          // Validate config untuk memastikan lengkap
          const validation = validateMikrotikConfig(mikrotikConfig);
          if (validation.valid) {
            mikrotikSettings = [mikrotikConfig as any];
      ...
      try {
        ...
        try {
          ...
          await pool.query(
            ...
            [(req.session as any)?.user?.id || 0, results.join('\n'), req.ip]
          );
        } catch (logError) {
  ...
  async resetSetup(req: Request, res: Response) {
    try {
      ...
      const natRulesToDelete = allNatArray.filter((r: any) => {
      ...
      const filterRulesToDelete = allFilterArray.filter((r: any) => {
        const hasPrepaidComment = r.comment && r.comment.toLowerCase().includes('prepaid');
        const hasPrepaidList = r['src-address-list'] && 
          (r['src-address-list'] === 'prepaid-no-package' || r['src-address-list'] === 'prepaid-active');
   ...
   */
  private async ensureAddressLists(api: RouterOSAPI): Promise<void> {
  ...
  private async createNATRules(api: RouterOSAPI, toAddress: string, toPort: number): Promise<void> {
    ...
    for (const rule of rules) {
      try {
        ...
        const existing = Array.isArray(allNatRules) ? allNatRules.filter((r: any) => {
  ...
  private async createFilterRules(api: RouterOSAPI, billingServerIp: string): Promise<void> {
    ...
    try {
      ...
      const prepaidActiveEntries = addressListsArray.filter((al: any) => al.list === 'prepaid-active');
      const prepaidNoPackageEntries = addressListsArray.filter((al: any) => al.list === 'prepaid-no-package');
    ...
    try {
      ...
      const existingPrepaidRules = Array.isArray(allFilterRules) ? allFilterRules.filter((r: any) => 
        r.comment && (r.comment.includes('prepaid') || r.comment.includes('STRICT'))
      ) : [];
    ...
    try {
      ...
      try {
        ...
        const found = allArray.find((r: any) => 
          r.comment && (r.comment.includes('Block ALL internet') || r.comment.includes('STRICT'))
        );
        if (found) {
    ...
    if (createdCount > 0) {
      ...
      try {
        ...
        const prepaidRules = allForwardRules.filter((r: any) => 
          r.comment && (r.comment.includes('prepaid') || r.comment.includes('STRICT'))
        );
    ...
    try {
      ...
      const rule1Found = finalArray.find((r: any) => r.comment && r.comment.includes('Allow internet for active prepaid'));
      const rule2Found = finalArray.find((r: any) => r.comment && r.comment.includes('Allow DNS for prepaid no package'));
      const rule4Found = finalArray.find((r: any) => r.comment && (r.comment.includes('Block ALL internet') || r.comment.includes('STRICT')));
    ...
    try {
      const prepaidRules = allFilterRules.filter((r: any) => 
        r.comment && (r.comment.toLowerCase().includes('prepaid') || r.comment.includes('STRICT'))
      );
      console.log(`[Setup] Existing prepaid filter rules:`, prepaidRules.map((r: any) => ({
        id: r['.id'],
        comment: r.comment,
        chain: r.chain,
  ...
  private async ensureSystemSettingsTable(): Promise<void> {
    try {
      ...
      const [descColumn] = await pool.query<RowDataPacket[]>(`
        SELECT COLUMN_NAME 
        FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'system_settings' 
   ...
   * Ensure activity_logs table exists
   */
  private async ensureActivityLogsTable(): Promise<void> {
    try {
      await pool.query(`
   ...
   */
  private async checkSetupStatus(mikrotikConfig: any): Promise<any> {
    if (!mikrotikConfig) {
      return {
        profiles: false,
        natRules: false,
    ...
    try {
      ...
      try {
        ...
        const natRules = Array.isArray(allNatRules) 
          ? allNatRules.filter((r: any) => 
              r.comment && r.comment.toLowerCase().includes('prepaid') &&
        ...
        console.log('[CheckStatus] NAT rules details:', natRules.map((r: any) => ({ comment: r.comment, chain: r.chain })));
        ...
        const filterRulesByComment = allRulesArray.filter((r: any) => {
        ...
        const addressListRules = allRulesArray.filter((r: any) => {
        ...
        const forwardChainRules = allRulesArray.filter((r: any) => r.chain === 'forward');
        ...
        if (addressListRules.length > 0) {
          ...
          addressListRules.forEach((r: any, idx: number) => {
        ...
        const uniqueRules = Array.from(new Map(combinedRules.map((r: any) => [r['.id'], r])).values());
        ...
        console.log('[CheckStatus] Filter rules details:', uniqueRules.map((r: any) => ({ 
