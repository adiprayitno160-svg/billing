...
export class InvoiceController {
    ...
    async generateBulkInvoices(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const createdIds = await InvoiceService.generateMonthlyInvoices(
                currentPeriod,
                customer_ids,
                true // forceAll = true because this is a manual trigger from UI
            );
        ...
        try {
            ...
            if (status === 'paid') {
                await databasePool.query(
                    'UPDATE invoices SET status = ?, remaining_amount = 0, updated_at = NOW() WHERE id = ?',
                    [status, id]
                );
            } else {
                await databasePool.query(
        ...
        try {
            ...
            if (notificationIds && notificationIds.length > 0) {
                for (const nid of notificationIds) {
                    UnifiedNotificationService.sendNotificationById(nid).catch((sendErr: any) => {
    ...
    async bulkDeleteInvoices(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const customerIds = (invList as any[]).map(row => row.customer_id).filter(id => id);
    ...
    async bulkSendInvoiceWhatsApp(req: Request, res: Response): Promise<void> {
        try {
            ...
            const promises = validInvoices.map(invoice =>
                UnifiedNotificationService.notifyInvoiceCreated(invoice.id, false)
                    .then(() => {
                        queuedCount++;
