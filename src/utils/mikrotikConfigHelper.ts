 ...
 */
async function ensureMikrotikSettingsTable(): Promise<void> {
  try {
...
export async function getMikrotikConfig(): Promise<MikrotikConfig | null> {
  ...
  return Promise.resolve().then(async () => {
    try {
      ...
      let queryResult: [RowDataPacket[], unknown] | [RowDataPacket[]] | unknown;
      try {
        queryResult = await databasePool.query<RowDataPacket[]>(
      ...
      if (Array.isArray(rows) && rows.length > 0) {
        const settings = rows[0] as RowDataPacket;
 ...
 */
export function validateMikrotikConfig(config: MikrotikConfig | null): { valid: boolean; error?: string } {
  if (!config) {
