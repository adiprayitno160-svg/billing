...
exports.autoFixInvoicesAndPaymentsTables = autoFixInvoicesAndPaymentsTables;
exports.autoFixWhatsAppTables = autoFixWhatsAppTables;
exports.autoFixCustomerColumns = autoFixCustomerColumns;
exports.autoFixPPPoEActivationTables = autoFixPPPoEActivationTables;
const pool_1 = __importDefault(require("../db/pool"));
/**
 * Auto-fix invoices and payments tables
 * Create tables if not exists - CRITICAL for bookkeeping functionality
 */
async function autoFixInvoicesAndPaymentsTables() {
    try {
        console.log('🔧 [AutoFix] Checking invoices and payments tables...');
        // Check if invoices table exists
        const [invoicesTables] = await pool_1.default.query("SHOW TABLES LIKE 'invoices'");
      ...
      `);
        ...
        const [invoiceItemsTables] = await pool_1.default.query("SHOW TABLES LIKE 'invoice_items'");
        if (invoiceItemsTables.length === 0) {
      ...
      `);
        ...
        const [paymentsTables] = await pool_1.default.query("SHOW TABLES LIKE 'payments'");
      ...
      `);
        ...
        const [pppoeProfilesTables] = await pool_1.default.query("SHOW TABLES LIKE 'pppoe_profiles'");
        if (pppoeProfilesTables.length === 0) {
      ...
      `);
        ...
        try {
            const [paidAtCols] = await pool_1.default.query("SHOW COLUMNS FROM invoices LIKE 'paid_at'");
            if (paidAtCols.length === 0) {
                console.log("⚠️ [AutoFix] Column 'paid_at' missing in invoices, adding...");
                await pool_1.default.query("ALTER TABLE invoices ADD COLUMN paid_at TIMESTAMP NULL DEFAULT NULL AFTER last_payment_date");
 ...
 */
async function autoFixWhatsAppTables() {
    try {
    ...
    `);
        ...
        try {
            const [colRows] = await pool_1.default.query("SHOW COLUMNS FROM whatsapp_sessions LIKE 'created_at'");
            if (colRows.length === 0) {
                console.log('⚠️ [AutoFix] Column created_at missing in whatsapp_sessions, adding...');
                await pool_1.default.query('ALTER TABLE whatsapp_sessions ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP AFTER temp_data');
    ...
    `);
        ...
        for (const col of pppoePackageColumns) {
            const [columns] = await pool_1.default.query(`SHOW COLUMNS FROM pppoe_packages LIKE '${col.name}'`);
            if (columns.length === 0) {
                console.log(`⚠️ [AutoFix] Column '${col.name}' missing in pppoe_packages, adding...`);
                await pool_1.default.query(`ALTER TABLE pppoe_packages ADD COLUMN ${col.name} ${col.def}`);
 ...
 */
async function autoFixCustomerColumns() {
    try {
        ...
        for (const col of monitoringColumns) {
            const [columns] = await pool_1.default.query(`SHOW COLUMNS FROM customers LIKE '${col.name}'`);
            if (columns.length === 0) {
                console.log(`⚠️ [AutoFix] Column '${col.name}' missing in customers, adding...`);
                await pool_1.default.query(`ALTER TABLE customers ADD COLUMN ${col.name} ${col.def}`);
                console.log(`✅ [AutoFix] Column '${col.name}' added successfully`);
            }
            else {
 ...
 */
async function autoFixPPPoEActivationTables() {
    try {
        ...
        for (const col of subColumns) {
            const [columns] = await pool_1.default.query(`SHOW COLUMNS FROM subscriptions LIKE '${col.name}'`);
            if (columns.length === 0) {
                console.log(`⚠️ [AutoFix] Column '${col.name}' missing in subscriptions, adding...`);
                await pool_1.default.query(`ALTER TABLE subscriptions ADD COLUMN ${col.name} ${col.def}`);
                console.log(`✅ [AutoFix] Column '${col.name}' added successfully`);
            }
        }
...
//# sourceMappingURL=autoFixDatabase.js.map