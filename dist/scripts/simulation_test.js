...
async function main() {
    ...
    for (const invId of invoiceIds) {
        ...
        await UnifiedNotificationService_1.UnifiedNotificationService.notifyInvoiceCreated(invId);
        console.log("Done.");
    }
    console.log("   > Processing Queue (Sending)...");
    const sendResult1 = await UnifiedNotificationService_1.UnifiedNotificationService.sendPendingNotifications();
    ...
    for (const invId of invoiceIds) {
        ...
        await UnifiedNotificationService_1.UnifiedNotificationService.notifyInvoiceReminder(invId);
        console.log("Done.");
    }
    console.log("   > Processing Queue (Sending)...");
    const sendResult2 = await UnifiedNotificationService_1.UnifiedNotificationService.sendPendingNotifications();
    ...
    await pool_1.databasePool.query("UPDATE invoices SET status = 'overdue' WHERE id IN (?)", [invoiceIds]);
    ...
    for (const invId of invoiceIds) {
        ...
        await UnifiedNotificationService_1.UnifiedNotificationService.notifyInvoiceOverdue(invId);
        console.log("Done.");
    }
    await UnifiedNotificationService_1.UnifiedNotificationService.sendPendingNotifications();
    // b) Mass Blocking
    console.log("   b) Running Mass Blocking (Auto Isolation)...");
    // Ensure customers are not isolated initially
    await pool_1.databasePool.query("UPDATE customers SET is_isolated = 0 WHERE id IN (?)", [customerIds]);
    ...
    const blockResult = await isolationService_1.IsolationService.autoIsolateOverdueCustomers();
    console.log("   > Block Result:", blockResult);
    // Verify Isolation
    const [custAfterBlock] = await pool_1.databasePool.query("SELECT id, name, is_isolated FROM customers WHERE id IN (?)", [customerIds]);
    ...
    await pool_1.databasePool.query("UPDATE invoices SET status = 'paid', remaining_amount = 0, paid_amount = total_amount WHERE id IN (?)", [allInvoices]);
    // Run Auto Restore
    ...
    // Verify Restoration
    const [custAfterRestore] = await pool_1.databasePool.query("SELECT id, name, is_isolated FROM customers WHERE id IN (?)", [customerIds]);
...
async function setupTestCustomers() {
    ...
    for (const c of customers) {
        // Try to find
        const [rows] = await pool_1.databasePool.query("SELECT id FROM customers WHERE email = ?", [c.email]);
        if (rows.length > 0) {
            ids.push(rows[0].id);
            // Reset status
            await pool_1.databasePool.query("UPDATE customers SET status = 'active', is_isolated = 0 WHERE id = ?", [rows[0].id]);
            // Clean invoices
            await pool_1.databasePool.query("DELETE FROM invoices WHERE customer_id = ?", [rows[0].id]);
        }
        else {
            const [res] = await pool_1.databasePool.query("INSERT INTO customers (name, email, phone, address, status, is_isolated, created_at) VALUES (?, ?, ?, ?, 'active', 0, NOW())", [c.name, c.email, c.phone, c.address]);
            ids.push(res.insertId);
        }
    }
    return ids;
}
async function createInvoices(customerIds, type) {
    const ids = [];
    const now = new Date();
    ...
    for (const cid of customerIds) {
        ...
        const [res] = await pool_1.databasePool.query("INSERT INTO invoices (customer_id, invoice_number, period, total_amount, remaining_amount, due_date, status, created_at) VALUES (?, ?, ?, 100000, 100000, ?, ?, NOW())", [cid, invNum, targetPeriod, dueDate, status]);
        ids.push(res.insertId);
    }
    return ids;
}
async function updateInvoicesDate(invoiceIds, daysFromNow) {
    const newDate = new Date();
    newDate.setDate(newDate.getDate() + daysFromNow);
    await pool_1.databasePool.query("UPDATE invoices SET due_date = ? WHERE id IN (?)", [newDate, invoiceIds]);
}
main().catch(console.error);
//# sourceMappingURL=simulation_test.js.map