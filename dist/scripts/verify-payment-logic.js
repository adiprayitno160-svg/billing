...
async function testPayment() {
    ...
    try {
        ...
        const [invs] = await conn.query('SELECT customer_id, remaining_amount FROM invoices WHERE id = ?', [847]);
        if (invs.length === 0) {
        ...
        // @ts-ignore
        const result = await controller.processPaymentTransaction({
            conn,
            customerId: custId,
...
testPayment();
//# sourceMappingURL=verify-payment-logic.js.map