...
async function checkState() {
    ...
    try {
        ...
        const [lalakRows] = await conn.query("SELECT id, name, status, is_isolated, isolation_enabled FROM customers WHERE name LIKE '%Lalak%'");
        console.log("\n=== Lalak Yuhandifa ===");
        if (lalakRows.length > 0) {
            for (const r of lalakRows) {
                ...
                const [subs] = await conn.query("SELECT id, status FROM subscriptions WHERE customer_id = ?", [r.id]);
                console.log("Subscriptions:", subs);
                const [invs] = await conn.query("SELECT id, invoice_number, period, total_amount, remaining_amount, status FROM invoices WHERE customer_id = ? ORDER BY period DESC", [r.id]);
        ...
        // Find Bu Nanik
        const [nanikRows] = await conn.query("SELECT id, name, status, is_isolated, isolation_enabled FROM customers WHERE name LIKE '%Nanik%'");
        console.log("\n=== Bu Nanik ===");
        if (nanikRows.length > 0) {
            for (const r of nanikRows) {
                ...
                for (const inv of invs) {
                    ...
                    const [items] = await conn.query("SELECT id, description, unit_price, total_price FROM invoice_items WHERE invoice_id = ?", [inv.id]);
