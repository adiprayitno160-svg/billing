...
exports.ensureInitialSchema = ensureInitialSchema;
...
async function checkDatabaseConnection() {
    const connection = await exports.databasePool.getConnection();
    try {
...
exports.db = exports.databasePool;
// Default export for common usage
exports.default = exports.databasePool;
async function ensureInitialSchema() {
    /*
    ...
    */
    ...
    const conn = await exports.databasePool.getConnection();
    try {
        ...
        if (Array.isArray(existingUsers) && existingUsers.length === 0) {
            ...
            await conn.query(`INSERT INTO users (username, email, phone, password, role, full_name, is_active) 
				VALUES (?, ?, ?, ?, ?, ?, ?)`, ['admin', 'admin@example.com', '08123456789', hashedPassword, 'superadmin', 'Administrator', 1]);
        ...
        try {
            ...
            columnNames = Array.isArray(allColumns) ? allColumns.map((col) => col.Field) : [];
        }
        catch (err) {
        ...
        const addColAfter = async (columnName, columnDef, afterColumn) => {
        ...
        // Create company_settings table (MAIN info)
        await conn.query(`CREATE TABLE IF NOT EXISTS company_settings (
            ...
            vpn_server_ip VARCHAR(191) NULL,
            vpn_username VARCHAR(191) NULL,
            vpn_password VARCHAR(191) NULL,
            bank_name VARCHAR(191) NULL,
            bank_account_number VARCHAR(191) NULL,
            bank_account_name VARCHAR(191) NULL,
            late_header TEXT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
        ...
        if (companyRows[0].count === 0) {
            await conn.query("INSERT INTO company_settings (company_name) VALUES ('Billing System')");
        }
        ...
        // Create customer_logs table if it doesn't exist
        await conn.query(`CREATE TABLE IF NOT EXISTS customer_logs (
			id INT AUTO_INCREMENT PRIMARY KEY,
			customer_id INT,
			action VARCHAR(50),
			description TEXT,
