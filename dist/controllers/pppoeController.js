...
async function postPackageCreate(req, res, next) {
    try {
        const { name, profile_id, price, duration_days, auto_activation, status, description, rate_limit_rx, rate_limit_tx, burst_limit_rx, burst_limit_tx, burst_threshold_rx, burst_threshold_tx, burst_time_rx, burst_time_tx, price_7_days, price_14_days, price_30_days, max_clients, priority, limit_at_download, limit_at_upload, burst_enabled } = req.body;
        if (!name)
        ...
        if (req.body.create_new_profile === 'on' || req.body.create_new_profile === '1') {
            ...
            if (!new_profile_remote_pool)
                throw new Error('Remote Address Pool wajib diisi untuk membuat profil baru');
            // Create Profile using Package Name
            // Use provided rate limits for the profile as well
            finalProfileId = await (0, pppoeService_1.createProfile)({
        ...
        await (0, pppoeService_1.createPackage)({
            ...
            burst_enabled: burst_enabled === '1' || burst_enabled === 'on' ? 1 : 0
        });
...
async function postPackageUpdate(req, res, next) {
    try {
        ...
        const { name, profile_id, price, duration_days, status, description, rate_limit_rx, rate_limit_tx, burst_limit_rx, burst_limit_tx, burst_threshold_rx, burst_threshold_tx, burst_time_rx, burst_time_tx, price_7_days, price_14_days, price_30_days, max_clients, priority, limit_at_download, limit_at_upload, burst_enabled } = req.body;
        if (!name)
            throw new Error('Nama paket wajib diisi');
        if (price && Number(price) < 0)
            throw new Error('Harga harus lebih dari 0');
