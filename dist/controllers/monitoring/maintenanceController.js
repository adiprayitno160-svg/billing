...
class MaintenanceController {
    ...
    async list(req, res) {
        try {
            ...
            const [schedules] = await pool_1.databasePool.query(`
                SELECT 
                    ms.*,
    ...
    async showCreate(req, res) {
        try {
            // Get areas
            const [areas] = await pool_1.databasePool.query(`
    ...
    async create(req, res) {
        try {
            const { title, description, maintenance_type, affected_area, affected_customers, start_time, end_time } = req.body;
            ...
            const customerIds = Array.isArray(affected_customers)
                ? affected_customers.map((id) => parseInt(id))
            ...
            if (hoursUntilStart > 0 && hoursUntilStart <= 48) {
                ...
                await alertRoutingService_1.default.sendMaintenanceNotification({
                    title,
