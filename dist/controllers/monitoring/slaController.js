...
class SLAController {
    ...
    async dashboard(req, res) {
        try {
            ...
            const [summary] = await pool_1.default.query(`
                SELECT * FROM v_monthly_sla_summary
                WHERE month_year = ?
            `, [monthDate]);
            // Get customers with SLA breach
            const [breaches] = await pool_1.default.query(`
                SELECT 
                    sr.*,
            ...
            const [activeIncidents] = await pool_1.default.query(`
                SELECT * FROM v_active_incidents
                ORDER BY duration_minutes DESC
                LIMIT 20
            `);
            // Get current connection status
            const [connectionStatus] = await pool_1.default.query(`
                SELECT 
                    current_status,
                    COUNT(*) AS count
    ...
    async customerDetail(req, res) {
        try {
            ...
            if (customer.connection_type === 'static_ip' && customer.ip_address) {
                customer.ip_address_display = (0, ipHelper_1.calculateCustomerIP)(customer.ip_address);
            }
            // Get SLA record for the month
            const slaRecord = await slaMonitoringService_1.default.getCustomerSLASummary(parsedCustomerId, monthDate);
            ...
            if (customer.service_type === 'pppoe') {
                bandwidthData = await bandwidthLogService_1.default.getCustomerBandwidth24h(parsedCustomerId);
            }
            // Get ping status (if Static IP)
            let pingStatus = null;
            if (customer.service_type === 'static_ip') {
                pingStatus = await pingService_1.default.getCustomerStatus(parsedCustomerId);
            }
            // Get monthly SLA history (last 6 months)
            const [slaHistory] = await pool_1.default.query(`
                SELECT 
    ...
    async incidents(req, res) {
        try {
            ...
            `, [...params, limit, offset]);
            // Get total count
            const [countResult] = await pool_1.default.query(`
    ...
    async approveDiscount(req, res) {
        try {
            ...
            await slaMonitoringService_1.default.approveDiscount(slaRecordId, userId);
            res.json({
                success: true,
    ...
    async getBandwidthTrend(req, res) {
        try {
            ...
            const parsedCustomerId = parseInt(customerId);
            const trend = await bandwidthLogService_1.default.getBandwidthTrend24h(parsedCustomerId);
            res.json({
                success: true,
                data: trend
            });
    ...
    async triggerCalculation(req, res) {
        try {
            ...
            slaMonitoringService_1.default.calculateMonthlySLA(monthDate).catch(err => {
