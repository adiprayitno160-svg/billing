...
class MonitoringController {
            ...
            try {
                ...
                try {
                    ...
                    if (mikrotikConfig) {
                        ...
                        const { getSystemHealth } = await Promise.resolve().then(() => __importStar(require('../../services/mikrotikService')));
    ...
    async monitorPPPoE(req, res) {
        try {
            ...
            const statusFilter = req.query.status_filter || ''; // New filter for online/offline
            ...
            const [odpList] = await pool_1.databasePool.query('SELECT id, name FROM ftth_odp ORDER BY name');
            ...
            const [globalStats] = await pool_1.databasePool.query(`
                SELECT 
                    COUNT(*) as total,
     ...
     * Network Maps page
     */
    async getNetworkMapsPage(req, res) {
        try {
    ...
    async getBandwidthTrend(req, res) {
        try {
            ...
            let trend;
            if (hours === 24) {
                trend = await bandwidthLogService_1.default.getBandwidthTrend24h(Number(customerId));
            }
            else {
                trend = await bandwidthLogService_1.default.getBandwidthTrend12h(Number(customerId));
            }
            res.json({
                success: true,
                period: hours + 'h',
     ...
     */
    async syncManually(req, res) {
        try {
            console.log('[MonitoringController] Manual sync requested');
            // Execute all sync tasks manually
            await NetworkMonitoringService_1.default.syncCustomerDevices();
            await NetworkMonitoringService_1.default.syncFTTHInfrastructure();
            await NetworkMonitoringService_1.default.autoCreateLinks();
            res.json({
                success: true,
