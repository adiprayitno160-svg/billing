...
exports.getTroubleCustomers = getTroubleCustomers;
exports.getDashboard = getDashboard;
exports.getInterfaceStats = getInterfaceStats;
const pool_1 = require("../db/pool");
const mikrotikService_1 = require("../services/mikrotikService");
const serverMonitoringService_1 = require("../services/serverMonitoringService");
function getLastNDatesLabels(days) {
    const labels = [];
    const now = new Date();
...
async function getTroubleCustomers() {
    try {
        // Query Maintenance Schedules
        let maintenance = [];
        try {
            // Check if customer_id column exists in maintenance_schedules
            const [cols] = await pool_1.databasePool.query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'maintenance_schedules' AND COLUMN_NAME = 'customer_id'");
            if (cols.length > 0) {
        ...
        monitoringData.customers.forEach((c) => monitoringMap.set(c.id, c));
        ...
        const devices = monitoringData.customers
            .filter((c) => c.status === 'offline' || c.status === 'degraded' || c.status === 'isolated')
            .map((c) => ({
            id: c.id,
            name: c.name,
        ...
        return Array.from(unique.values()).sort((a, b) => {
            const getScore = (item) => {
                if (item.device_status === 'offline')
                    return 1;
                if (item.trouble_type === 'maintenance')
                    return 2;
                return 3;
            };
...
async function getDashboard(req, res) {
    try {
        ...
        if (schemaCache && schemaCache.hasLatePaymentCount !== undefined) {
            hasLatePaymentCount = schemaCache.hasLatePaymentCount;
        }
        else {
            try {
                const [cols] = await pool_1.databasePool.query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'customers' AND COLUMN_NAME = 'late_payment_count'");
                hasLatePaymentCount = cols.length > 0;
                // Update cache
                if (schemaCache) {
                    schemaCache.hasLatePaymentCount = hasLatePaymentCount;
                }
            }
            catch {
        ...
        const [activeCustomersP, totalCustomersP, inactiveCustomersP, suspendedCustomersP, pppoeCustomersP, staticIpCustomersP, pppoePackagesP, pppoeProfilesP, newRequests7dP, recentRequestsP, chartRawP, oltCountP, odcCountP, odpCountP, mtSettingsP, troubleCustomersP, latePaymentHighRiskP, latePaymentWarning4P, verifyingJobsP, pendingPaymentsP] = await Promise.all([
            ...
            pool_1.databasePool.query("SELECT COUNT(*) AS cnt FROM customers WHERE is_isolated=1 AND status='active'"),
            pool_1.databasePool.query("SELECT COUNT(*) AS cnt FROM customers WHERE connection_type='pppoe' AND status='active'"),
            pool_1.databasePool.query("SELECT COUNT(*) AS cnt FROM customers WHERE connection_type='static_ip' AND status='active'"),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM pppoe_packages'),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM pppoe_profiles'),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM pppoe_new_requests WHERE created_at >= (CURDATE() - INTERVAL 7 DAY)'),
            pool_1.databasePool.query('SELECT customer_name, phone, package_id, created_at FROM pppoe_new_requests ORDER BY created_at DESC LIMIT 5'),
            pool_1.databasePool.query("SELECT DATE(created_at) AS d, COUNT(*) AS c FROM pppoe_new_requests WHERE created_at >= (CURDATE() - INTERVAL 6 DAY) GROUP BY DATE(created_at)"),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM ftth_olt'),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM ftth_odc'),
            pool_1.databasePool.query('SELECT COUNT(*) AS cnt FROM ftth_odp'),
            pool_1.databasePool.query('SELECT id, host, port, username, password, use_tls, created_at, updated_at FROM mikrotik_settings ORDER BY id DESC LIMIT 1'),
            getTroubleCustomers(),
            (async () => {
                if (!hasLatePaymentCount)
                    return [[{ cnt: 0 }]];
                try {
                    const [result] = await pool_1.databasePool.query("SELECT COUNT(*) AS cnt FROM customers WHERE COALESCE(late_payment_count, 0) >= 3");
                    return result;
                }
                catch {
                    return [[{ cnt: 0 }]];
            ...
            (async () => {
                if (!hasLatePaymentCount)
                    return [[{ cnt: 0 }]];
                try {
                    const [result] = await pool_1.databasePool.query("SELECT COUNT(*) AS cnt FROM customers WHERE COALESCE(late_payment_count, 0) >= 4");
                    return result;
                }
                catch {
                    return [[{ cnt: 0 }]];
        ...
        const pppoeProfiles = pppoeProfilesP[0][0]?.cnt ?? 0;
        const totalCustomers = totalCustomersP[0][0]?.cnt ?? 0;
        const inactiveCustomers = inactiveCustomersP[0][0]?.cnt ?? 0;
        const suspendedCustomers = suspendedCustomersP[0][0]?.cnt ?? 0;
        const pppoeCustomers = pppoeCustomersP[0][0]?.cnt ?? 0;
        const staticIpCustomers = staticIpCustomersP[0][0]?.cnt ?? 0;
        const oltCount = oltCountP[0][0]?.cnt ?? 0;
        const odcCount = odcCountP[0][0]?.cnt ?? 0;
        const odpCount = odpCountP[0][0]?.cnt ?? 0;
        const newRequests = newRequests7dP[0][0]?.cnt ?? 0;
        const recentRequests = recentRequestsP[0] ?? [];
        // Ensure troubleCustomers is always an array
        const troubleCustomers = Array.isArray(troubleCustomersP) ? troubleCustomersP : [];
        const recentAlerts = []; // Forced empty
        const latePaymentHighRisk = latePaymentHighRiskP[0][0]?.cnt ?? 0;
        ...
        const mtSettings = Array.isArray(mtSettingsP[0]) && mtSettingsP[0].length ? mtSettingsP[0][0] : null;
        ...
        try {
            serverMonitoring = await serverMonitoringService_1.ServerMonitoringService.getServerStatus();
        }
        catch (error) {
        ...
        res.render('dashboard/index', {
            ...
            stats: {
                ...
                odcCount,
                odpCount,
                pendingPaymentVerifications: Array.isArray(pendingPaymentsP[0]) ? pendingPaymentsP[0].length : 0
            },
            latePaymentStats: {
            ...
            verifyingJobs,
            pendingPaymentVerifications: Array.isArray(pendingPaymentsP[0]) ? pendingPaymentsP[0] : [],
            recentRequests,
            troubleCustomers,
            recentAlerts,
...
async function getInterfaceStats(req, res) {
    try {
        ...
        const [mtSettingsRows] = await pool_1.databasePool.query('SELECT id, host, port, username, password, use_tls, created_at, updated_at FROM mikrotik_settings ORDER BY id DESC LIMIT 1');
        const mtSettings = Array.isArray(mtSettingsRows) && mtSettingsRows.length ? mtSettingsRows[0] : null;
        if (!mtSettings) {
            res.json([]);
            return;
        }
        ...
        const interfacesPromise = (0, mikrotikService_1.getInterfaces)(config);
