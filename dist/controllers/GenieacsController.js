...
class GenieacsController {
    ...
    static async dashboard(req, res) {
        try {
            ...
            const config = await GenieacsService_1.GenieacsService.getConfigFromDb();
            // Test connection
    ...
    static async devices(req, res) {
        try {
            ...
            const [customers] = await pool_1.databasePool.query('SELECT id, name, phone, pppoe_username, serial_number FROM customers');
            ...
            customers.forEach((c) => {
                ...
                if (c.pppoe_username)
                    customerByPPPoE.set(c.pppoe_username.toLowerCase(), c);
                if (c.serial_number)
                    customerByCustomerSerial.set(c.serial_number, c);
            });
            // 5. Get Network Devices
            const [netDevices] = await pool_1.databasePool.query('SELECT customer_id, genieacs_serial FROM network_devices WHERE device_type="ont" AND genieacs_serial IS NOT NULL');
            const customerBySerial = new Map();
            netDevices.forEach((nd) => {
                const c = customerById.get(nd.customer_id);
                if (c)
                    customerBySerial.set(nd.genieacs_serial, c);
            });
    ...
    static async deviceDetail(req, res) {
        try {
            ...
            const [netDevices] = await pool_1.databasePool.query('SELECT customer_id FROM network_devices WHERE device_type="ont" AND genieacs_serial = ? LIMIT 1', [deviceInfo.serialNumber]);
            if (netDevices.length > 0 && netDevices[0].customer_id) {
                const [custs] = await pool_1.databasePool.query('SELECT id, name, phone, address FROM customers WHERE id = ?', [netDevices[0].customer_id]);
                if (custs.length > 0)
                    customer = custs[0];
            }
            // 2. Try via PPPoE Username
            if (!customer && wanStatus.username && wanStatus.username !== '-') {
                const [custs] = await pool_1.databasePool.query('SELECT id, name, phone, address FROM customers WHERE pppoe_username = ?', [wanStatus.username]);
                if (custs.length > 0)
                    customer = custs[0];
            }
            // 3. Try via customers.serial_number (Fallback)
            if (!customer) {
                const [custs] = await pool_1.databasePool.query('SELECT id, name, phone, address FROM customers WHERE serial_number = ?', [deviceInfo.serialNumber]);
                if (custs.length > 0)
                    customer = custs[0];
            }
            // Get all customers for assignment dropdown
            const [allCustomers] = await pool_1.databasePool.query('SELECT id, name, customer_code, phone FROM customers ORDER BY name ASC');
     ...
     * Refresh WiFi Info
     */
    static async refreshWiFi(req, res) {
        try {
            const { id } = req.params;
            const genieacs = await GenieacsService_1.GenieacsService.getInstanceFromDb();
            ...
            if (result.success) {
                ...
                try {
                    ...
                    if (serialNumber) {
                        ...
                        await pool_1.databasePool.query(`UPDATE customers SET wifi_ssid = ?, wifi_password = ?, updated_at = NOW() 
                             WHERE serial_number = ?`, [ssid, password, serialNumber]);
     ...
     */
    static async changePPPoECredentials(req, res) {
        try {
            const { id } = req.params;
     ...
     */
    static async configureWan(req, res) {
        try {
            const { id } = req.params;
            const { wanMode, // 'pppoe', 'bridge', 'static', 'dhcp'
            vlanId, pppUsername, pppPassword, ipAddress, subnetMask, gateway, dnsServers, enableConnection } = req.body;
            const genieacs = await GenieacsService_1.GenieacsService.getInstanceFromDb();
     ...
     */
    static async syncAllTags(req, res) {
        try {
            console.log('[SyncTags] Starting enhanced full sync from Billing to GenieACS...');
            const genieacs = await GenieacsService_1.GenieacsService.getInstanceFromDb();
            // 1. Get all customers from DB
            const [customers] = await pool_1.databasePool.query('SELECT id, name, serial_number, pppoe_username FROM customers');
     ...
     */
    static async syncCustomerTag(req, res) {
        try {
            const { id } = req.params;
            const genieacs = await GenieacsService_1.GenieacsService.getInstanceFromDb();
            ...
            if (!customerName) {
                const [custs] = await pool_1.databasePool.query('SELECT name FROM customers WHERE serial_number = ? LIMIT 1', [serialNumber]);
                if (custs.length > 0)
                    customerName = custs[0].name;
            }
            // B. Try via network_devices (Serial)
            if (!customerName) {
                const [netDevices] = await pool_1.databasePool.query('SELECT customer_id FROM network_devices WHERE device_type="ont" AND genieacs_serial = ? LIMIT 1', [serialNumber]);
                if (netDevices.length > 0 && netDevices[0].customer_id) {
                    const [custs] = await pool_1.databasePool.query('SELECT name FROM customers WHERE id = ?', [netDevices[0].customer_id]);
                    if (custs.length > 0)
                        customerName = custs[0].name;
                }
            }
            ...
            if (!customerName) {
                const pppoe = genieacs.getPPPoEDetails(device);
                if (pppoe.username && pppoe.username !== '-') {
                    const [custs] = await pool_1.databasePool.query('SELECT name FROM customers WHERE pppoe_username = ?', [pppoe.username]);
                    if (custs.length > 0)
                        customerName = custs[0].name;
                }
            }
     ...
     */
    static async assignCustomer(req, res) {
        try {
            ...
            await pool_1.databasePool.query('UPDATE customers SET serial_number = ? WHERE id = ?', [serialNumber, customer_id]);
            // Get customer name for flash message
            const [customers] = await pool_1.databasePool.query('SELECT name FROM customers WHERE id = ?', [customer_id]);
     ...
     */
    static async unlinkCustomer(req, res) {
        try {
            const { id } = req.params; // Device ID
            const genieacs = await GenieacsService_1.GenieacsService.getInstanceFromDb();
            // Get device to extract serial number
            ...
            const [customers] = await pool_1.databasePool.query('SELECT id, name FROM customers WHERE serial_number = ?', [serialNumber]);
            ...
            await pool_1.databasePool.query('UPDATE customers SET serial_number = NULL WHERE id = ?', [customer.id]);
