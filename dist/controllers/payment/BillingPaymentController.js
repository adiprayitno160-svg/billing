...
exports.BillingPaymentController = void 0;
...
const discountService_1 = require("../../services/billing/discountService");
class BillingPaymentController {
    ...
    async createInvoicePayment(req, res) {
        try {
            const { invoiceId, customerId, gatewayCode, paymentMethod, callbackUrl, redirectUrl, } = req.body;
            // Validasi input
     ...
     */
    async getInvoiceWithPaymentOptions(req, res) {
        try {
            ...
            const result = await this.billingPaymentService.getInvoiceWithPaymentOptions(parseInt(invoiceId), parseInt(customerId));
            res.json({
                success: true,
                data: result,
            });
     ...
     */
    async getCustomerPaymentHistory(req, res) {
        try {
            ...
            const history = await this.billingPaymentService.getCustomerPaymentHistory(parseInt(customerId), parseInt(limit), parseInt(offset));
            res.json({
                success: true,
                data: history,
                pagination: {
    ...
    async getAvailablePaymentMethods(req, res) {
        try {
            ...
            const methods = await this.billingPaymentService.getAvailablePaymentMethods(parseInt(customerId));
            res.json({
                success: true,
                data: methods,
            });
    ...
    async createPaymentLink(req, res) {
        try {
            ...
            const paymentLink = await this.billingPaymentService.createPaymentLink(parseInt(invoiceId), parseInt(customerId));
            res.json({
                success: true,
                data: {
                    paymentLink,
    ...
    async processPaymentFailure(req, res) {
        try {
            ...
            await this.billingPaymentService.processPaymentFailure(transactionId, reason || 'Payment failed');
            res.json({
                success: true,
    ...
    async applyDiscount(req, res) {
        try {
            ...
            const result = await discountService_1.DiscountService.applyMarketingDiscount(parseInt(invoiceId), code, userId);
            res.json(result);
        }
        catch (error) {
...
exports.BillingPaymentController = BillingPaymentController;
//# sourceMappingURL=BillingPaymentController.js.map