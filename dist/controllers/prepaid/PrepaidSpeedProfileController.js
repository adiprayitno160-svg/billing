...
const MikrotikConnectionPool_1 = require("../../services/mikrotik/MikrotikConnectionPool");
const MikrotikHealthCheck_1 = require("../../services/mikrotik/MikrotikHealthCheck");
/**
...
class PrepaidSpeedProfileController {
    ...
    async index(req, res) {
        ...
        try {
            ...
            const health = await MikrotikHealthCheck_1.MikrotikHealthCheck.checkHealth();
            if (!health.isOnline) {
            ...
            try {
                ...
                if (mikrotikSettings.length === 0) {
                    const [anySettings] = await pool_1.default.query('SELECT * FROM mikrotik_settings LIMIT 1');
                    mikrotikSettings = anySettings;
                }
            }
            catch (dbError) {
    ...
    async createProfile(req, res) {
        try {
            ...
            const settings = mikrotikSettings.length > 0 ? mikrotikSettings[0] : (await pool_1.default.query('SELECT * FROM mikrotik_settings LIMIT 1'))[0][0];
            // Connect to Mikrotik
     ...
     */
    async getProfilesFromMikrotik(settings) {
        try {
            console.log('[SpeedProfile] Fetching profiles via connection pool...');
            // Use connection pool with caching
            const profiles = await MikrotikConnectionPool_1.MikrotikConnectionPool.executeCommand('/ppp/profile/print', [], 'ppp_profiles', // Cache key
    ...
    clearCache() {
        ...
        MikrotikConnectionPool_1.MikrotikConnectionPool.clearCache('ppp_profiles');
...
PrepaidSpeedProfileController.profilesCache = null;
PrepaidSpeedProfileController.cacheTime = 0;
PrepaidSpeedProfileController.CACHE_TTL = 60000; // 60 seconds cache
exports.default = new PrepaidSpeedProfileController();
//# sourceMappingURL=PrepaidSpeedProfileController.js.map