...
class KasirController {
    ...
    async processPayment(req, res) {
        try {
            ...
            // Proses pembayaran
            const paymentResult = await this.processPaymentTransaction(parseInt(customerId), parseFloat(amount) || 0, paymentMethod || 'cash', notes || '', req.user.id, paymentType);
    ...
    async processPaymentTransaction(customerId, amount, paymentMethod, notes, kasirId, paymentType) {
        const conn = await pool_1.databasePool.getConnection();
        try {
            ...
            if (newStatus === 'paid') {
                await conn.query('UPDATE customers SET is_isolated = FALSE WHERE id = ?', [customerId]);
            }
            await conn.commit();
    ...
    async sendPaymentNotification(customer, invoice, amount, paymentMethod, paymentId, paymentType) {
        try {
            if (!customer.phone) {
