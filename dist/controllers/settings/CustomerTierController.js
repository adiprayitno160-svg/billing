...
exports.CustomerTierController = void 0;
const CustomerTierService_1 = __importDefault(require("../../services/billing/CustomerTierService"));
class CustomerTierController {
    ...
    async getAllTiers(req, res) {
        try {
            const tiers = await CustomerTierService_1.default.getAllTiers();
            res.json({
                success: true,
                data: tiers
            });
    ...
    async getTierById(req, res) {
        try {
            ...
            const tier = await CustomerTierService_1.default.getTierById(id);
            if (!tier) {
                res.status(404).json({
                    success: false,
    ...
    async createTier(req, res) {
        try {
            ...
            const newTierId = await CustomerTierService_1.default.createTier({
                name,
    ...
    async updateTier(req, res) {
        try {
            ...
            if (max_discount_percent !== undefined)
                updateData.max_discount_percent = max_discount_percent;
            if (priority_level !== undefined)
                updateData.priority_level = priority_level;
            const updated = await CustomerTierService_1.default.updateTier(id, updateData);
            if (!updated) {
                res.status(404).json({
                    success: false,
    ...
    async deleteTier(req, res) {
        try {
            ...
            const deleted = await CustomerTierService_1.default.deleteTier(id);
            if (!deleted) {
                res.status(404).json({
                    success: false,
                    message: 'Tier not found'
     ...
     */
    async getCustomerSLASettings(req, res) {
        try {
            ...
            const settings = await CustomerTierService_1.default.getCustomerSLASettings(customerId);
            res.json({
                success: true,
                data: settings
            });
     ...
     */
    async updateCustomerSLASettings(req, res) {
        try {
            const customerId = parseInt(req.params.customerId);
            if (isNaN(customerId)) {
                res.status(400).json({
            ...
            if (custom_sla_target !== undefined)
                updateData.custom_sla_target = custom_sla_target;
            if (custom_discount_rate !== undefined)
                updateData.custom_discount_rate = custom_discount_rate;
            if (custom_max_discount_percent !== undefined)
                updateData.custom_max_discount_percent = custom_max_discount_percent;
            if (priority_override !== undefined)
                updateData.priority_override = priority_override;
            const updated = await CustomerTierService_1.default.updateCustomerSLASettings(customerId, updateData);
            if (!updated) {
                res.status(500).json({
                    success: false,
     ...
     */
    async calculateCustomerCreditScore(req, res) {
        try {
            const customerId = parseInt(req.params.customerId);
            if (isNaN(customerId)) {
                res.status(400).json({
            ...
            const score = await CustomerTierService_1.default.calculateAndUpdateCreditScore(customerId);
            res.json({
                success: true,
...
exports.CustomerTierController = CustomerTierController;
exports.default = new CustomerTierController();
//# sourceMappingURL=CustomerTierController.js.map