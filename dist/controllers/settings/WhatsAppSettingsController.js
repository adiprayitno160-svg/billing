...
class WhatsAppSettingsController {
    ...
    static async testSendMessage(req, res) {
        try {
            ...
            try {
                await WhatsAppService_1.whatsappService.sendMessage(phone.trim(), message.trim());
                res.json({
                    success: true,
     ...
     */
    static async showMonitor(req, res) {
        try {
    ...
    static async getPendingVerifications(req, res) {
        try {
            ...
            const [pending] = await pool_1.databasePool.query(`
     ...
     */
    static async getVerificationDetail(req, res) {
        try {
            const { id } = req.params;
    ...
    static async processVerification(req, res) {
        ...
        try {
            ...
            if (action === 'reject') {
                ...
                if (verRows[0]?.phone) {
                    try {
                        await WhatsAppService_1.whatsappService.sendMessage(verRows[0].phone, `❌ *VERIFIKASI PEMBAYARAN DITOLAK*\n\n` +
            ...
            // Get invoice details
            const [invoiceRows] = await connection.query('SELECT * FROM invoices WHERE id = ?', [invoiceId]);
            ...
            await connection.query(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, created_at)
                 VALUES (?, 'transfer', ?, NOW(), ?, NOW())`, [invoiceId, paymentAmount, `Manual verification by admin - ${notes || 'Approved'}`]);
            // Update invoice
            ...
            if (isFullPayment && customerId) {
                ...
                if (unpaidCheck[0]?.count === 0) {
                    const [customerCheck] = await connection.query('SELECT is_isolated FROM customers WHERE id = ?', [customerId]);
            ...
            const [custRows] = await connection.query('SELECT phone FROM customers WHERE id = ?', [customerId]);
            await connection.commit();
            // Send success notification
            if (custRows[0]?.phone) {
                try {
                    await WhatsAppService_1.whatsappService.sendMessage(custRows[0].phone, `✅ *PEMBAYARAN BERHASIL DIVERIFIKASI*\n\n` +
     ...
     * Update Foonte Token
     */
    static async updateFoonteToken(req, res) {
        try {
