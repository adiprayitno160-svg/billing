...
exports.TechnicianSalaryController = void 0;
const TechnicianAttendanceService_1 = require("../../services/technician/TechnicianAttendanceService");
const pool_1 = require("../../db/pool");
class TechnicianSalaryController {
    // Check-In Action
    static async checkIn(req, res) {
        try {
            ...
            await TechnicianAttendanceService_1.TechnicianAttendanceService.checkIn(userId, defaultWage);
    ...
    static async viewMonthlyRecap(req, res) {
        try {
            ...
            const [technicians] = await pool_1.databasePool.query('SELECT id, full_name FROM users WHERE role = "teknisi"');
            let report = null;
            let selectedTech = null;
            if (technician_id) {
                const techId = Number(technician_id);
                report = await TechnicianAttendanceService_1.TechnicianAttendanceService.getMonthlyReport(techId, m, y);
                selectedTech = technicians.find(t => t.id === techId);
            }
    ...
    static async approveSalary(req, res) {
        try {
            await TechnicianSalaryController.ensurePaymentTable();
            const adminId = req.user?.id;
            const { technician_id, month, year, total_attendance, total_jobs, base_salary, bonus, deduction, notes } = req.body;
            ...
            const [existing] = await pool_1.databasePool.query("SELECT id FROM technician_payments WHERE technician_id = ? AND month = ? AND year = ?", [technician_id, month, year]);
            if (existing.length > 0) {
                return res.json({ success: false, error: 'Gaji untuk periode ini sudah disetujui sebelumnya.' });
            }
            // Insert Payment Record
            await pool_1.databasePool.query(`INSERT INTO technician_payments 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'paid', ?)`, [technician_id, month, year, total_attendance, total_jobs, base_salary, bonusVal, deductionVal, finalAmount, notes, adminId]);
    ...
    static async viewPaymentSummary(req, res) {
        try {
            await TechnicianSalaryController.ensurePaymentTable();
    ...
    static async printSalarySlip(req, res) {
        try {
            const { id } = req.params;
    ...
    static async viewMySalaryHistory(req, res) {
        try {
            const user = req.user;
...
exports.TechnicianSalaryController = TechnicianSalaryController;
//# sourceMappingURL=TechnicianSalaryController.js.map