...
const NotificationTemplateService_1 = require("../../services/notification/NotificationTemplateService");
...
class NotificationTemplateController {
     ...
     */
    async showTemplatesPage(req, res) {
        try {
            console.log('[NotificationTemplateController] Loading templates page...');
            // Get templates with error handling
            let templates = [];
            try {
                templates = await NotificationTemplateService_1.NotificationTemplateService.getAllTemplates();
            ...
            try {
                stats = await UnifiedNotificationService_1.UnifiedNotificationService.getStatistics(30);
     ...
     */
    async showEditTemplatePage(req, res) {
        try {
            const { code } = req.params;
            const template = await NotificationTemplateService_1.NotificationTemplateService.getTemplateByCode(code);
            if (!template) {
     ...
     */
    async showCreateTemplatePage(req, res) {
        try {
    ...
    async listTemplates(req, res) {
        try {
            ...
            const templates = await NotificationTemplateService_1.NotificationTemplateService.getAllTemplates({
    ...
    async createTemplate(req, res) {
        try {
            const { template_code, template_name, notification_type, channel, title_template, message_template, variables, is_active, priority, schedule_days_before } = req.body;
            // Validation
            ...
            const existing = await NotificationTemplateService_1.NotificationTemplateService.getTemplateByCode(template_code);
            if (existing) {
                res.status(400).json({
                    success: false,
    ...
    async updateTemplate(req, res) {
        try {
            ...
            const success = await NotificationTemplateService_1.NotificationTemplateService.updateTemplate(code, updates);
            if (!success) {
                res.status(404).json({
                    success: false,
                    message: 'Template tidak ditemukan'
    ...
    async deleteTemplate(req, res) {
        try {
            const { code } = req.params;
            const success = await NotificationTemplateService_1.NotificationTemplateService.deleteTemplate(code);
            if (!success) {
                res.status(404).json({
                    success: false,
                    message: 'Template tidak ditemukan'
    ...
    async getStatistics(req, res) {
        try {
            ...
            const stats = await UnifiedNotificationService_1.UnifiedNotificationService.getStatistics(days);
            res.json({
                success: true,
                data: stats
            });
    ...
    async testTemplate(req, res) {
        try {
            ...
            const template = await NotificationTemplateService_1.NotificationTemplateService.getTemplateByCode(template_code);
            if (!template) {
                res.status(404).json({
                    success: false,
                    message: 'Template tidak ditemukan'
    ...
    async getQueueStatus(req, res) {
        try {
           ...
           WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)`);
                ...
                const [pendingRows] = await connection.query(`SELECT 
           ...
           LIMIT 20`);
                ...
                const [failedRows] = await connection.query(`SELECT 
            id, customer_id, notification_type, channel, status, 
     ...
     */
    async debugCustomerNotification(req, res) {
        try {
            ...
            try {
                ...
                const [customerRows] = await connection.query('SELECT id, name, phone, email, customer_code FROM customers WHERE id = ?', [customerId]);
                ...
                const [queueRows] = await connection.query(`SELECT 
    ...
    async getRecentLogs(req, res) {
        try {
           ...
           LIMIT ?`, [limit]);
                ...
                try {
                    const [logRows] = await connection.query(`SELECT 
              id,
              customer_id,
              channel,
              recipient,
             ...
             LIMIT ?`, [limit]);
                ...
                let customerLogs = [];
                try {
                    const [customerLogRows] = await connection.query(`SELECT 
              id,
              customer_id,
              channel,
              notification_type,
           ...
           WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)`);
                ...
                const [customerCreatedRows] = await connection.query(`SELECT 
            unq.id,
            unq.customer_id,
            c.name as customer_name,
            c.phone as customer_phone,
     ...
     */
    async analyzeNotificationFlow(req, res) {
        try {
            const connection = await pool_1.databasePool.getConnection();
            try {
                const analysis = {
                ...
                const [recentQueue] = await connection.query(`SELECT 
            COUNT(*) as total,
            SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) as pending,
            SUM(CASE WHEN status = 'sent' THEN 1 ELSE 0 END) as sent,
            SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) as failed
           ...
           WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)`);
                ...
                const [customerCreated] = await connection.query(`SELECT 
           ...
           LIMIT 5`);
                ...
                const [failed] = await connection.query(`SELECT 
           ...
           LIMIT 10`);
                ...
                // Summary
                const allOk = analysis.checks.every((c) => c.status === 'ok' || c.status === 'ready' || c.status === 'found' || c.status === 'none' || c.status === 'no_errors');
                analysis.summary = {
                    allChecksPassed: allOk,
                    issues: analysis.checks.filter((c) => c.status !== 'ok' && c.status !== 'ready' && c.status !== 'found' && c.status !== 'none' && c.status !== 'no_errors').map((c) => ({
                        check: c.check,
                        status: c.status,
                        details: c.details
                    }))
    ...
    async retryNotification(req, res) {
        try {
            ...
            const connection = await pool_1.databasePool.getConnection();
            try {
                const [result] = await connection.query("UPDATE unified_notifications_queue SET status = 'pending', retry_count = 0, error_message = NULL WHERE id = ?", [id]);
     ...
     */
    async clearOldQueue(req, res) {
        try {
            const connection = await pool_1.databasePool.getConnection();
            try {
                // Clear pending notifications older than 7 days
                const [result] = await connection.query("DELETE FROM unified_notifications_queue WHERE status = 'pending' AND created_at < DATE_SUB(NOW(), INTERVAL 7 DAY)");
                res.json({
                    success: true,
                    message: `Cleared ${result.affectedRows} old pending notifications`
                });
            }
            finally {
                connection.release();
    ...
    async retryAllFailed(req, res) {
        try {
            ...
            try {
                ...
                const [result] = await connection.query("UPDATE unified_notifications_queue SET status = 'pending', retry_count = 0, error_message = NULL WHERE status IN ('failed', 'skipped')");
                res.json({
                    success: true,
                    message: `Berhasil mereset ${result.affectedRows} notifikasi ke antrian pending`
                });
            }
            finally {
                connection.release();
...
//# sourceMappingURL=NotificationTemplateController.js.map