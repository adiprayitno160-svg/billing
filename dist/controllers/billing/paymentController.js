...
class PaymentController {
    ...
    async getDebtTrackingList(req, res) {
        try {
            ...
            const offset = (page - 1) * limit;
            // Build query conditions
            const whereConditions = ['dt.status = ?'];
    ...
    async getPaymentForm(req, res) {
        try {
            ...
            // Get invoice details
            const [invoiceResult] = await pool_1.databasePool.query(`
                SELECT 
                    i.*,
                    c.name as customer_name,
                    c.customer_code,
            ...
            const [gateways] = await pool_1.databasePool.query(`
                SELECT id, name, code, is_active, config, created_at, updated_at FROM payment_gateways WHERE is_active = 1
            `);
            // Get payment methods for customer
            const availableMethods = await this.billingPaymentService.getAvailablePaymentMethods(invoice.customer_id);
