...
const BillingPaymentIntegration_1 = require("../../services/payment/BillingPaymentIntegration");
...
class PaymentController {
    constructor() {
        ...
        this.billingPaymentService = new BillingPaymentIntegration_1.BillingPaymentIntegration(paymentService);
    }
    /**
    ...
    async getPaymentHistory(req, res) {
        ...
        `;
            const [paymentsResult, countResult] = await Promise.all([
                pool_1.databasePool.query(paymentsQuery, [...queryParams, limit, offset]),
            ...
            ${whereClause}
        `;
            const [statsResult] = await pool_1.databasePool.query(statsQuery, queryParams);
     ...
     */
    async processFullPayment(req, res) {
        const conn = await pool_1.databasePool.getConnection();
        try {
            await conn.beginTransaction();
            const { invoice_id, payment_method, payment_date, reference_number, notes } = req.body;
            // Validate input
            ...
            try {
                const [paymentRows] = await conn.query('SELECT id FROM payments WHERE invoice_id = ? ORDER BY id DESC LIMIT 1', [invoice_id]);
                if (paymentRows.length > 0) {
                    ...
                    AccountingService.generatePaymentJournalEntry(paymentRows[0].id).catch(err => console.error('[AdminPayment] Journal Sync Error:', err));
                }
            }
            catch (accErr) {
            ...
            Promise.resolve().then(() => __importStar(require('../../services/pppoe/pppoeActivationService'))).then(({ pppoeActivationService }) => {
                pppoeActivationService.resetNextBlockDate(invoice.customer_id).catch(err => console.error('Background PPPoE reset error:', err));
     ...
     */
    async processPartialPayment(req, res) {
        const conn = await pool_1.databasePool.getConnection();
        try {
            await conn.beginTransaction();
            const { invoice_id, payment_amount, payment_method, payment_date, reference_number, notes } = req.body;
            // Validate input
            ...
            if (newRemainingAmount > 0) {
                ...
                const [existingDebt] = await conn.query('SELECT id FROM debt_tracking WHERE invoice_id = ? AND status = "active"', [invoice_id]);
                if (existingDebt.length > 0) {
                    await conn.query('UPDATE debt_tracking SET debt_amount = ?, updated_at = NOW() WHERE id = ?', [newRemainingAmount, existingDebt[0].id]);
                }
                else {
                ...
                const nextPeriod = (0, periodHelper_1.getNextPeriod)(invoice.period);
                await conn.execute('INSERT INTO carry_over_invoices (customer_id, carry_over_amount, target_period, status) VALUES (?, ?, ?, "pending") ON DUPLICATE KEY UPDATE carry_over_amount = ?', [invoice.customer_id, newRemainingAmount, nextPeriod, newRemainingAmount]);
            }
            await conn.commit();
            ...
            try {
                ...
                if (paymentRows.length > 0) {
                    const { AccountingService } = await Promise.resolve().then(() => __importStar(require('../../services/billing/accountingService')));
                    AccountingService.generatePaymentJournalEntry(paymentRows[0].id).catch(err => console.error('[AdminPartialPayment] Journal Sync Error:', err));
                }
            }
            catch (accErr) {
     ...
     */
    async processDebtPayment(req, res) {
        const conn = await pool_1.databasePool.getConnection();
        try {
            await conn.beginTransaction();
            const { invoice_id, debt_reason, due_date, notes } = req.body;
            // Validate input
            if (!invoice_id) {
                res.status(400).json({
                    success: false,
            ...
            const [existingDebt] = await conn.query('SELECT id FROM debt_tracking WHERE invoice_id = ? AND status = "active"', [invoice_id]);
            if (existingDebt.length > 0) {
                await conn.query('UPDATE debt_tracking SET debt_amount = ?, debt_reason = ?, due_date = ?, updated_at = NOW() WHERE id = ?', [remainingAmount, debt_reason || 'Pencatatan hutang pelanggan', due_date || null, existingDebt[0].id]);
            }
            else {
                const debtInsertQuery = `
                    INSERT INTO debt_tracking (
            ...
            const nextPeriod = (0, periodHelper_1.getNextPeriod)(invoice.period);
            await conn.execute('INSERT INTO carry_over_invoices (customer_id, carry_over_amount, target_period, status) VALUES (?, ?, ?, "pending") ON DUPLICATE KEY UPDATE carry_over_amount = ?', [invoice.customer_id, remainingAmount, nextPeriod, remainingAmount]);
            ...
            try {
                const [customerRows] = await pool_1.databasePool.query('SELECT name, phone, customer_code FROM customers WHERE id = ?', [invoice.customer_id]);
     ...
     */
    async getDebtTrackingList(req, res) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            let status = req.query.status || 'active';
            const search = req.query.search || '';
            const min_amount = parseFloat(req.query.min_amount) || 0;
            const format = req.query.format || '';
            const offset = (page - 1) * limit;
            // Build query conditions
            ...
            const [debtsResult, countResult] = await Promise.all([
                pool_1.databasePool.query(debtsQuery, [...queryParams, limit, offset]),
                pool_1.databasePool.query(countQuery, queryParams)
            ]);
     ...
     */
    async processGatewayPayment(req, res) {
        try {
            const { invoice_id, customer_id, gateway_code, payment_method, callback_url, redirect_url } = req.body;
    ...
    async processPayment(req, res) {
        ...
        try {
            ...
            const [invoiceRows] = await pool_1.databasePool.query('SELECT customer_id FROM invoices WHERE id = ?', [invoice_id]);
            if (invoiceRows.length === 0) {
                res.status(404).json({ success: false, message: 'Invoice tidak ditemukan' });
                return;
            }
            const customerId = invoiceRows[0].customer_id;
            const kasirName = req.session?.username || 'admin';
    ...
    async processPaymentTransaction(params) {
        ...
        try {
            await conn.beginTransaction();
            const { customerId, amount, paymentMethod, paymentType, selectedInvoiceIds, notes, kasirName, discountAmount = 0, discountReason = '', slaDiscountAmount = 0, manualDiscountValue = 0, manualDiscountType = 'manual', dueDate } = params;
            ...
            const [invoices] = await conn.query(`SELECT * FROM invoices 
                 ...
                 ORDER BY period ASC`, [selectedInvoiceIds, customerId]);
            ...
            for (const inv of invoices) {
                ...
                if (totalDiscountRemaining > 0) {
                    ...
                    if (invDiscount > 0) {
                        ...
                        if (finalSla > 0) {
                            await conn.execute('INSERT INTO discounts (invoice_id, discount_type, discount_value, reason, created_at) VALUES (?, "sla", ?, "Kompensasi SLA", NOW())', [invId, finalSla]);
                        }
                        if (finalManual > 0) {
                            await conn.execute('INSERT INTO discounts (invoice_id, discount_type, discount_value, reason, created_at) VALUES (?, ?, ?, ?, NOW())', [invId, manualDiscountType, finalManual, discountReason || 'Diskon Manual']);
                        }
                    }
                }
                ...
                if (invPayment > 0 || invDiscount > 0 || paymentType === 'debt' || paymentType === 'janji_bayar') {
                    ...
                    if (invPayment > 0) {
                        ...
                        const [pResult] = await conn.execute('INSERT INTO payments (invoice_id, payment_method, amount, payment_date, gateway_status, notes, kasir_name, created_at) VALUES (?, ?, ?, ?, "completed", ?, ?, NOW())', [invId, paymentMethod, invPayment, paymentDateStr, notes || defaultNotes, kasirName]);
                        if (!firstPaymentId)
                    ...
                    await conn.execute(`UPDATE invoices SET 
                            ...
                            last_payment_date = ?,
                            updated_at = NOW() 
                         WHERE id = ?`, [newPaid, newDiscount, newRemaining, newStatus, paymentDateStr, invId]);
                    // Sync debt tracking
                    if (newStatus === 'paid') {
                        ...
                        try {
                            const [custData] = await conn.query('SELECT c.name, c.phone, i.due_date as inv_due FROM customers c JOIN invoices i ON c.id = i.customer_id WHERE c.id = ? AND i.id = ?', [customerId, invId]);
            ...
            if (remainingPool > 0) {
                const [cust] = await conn.query('SELECT billing_mode FROM customers WHERE id = ?', [customerId]);
                if (cust.length > 0 && cust[0].billing_mode !== 'postpaid') {
                    await conn.execute('UPDATE customers SET balance = COALESCE(balance, 0) + ? WHERE id = ?', [remainingPool, customerId]);
                    await conn.execute('INSERT INTO customer_balance_logs (customer_id, type, amount, description, created_at) VALUES (?, "credit", ?, ?, NOW())', [customerId, remainingPool, 'Kelebihan pembayaran (Multi-Invoice)']);
                }
            }
            await conn.commit();
                        ...
                        if (selectedInvoiceIds.length > 0) {
                            setTimeout(async () => {
                                try {
                                    ...
                                    const [invs] = await databasePool.query('SELECT period FROM invoices WHERE id = ?', [selectedInvoiceIds[0]]);
                                    if (invs && invs.length > 0) {
                                        ...
                                        const [nextInvs] = await databasePool.query("SELECT id, status FROM invoices WHERE customer_id = ? AND period = ?", [customerId, nextPeriod]);
                                    ...
                                    console.error('[AutoRegen] Error dynamically reconstructing next invoice:', e);
                                }
