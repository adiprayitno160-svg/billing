...
class VerificationController {
    ...
    static async getList(req, res) {
        try {
            ...
            // Get total count
            const [countRows] = await pool_1.databasePool.query(`
                SELECT COUNT(*) as total FROM manual_payment_verifications WHERE status = ?
            `, [status]);
            // Get stats
            const [statsRows] = await pool_1.databasePool.query(`
                SELECT 
    ...
    static async getImage(req, res) {
        try {
            const { id } = req.params;
            const [rows] = await pool_1.databasePool.query('SELECT image_data, image_mimetype FROM manual_payment_verifications WHERE id = ?', [id]);
    ...
    */
    static async getCustomerInvoices(req, res) {
        try {
            ...
            const [invoices] = await pool_1.databasePool.query(`
    ...
    static async process(req, res) {
        ...
        try {
            ...
            if (action === 'reject') {
                // Reject verification
                await connection.query(`UPDATE manual_payment_verifications 
                     SET status = 'rejected', verified_by = ?, verified_at = NOW(), notes = ?
                     WHERE id = ?`, [userId, notes || 'Ditolak oleh admin', verificationId]);
                // Get customer info for notification
                const [verRows] = await connection.query(`SELECT mpv.customer_id, c.phone FROM manual_payment_verifications mpv
                     JOIN customers c ON mpv.customer_id = c.id WHERE mpv.id = ?`, [verificationId]);
                // Send rejection notification
                if (verRows[0]?.phone) {
                    try {
                        const { whatsappService } = await Promise.resolve().then(() => __importStar(require('../../services/whatsapp')));
                        await whatsappService.sendMessage(verRows[0].phone, `❌ *VERIFIKASI PEMBAYARAN DITOLAK*\n\n` +
            ...
            const [invoiceRows] = await connection.query('SELECT i.*, c.billing_mode FROM invoices i JOIN customers c ON i.customer_id = c.id WHERE i.id = ?', [invoiceId]);
            ...
            const isFullPayment = newRemaining <= 100; // Standardized tolerance
            ...
            const [paymentResult] = await connection.query(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, created_at)
                 VALUES (?, 'transfer', ?, NOW(), ?, NOW())`, [invoiceId, paymentAmount, `Manual verification by admin - ${notes || 'Approved'}${(excessAmount > 0 ? (invoice.billing_mode === 'postpaid' ? ` (Kelebihan Rp ${excessAmount} - Pascabayar tidak masuk saldo)` : ` (Kelebihan Rp ${excessAmount} masuk saldo)`) : '')}`]);
            ...
            await connection.query(`UPDATE invoices 
                 SET paid_amount = ?, remaining_amount = ?, status = ?, 
                     ...
                     updated_at = NOW() 
                 WHERE id = ?`, [newPaid, Math.max(0, newRemaining), newStatus, newStatus, invoiceId]);
            // Update verification record
            await connection.query(`UPDATE manual_payment_verifications 
                 SET status = 'approved', verified_by = ?, verified_at = NOW(), notes = ?, invoice_id = ?
                 WHERE id = ?`, [userId, notes || 'Approved', invoiceId, verificationId]);
            // Sync with debt_tracking for partial payments
            if (newRemaining > 0) {
                const [existingDebt] = await connection.query('SELECT id FROM debt_tracking WHERE invoice_id = ? AND status = "active"', [invoiceId]);
                if (existingDebt.length > 0) {
                    await connection.query('UPDATE debt_tracking SET debt_amount = ?, updated_at = NOW() WHERE id = ?', [newRemaining, existingDebt[0].id]);
                }
                else {
                    await connection.query(`
                ...
                try {
                    ...
                    await connection.query('INSERT INTO carry_over_invoices (customer_id, carry_over_amount, target_period, status) VALUES (?, ?, ?, "pending") ON DUPLICATE KEY UPDATE carry_over_amount = ?', [invoice.customer_id, newRemaining, nextPeriod, newRemaining]);
                }
                catch (e) {
                ...
                await connection.query('UPDATE debt_tracking SET status = "resolved", resolved_at = NOW() WHERE invoice_id = ? AND status = "active"', [invoiceId]);
            }
            // Get verification info for isolation removal
            const [verInfo] = await connection.query('SELECT customer_id FROM manual_payment_verifications WHERE id = ?', [verificationId]);
            ...
            if (isFullPayment && customerId) {
                ...
                const [unpaidCheck] = await connection.query(`SELECT COUNT(*) as count FROM invoices 
                     WHERE customer_id = ? AND id != ? AND status != 'paid' AND remaining_amount > 0`, [customerId, invoiceId]);
                if (unpaidCheck[0]?.count === 0) {
                    ...
                    Promise.resolve().then(() => __importStar(require('../../services/pppoe/pppoeActivationService'))).then(({ pppoeActivationService }) => {
                        pppoeActivationService.resetNextBlockDate(customerId).catch(err => console.error('Background PPPoE reset error:', err));
                    }).catch(e => console.error('Error importing PPPoE service:', e));
                    // Use standardized restore logic
                    Promise.resolve().then(() => __importStar(require('../../services/billing/isolationService'))).then(({ IsolationService }) => {
                        IsolationService.restoreIfQualified(customerId, connection).catch(e => console.warn('Background isolation removal failed in verification:', e));
