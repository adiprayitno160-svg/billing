...
class InvoiceController {
    ...
    async getInvoiceList(req, res) {
        try {
            ...
            const hide_isolated = req.query.hide_isolated || '';
            ...
            if (period) {
                try {
                    const [existing] = await pool_1.databasePool.query('SELECT customer_id FROM invoices WHERE period = ?', [period]);
                    excludedIds = existing.map((r) => r.customer_id);
                }
                catch (e) {
                    console.error('Error fetching excluded IDs:', e);
                }
            }
            let customerQuery = `
            ...
            const [activeResult] = await pool_1.databasePool.query(customerQuery);
            const activeCustomers = activeResult;
    ...
    async getInvoiceDetail(req, res) {
        try {
            ...
            // Get discounts
            const [discounts] = await pool_1.databasePool.query(`
    ...
    async generateBulkInvoices(req, res) {
        const { period, customer_ids, send_whatsapp } = req.body;
        const logTag = `[BulkGen]`;
        try {
            const currentPeriod = period || new Date().toISOString().slice(0, 7);
            const { InvoiceService } = await Promise.resolve().then(() => __importStar(require('../../services/billing/invoiceService')));
            // Call the smart InvoiceService
            // customer_ids can be null (means all), or an array of IDs
            const createdIds = await InvoiceService.generateMonthlyInvoices(currentPeriod, customer_ids, true // forceAll = true because this is a manual trigger from UI
            );
            res.json({
                success: true,
                message: `Berhasil membuat ${createdIds.length} tagihan.`,
    ...
    async generateBulkInvoices_UNUSED(req, res) {
        const conn = await pool_1.databasePool.getConnection();
        ...
        try {
            ...
            try {
                const [sysSettings] = await conn.query(`
                    SELECT setting_key, setting_value FROM system_settings 
                    WHERE setting_key IN ('due_date_mode', 'due_date_fixed_day', 'due_date_offset_days')
                `);
                const settingsMap = {};
                sysSettings.forEach((row) => settingsMap[row.setting_key] = row.setting_value);
                ...
                if (settingsMap['due_date_fixed_day'])
                    fixedDay = parseInt(settingsMap['due_date_fixed_day']);
                if (settingsMap['due_date_offset_days'])
                    dayOffset = parseInt(settingsMap['due_date_offset_days']);
                if (due_date_offset) {
            ...
            const [diagRows] = await conn.query('SELECT COUNT(*) as total, SUM(CASE WHEN status = "active" OR status = "Active" THEN 1 ELSE 0 END) as active_count FROM customers');
        ...
        `;
            ...
            if (customer_ids && Array.isArray(customer_ids) && customer_ids.length > 0) {
                const numericIds = customer_ids.map((id) => parseInt(id)).filter((id) => !isNaN(id));
                queryParams.push(numericIds);
                console.log(`${logTag} Filtering by IDs: ${numericIds.length} IDs`);
            }
            const [subscriptions] = await conn.query(subscriptionsQuery, queryParams);
        ...
        `;
            ...
            const customersWithGeneralInvoices = new Set();
            for (const inv of existingInvoices) {
                ...
                if (normalizedSubId === 0)
                    customersWithGeneralInvoices.add(inv.customer_id);
            }
            let createdCount = 0;
            let skippedCount = 0;
            ...
            for (const sub of sortedSubscriptions) {
                ...
                try {
                    ...
                    const [pricingSettings] = await conn.query(`
                        SELECT setting_key, setting_value FROM system_settings 
                        WHERE setting_key IN ('ppn_enabled', 'ppn_rate', 'device_rental_enabled', 'device_rental_fee')
                    `);
                    pricingSettings.forEach((row) => {
                        if (row.setting_key === 'ppn_enabled')
                            ppnEnabled = row.setting_value === 'true' || row.setting_value === '1';
                        if (row.setting_key === 'ppn_rate')
                            ppnRate = parseFloat(row.setting_value);
                        if (row.setting_key === 'device_rental_enabled')
                            deviceRentalEnabled = row.setting_value === 'true' || row.setting_value === '1';
                        if (row.setting_key === 'device_rental_fee')
                            deviceRentalFee = parseFloat(row.setting_value);
                    });
    ...
    async updateInvoiceStatus(req, res) {
        try {
            ...
            if (status === 'paid') {
                await pool_1.databasePool.query('UPDATE invoices SET status = ?, remaining_amount = 0, updated_at = NOW() WHERE id = ?', [status, id]);
            }
            else {
            ...
            if (status === 'paid') {
                try {
                    ...
                    if (rows.length > 0 && rows[0].phone) {
                        ...
                        const sendResult = await whatsappService.sendDocument(phone, pdfPath, `Invoice-${invoice_number}.pdf`, caption);
                        if (sendResult && !sendResult.success) {
                            console.error(`[Invoice] WhatsApp sendDocument failed: ${sendResult.error}`);
                        }
                        else {
                    ...
                    console.error('[Invoice] ❌ Failed to generate/send PDF:', pdfError.message);
            ...
            if (status === 'paid') {
                try {
                    ...
                    const [invRows] = await pool_1.databasePool.query('SELECT customer_id FROM invoices WHERE id = ?', [id]);
                    if (invRows.length > 0) {
                        await isolationService_1.IsolationService.restoreIfQualified(invRows[0].customer_id);
                    }
                }
                catch (unblockError) {
    ...
    async sendInvoiceWhatsApp(req, res) {
        try {
            ...
            // Check WhatsApp status first to give immediate feedback
            const { whatsappService } = await Promise.resolve().then(() => __importStar(require('../../services/whatsapp')));
            ...
            if (!waStatus.ready) {
                if (waStatus.qr) {
                    res.status(400).json({
                        success: false,
                        message: 'WhatsApp belum terhubung (Perlu Scan QR). Silakan buka menu WhatsApp Bisnis untuk login.',
                        needs_qr: true
                    });
                    return;
                }
                else if (waStatus.initializing) {
                    res.status(400).json({
                        success: false,
                        message: 'WhatsApp sedang menghubungkan ulang. Silakan tunggu 10-20 detik dan coba lagi.'
                    });
                    return;
                }
                else {
                    res.status(400).json({
                        success: false,
                        message: 'WhatsApp sedang gangguan atau terputus. Silakan cek menu WhatsApp Bisnis.'
                    });
                    return;
                }
            }
            ...
            if (invoice.status === 'paid') {
                const [paymentRows] = await pool_1.databasePool.query(`
            ...
            res.json({ success: true, message: 'Tagihan Berhasil dikirim via WhatsApp' });
        }
        catch (error) {
    ...
    async deleteInvoice(req, res) {
        ...
        try {
            ...
            const customerId = invInfo[0]?.customer_id;
            ...
            if (customerId) {
                const [checkUnpaid] = await conn.execute("SELECT COUNT(*) as count FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled')", [customerId]);
                if (checkUnpaid[0].count === 0) {
                    try {
    ...
    async bulkDeleteInvoices(req, res) {
        ...
        try {
            ...
            const validIds = invoiceIds.map((id) => parseInt(id)).filter((id) => !isNaN(id));
            ...
            // Get customer_ids before deleting to check if we should un-isolate
            const [invList] = await conn.execute(`SELECT DISTINCT customer_id FROM invoices WHERE id IN (${idListStr})`);
            const customerIds = invList.map(row => row.customer_id).filter(id => id);
            // Delete associated items and metadata in batch
            ...
            try {
                await conn.execute(`DELETE FROM customer_late_payment_tracking WHERE invoice_id IN (${idListStr})`);
            }
            catch (e) { /* ignore if table doesn't exist */ }
            // Delete invoice
            ...
            if (customerIds.length > 0) {
                for (const custId of customerIds) {
                    const [checkUnpaid] = await conn.execute("SELECT COUNT(*) as count FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled')", [custId]);
                    if (checkUnpaid[0].count === 0) {
                        try {
                            const { IsolationService } = await Promise.resolve().then(() => __importStar(require('../../services/billing/isolationService')));
                            await IsolationService.isolateCustomer({
    ...
    async updateInvoiceNotes(req, res) {
        try {
            ...
            await pool_1.databasePool.query('UPDATE invoices SET notes = ?, updated_at = NOW() WHERE id = ?', [notes, id]);
            res.json({
                success: true,
    ...
    async updateDueDate(req, res) {
        try {
            ...
            // Update invoice
            await pool_1.databasePool.query('UPDATE invoices SET due_date = ?, status = ?, updated_at = NOW() WHERE id = ?', [due_date, newStatus, id]);
     ...
     */
    async checkInvoicesForPeriod(req, res) {
        try {
            ...
            const [rows] = await pool_1.databasePool.query('SELECT customer_id FROM invoices WHERE period = ?', [period]);
     ...
     */
    async forceCleanupPeriod(req, res) {
        const conn = await pool_1.databasePool.getConnection();
        ...
        try {
            ...
            // 1. Get Invoice IDs
            const [rows] = await conn.query('SELECT id FROM invoices WHERE period = ?', [period]);
            ...
            const [result] = await conn.query('DELETE FROM invoices WHERE period = ?', [period]);
            await conn.commit();
     ...
     */
    async sendPaidInvoicePdf(req, res) {
        try {
            const { id } = req.params;
            // Get invoice data
            const [rows] = await pool_1.databasePool.query(`
            ...
            if (invoice.customer_phone) {
                ...
                const sendResult = await whatsappService.sendDocument(invoice.customer_phone, pdfPath, `Invoice-${invoice.invoice_number}-LUNAS.pdf`, caption);
                if (sendResult && !sendResult.success) {
                    throw new Error(`WhatsApp gagal mengirim: ${sendResult.error}`);
                }
     ...
     */
    async bulkSendInvoiceWhatsApp(req, res) {
        try {
            ...
            if (!waStatus.ready) {
                res.status(400).json({
                    success: false,
                    message: waStatus.qr
                        ? 'WhatsApp belum terhubung (Perlu Scan QR). Silakan buka menu WhatsApp Bisnis.'
                        : 'WhatsApp sedang gangguan atau terputus.'
                });
                return;
            }
            const { UnifiedNotificationService } = await Promise.resolve().then(() => __importStar(require('../../services/notification/UnifiedNotificationService')));
            // Fetch invoice statuses to filter
            const [rows] = await pool_1.databasePool.query('SELECT id, status FROM invoices WHERE id IN (?)', [invoice_ids]);
            ...
            const validInvoices = rows.filter(r => r.status === 'sent' || r.status === 'partial' || r.status === 'overdue');
            ...
            const promises = validInvoices.map(invoice => UnifiedNotificationService.notifyInvoiceCreated(invoice.id)
                ...
                if (invoice.status === 'draft') {
                    pool_1.databasePool.execute('UPDATE invoices SET status = "sent" WHERE id = ?', [invoice.id]);
                }
            })
     ...
     */
    async applyDowntimeDiscount(req, res) {
        try {
            ...
            const { DiscountService } = await Promise.resolve().then(() => __importStar(require('../../services/billing/discountService')));
            ...
            await DiscountService.applyDowntimeDiscount(parseInt(invoiceId), parseInt(downtimeDays), reason);
            res.json({
                success: true,
