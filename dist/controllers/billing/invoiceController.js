...
class InvoiceController {
    ...
    async getInvoiceList(req, res) {
        try {
            ...
            const monthlyStatsQuery = `
                ...
                WHERE period = ?
            `;
            const [monthlyStatsResult] = await pool_1.databasePool.query(monthlyStatsQuery, [period]);
        ...
        try {
            ...
            if (customerIds.length > 0) {
                setTimeout(async () => {
                    let tempConn;
                    try {
                        tempConn = await pool_1.databasePool.getConnection();
                        for (const custId of customerIds) {
                            const [checkUnpaid] = await tempConn.execute("SELECT COUNT(*) as count FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled')", [custId]);
    ...
    async applyManualDiscount(req, res) {
        try {
            ...
            const appliedBy = req.user?.id || null;
            if (!invoiceId) {
                res.status(400).json({ success: false, message: 'ID tagihan tidak valid' });
                return;
            }
