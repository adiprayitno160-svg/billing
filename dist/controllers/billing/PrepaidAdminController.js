...
exports.PrepaidAdminController = void 0;
const pool_1 = require("../../db/pool");
const VoucherService_1 = require("../../services/billing/VoucherService");
class PrepaidAdminController {
    /**
     * Show prepaid payment monitoring dashboard
     */
    static async paymentMonitoring(req, res) {
        try {
            ...
            if (status && status !== 'all') {
                statusFilter = `AND pr.status = ${pool_1.databasePool.escape(status)}`;
            }
            // Get payment requests
            const [paymentRequests] = await pool_1.databasePool.query(`SELECT 
                    pr.*,
            ...
            // Get counts
            const [counts] = await pool_1.databasePool.query(`SELECT 
                    COUNT(*) as total,
     ...
     */
    static async viewPaymentRequest(req, res) {
        try {
            const { id } = req.params;
            const [requests] = await pool_1.databasePool.query(`SELECT 
                    pr.*,
                    c.name as customer_name,
                    c.phone as customer_phone,
                    c.customer_code,
    ...
    static async approvePayment(req, res) {
        try {
            ...
            const result = await PrepaidService.confirmPayment(parseInt(id), userId, 'manual_approval');
            res.json(result);
        }
        catch (error) {
    ...
    static async rejectPayment(req, res) {
        try {
            ...
            await pool_1.databasePool.query(`UPDATE payment_requests 
                SET status = 'rejected', rejection_reason = ? 
                WHERE id = ?`, [reason || 'Manual rejection', id]);
    ...
    static async reports(req, res) {
        try {
            ...
            // Revenue stats
            const [revenueStats] = await pool_1.databasePool.query(`SELECT 
            ...
            const [dailyTrend] = await pool_1.databasePool.query(`SELECT 
                ...
                ORDER BY date ASC`);
            // Top vouchers
            const [voucherStats] = await pool_1.databasePool.query(`SELECT 
                    v.code,
                    v.name,
    ...
    static async listVouchers(req, res) {
        try {
            const [vouchers] = await pool_1.databasePool.query(`SELECT 
                    v.*,
    ...
    static async createVoucher(req, res) {
        try {
            ...
            const voucherId = await VoucherService_1.VoucherService.createVoucher(voucherData);
            res.json({
                success: true,
    ...
    static async updateVoucher(req, res) {
        try {
            const { id } = req.params;
            const success = await VoucherService_1.VoucherService.updateVoucher(parseInt(id), req.body);
            res.json({
                success,
    ...
    static async deleteVoucher(req, res) {
        try {
            const { id } = req.params;
            const success = await VoucherService_1.VoucherService.deleteVoucher(parseInt(id));
            res.json({
                success,
     ...
     * Referral tracking
     */
    static async referralTracking(req, res) {
        try {
...
exports.PrepaidAdminController = PrepaidAdminController;
