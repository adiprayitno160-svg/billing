...
async function postStaticIpPackageCreate(req, res, next) {
    try {
        ...
        if (!child_upload_name)
            throw new Error('Child upload name wajib diisi');
        if (!child_download_name)
            throw new Error('Child download name wajib diisi');
...
async function postStaticIpPackageUpdate(req, res, next) {
    try {
        ...
        if (max_clients && Number(max_clients) < 1)
            throw new Error('Max clients harus minimal 1');
        if (child_upload_name && !child_upload_name.trim())
            throw new Error('Child upload name tidak boleh kosong');
        if (child_download_name && !child_download_name.trim())
            throw new Error('Child download name tidak boleh kosong');
        if (price && Number(price) < 0)
            throw new Error('Harga harus lebih dari 0');
        if (duration_days && Number(duration_days) < 1)
            throw new Error('Durasi harus minimal 1 hari');
