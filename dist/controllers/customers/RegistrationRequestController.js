...
exports.RegistrationRequestController = void 0;
const pool_1 = require("../../db/pool");
const WhatsAppService_1 = require("../../services/whatsapp/WhatsAppService"); // Adjust path if needed
class RegistrationRequestController {
    static async index(req, res) {
        try {
            const [requests] = await pool_1.databasePool.query(`SELECT * FROM registration_requests ORDER BY created_at DESC`);
    ...
    static async approve(req, res) {
        ...
        try {
            ...
            // 1. Get Request Data
            const [rows] = await connection.query('SELECT * FROM registration_requests WHERE id = ? FOR UPDATE', [id]);
            if (rows.length === 0) {
            ...
            const [maxIdRows] = await connection.query('SELECT MAX(id) as maxId FROM customers');
            ...
            const [result] = await connection.query(`INSERT INTO customers (
                    ...
                    latitude, longitude
                ) VALUES (?, ?, ?, ?, 'inactive', NOW(), NOW(), ?, ?)`, [customerCode, request.name, request.address, request.phone, request.latitude, request.longitude]);
            const newCustomerId = result.insertId;
            // 4. Update Request Status
            await connection.query('UPDATE registration_requests SET status = ?, updated_at = NOW() WHERE id = ?', ['approved', id]);
            await connection.commit();
            // 5. Notify User via WhatsApp
            try {
                const whatsappService = WhatsAppService_1.WhatsAppService.getInstance();
                await whatsappService.sendMessage(request.phone, `🎉 *Selamat!*\n\nRegistrasi Anda a.n *${request.name}* telah diterima.\nID Pelanggan Anda: *${customerCode}*.\n\nAdmin kami akan segera menghubungi Anda untuk pemasangan/aktivasi paket.`);
            }
            catch (waError) {
    ...
    static async reject(req, res) {
        ...
        try {
            ...
            const [rows] = await pool_1.databasePool.query('SELECT * FROM registration_requests WHERE id = ?', [id]);
            if (rows.length === 0) {
                req.flash('error', 'Data tidak ditemukan.');
                return res.redirect('/customers/registration-requests');
            }
            const request = rows[0];
            // 2. Update Status
            await pool_1.databasePool.query('UPDATE registration_requests SET status = ?, notes = ?, updated_at = NOW() WHERE id = ?', ['rejected', reason || 'Tidak memenuhi syarat', id]);
            // 3. Notify User via WhatsApp
            try {
                const whatsappService = WhatsAppService_1.WhatsAppService.getInstance();
                await whatsappService.sendMessage(request.phone, `⚠️ *Maaf*\n\nRegistrasi Anda a.n *${request.name}* belum dapat kami terima.\nAlasan: ${reason || 'Data tidak valid/lengkap'}.\n\nSilakan hubungi admin untuk info lebih lanjut.`);
            }
            catch (waError) {
...
exports.RegistrationRequestController = RegistrationRequestController;
//# sourceMappingURL=RegistrationRequestController.js.map