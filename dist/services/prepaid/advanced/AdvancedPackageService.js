...
exports.AdvancedPackageService = void 0;
const pool_1 = require("../../../db/pool");
class AdvancedPackageService {
    /**
    ...
    async getPackageById(id) {
        const [rows] = await pool_1.databasePool.query('SELECT * FROM prepaid_packages_v2 WHERE id = ?', [id]);
        if (rows.length === 0) {
            return null;
        }
    ...
    async getPackageByCode(code) {
        const [rows] = await pool_1.databasePool.query('SELECT * FROM prepaid_packages_v2 WHERE package_code = ?', [code]);
        if (rows.length === 0) {
            return null;
        }
        return this.mapRowToPackage(rows[0]);
    ...
    async createPackage(pkg) {
        ...
        try {
            ...
            const [result] = await connection.query(`INSERT INTO prepaid_packages_v2 (
    ...
    async updatePackage(id, pkg) {
        ...
        try {
            ...
            params.push(id);
            await connection.query(`UPDATE prepaid_packages_v2 SET ${updateFields.join(', ')}, updated_at = NOW() WHERE id = ?`, params);
            await connection.commit();
            return true;
        }
        catch (error) {
    ...
    async deletePackage(id) {
        const [result] = await pool_1.databasePool.query('UPDATE prepaid_packages_v2 SET is_active = 0 WHERE id = ?', [id]);
        return result.affectedRows > 0;
    }
    /**
     * Calculate final price with discounts
     */
    calculateFinalPrice(pkg, voucherDiscount) {
     ...
     */
    async getFeaturedPackages(limit = 3) {
...
exports.AdvancedPackageService = AdvancedPackageService;
exports.default = new AdvancedPackageService();
//# sourceMappingURL=AdvancedPackageService.js.map