...
const MikrotikService_1 = __importDefault(require("../mikrotik/MikrotikService"));
...
class AddressListService {
    constructor() {
        this.mikrotikService = MikrotikService_1.default;
    }
    /**
     * Get portal-redirect address list
     */
    async getPortalRedirectList() {
        const [rows] = await pool_1.default.query('SELECT * FROM mikrotik_address_lists WHERE purpose = ? AND is_active = 1', ['portal-redirect']);
        return rows.length > 0 ? rows[0] : null;
    }
    /**
     * Add customer to portal-redirect list
     */
    async addToPortalRedirect(customerId, reason = 'No active package') {
        try {
            ...
            const [customerRows] = await pool_1.default.query('SELECT customer_code, name FROM customers WHERE id = ?', [customerId]);
            const customer = customerRows[0];
            // Add to database first
            const [result] = await pool_1.default.query(`INSERT INTO mikrotik_address_list_items 
         (address_list_id, customer_id, ip_address, reason, sync_status, auto_remove)
         VALUES (?, ?, ?, ?, 'pending', 1)`, [list.id, customerId, customerIP, reason]);
            ...
            try {
                ...
                await pool_1.default.query(`UPDATE mikrotik_address_list_items 
           SET mikrotik_entry_id = ?, sync_status = 'synced', synced_at = NOW()
           WHERE id = ?`, [mikrotikEntryId, itemId]);
                console.log(`✅ Customer ${customerId} added to portal-redirect list: ${customerIP}`);
                return true;
            }
            catch (error) {
                // Mark as failed
                await pool_1.default.query(`UPDATE mikrotik_address_list_items SET sync_status = 'failed' WHERE id = ?`, [itemId]);
     ...
     */
    async removeFromPortalRedirect(customerId) {
        try {
            // Get portal-redirect list
            const list = await this.getPortalRedirectList();
            if (!list) {
            ...
            const [items] = await pool_1.default.query(`SELECT * FROM mikrotik_address_list_items 
         WHERE customer_id = ? AND address_list_id = ?`, [customerId, list.id]);
            if (items.length === 0) {
     ...
     */
    async isCustomerInList(customerId, listId) {
        const [rows] = await pool_1.default.query(`SELECT id FROM mikrotik_address_list_items 
       WHERE customer_id = ? AND address_list_id = ?`, [customerId, listId]);
        return rows.length > 0;
    }
    /**
     * Get customer IP address
     */
    async getCustomerIP(customerId) {
       ...
       WHERE c.id = ?`, [customerId]);
        ...
        if (customer.connection_type === 'static_ip' && customer.ip_address) {
            ...
            return (0, ipHelper_1.calculateCustomerIP)(customer.ip_address);
        }
     ...
     */
    async getCustomersInPortalRedirect() {
        const list = await this.getPortalRedirectList();
        if (!list)
            return [];
        const [rows] = await pool_1.default.query(`SELECT 
       ...
       ORDER BY ali.created_at DESC`, [list.id]);
        return rows;
    }
    /**
    ...
    async syncPendingItems() {
        const [items] = await pool_1.default.query(`SELECT ali.*, mal.list_name, c.customer_code
       FROM mikrotik_address_list_items ali
       ...
       WHERE ali.sync_status = 'pending'`);
        ...
        for (const item of items) {
            try {
                ...
                await pool_1.default.query(`UPDATE mikrotik_address_list_items 
           SET mikrotik_entry_id = ?, sync_status = 'synced', synced_at = NOW()
           WHERE id = ?`, [mikrotikEntryId, item.id]);
                synced++;
            }
            catch (error) {
                await pool_1.default.query(`UPDATE mikrotik_address_list_items SET sync_status = 'failed' WHERE id = ?`, [item.id]);
                failed++;
            }
        }
    ...
    async cleanExpiredEntries() {
        const [result] = await pool_1.default.query(`DELETE FROM mikrotik_address_list_items 
     ...
     */
    async getCustomerAddressLists(customerId) {
        const [rows] = await pool_1.default.query(`SELECT 
        ali.*,
        mal.list_name,
        mal.description,
       ...
       ORDER BY ali.created_at DESC`, [customerId]);
        return rows;
    }
}
exports.default = new AddressListService();
