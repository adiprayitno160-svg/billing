...
exports.PrepaidQueueService = void 0;
const node_routeros_1 = require("node-routeros");
const pool_1 = __importDefault(require("../../db/pool"));
class PrepaidQueueService {
    constructor(config) {
        this.config = config;
    }
    /**
     ...
     */
    static async getMikrotikConfig() {
        try {
    ...
    async getParentQueues() {
        const connection = new node_routeros_1.RouterOSAPI({
     ...
     */
    async createOrUpdateQueue(params) {
        const connection = new node_routeros_1.RouterOSAPI({
            host: this.config.host,
            user: this.config.username,
            password: this.config.password,
     ...
     * Find queue by name
     */
    async findQueueByName(connection, queueName) {
        try {
            const queues = (await connection.write('/queue/tree/print', [
                `?name=${queueName}`,
            ]));
     ...
     */
    async removeQueue(customerName) {
        const connection = new node_routeros_1.RouterOSAPI({
            host: this.config.host,
            user: this.config.username,
            password: this.config.password,
     ...
     */
    async checkMangleRules(ipAddress) {
        const connection = new node_routeros_1.RouterOSAPI({
            host: this.config.host,
            user: this.config.username,
            password: this.config.password,
...
exports.PrepaidQueueService = PrepaidQueueService;
exports.default = PrepaidQueueService;
//# sourceMappingURL=PrepaidQueueService.js.map