...
exports.PrepaidPaymentService = void 0;
...
class PrepaidPaymentService {
    ...
    static async createTransaction(data) {
        try {
            const [result] = await pool_1.default.query(`INSERT INTO prepaid_transactions (
          ...
          payment_proof_url, payment_gateway_reference, payment_gateway_type, payment_notes
            ...
            console.log(`[PrepaidPaymentService] Transaction created: ID ${result.insertId}, Customer ${data.customer_id}, Amount ${data.amount}`);
    ...
    static async getTransactionById(transactionId) {
        try {
            const [rows] = await pool_1.default.query(`SELECT * FROM prepaid_transactions WHERE id = ?`, [transactionId]);
    ...
    static async updateTransactionStatus(transactionId, status, additionalData) {
        try {
            ...
            await pool_1.default.query(`UPDATE prepaid_transactions SET ${setClause} WHERE id = ?`, [...values, transactionId]);
     ...
     */
    static async verifyPayment(transactionId, adminId, notes) {
        const connection = await pool_1.default.getConnection();
        try {
            await connection.beginTransaction();
            // Update transaction status
            await connection.query(`UPDATE prepaid_transactions 
         SET payment_status = 'verified', verified_at = NOW(), verified_by = ?
         WHERE id = ?`, [adminId, transactionId]);
            // Log verification
            await connection.query(`INSERT INTO prepaid_payment_verification_log 
         (transaction_id, admin_id, action, notes, ip_address)
         VALUES (?, ?, 'approve', ?, ?)`, [transactionId, adminId, notes || null, null] // IP can be added from req.ip
            );
     ...
     */
    static async rejectPayment(transactionId, adminId, reason) {
        const connection = await pool_1.default.getConnection();
        try {
            await connection.beginTransaction();
            // Update transaction status
         ...
         WHERE id = ?`, [reason, transactionId]);
            // Log rejection
            await connection.query(`INSERT INTO prepaid_payment_verification_log 
         (transaction_id, admin_id, action, notes, ip_address)
         VALUES (?, ?, 'reject', ?, ?)`, [transactionId, adminId, reason, null]);
            await connection.commit();
     ...
     */
    static async savePaymentProof(file, transactionId) {
        try {
            const uploadDir = path_1.default.join(__dirname, '../../../public/uploads/payment-proofs');
            ...
            // Update transaction
            await pool_1.default.query('UPDATE prepaid_transactions SET payment_proof_url = ? WHERE id = ?', [relativeUrl, transactionId]);
    ...
    static async expirePendingPayments() {
        try {
            ...
            const [result] = await pool_1.default.query(`UPDATE prepaid_transactions
         SET payment_status = 'expired', expired_at = NOW()
         WHERE payment_status = 'pending'
           AND payment_method = 'manual_transfer'
           AND TIMESTAMPDIFF(HOUR, created_at, NOW()) >= ?`, [expireHours]);
     ...
     */
    static async getPaymentStatistics(dateFrom, dateTo) {
        try {
...
exports.PrepaidPaymentService = PrepaidPaymentService;
exports.default = PrepaidPaymentService;
//# sourceMappingURL=PrepaidPaymentService.js.map