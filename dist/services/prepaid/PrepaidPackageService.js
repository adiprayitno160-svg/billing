...
exports.PrepaidPackageService = void 0;
const pool_1 = __importDefault(require("../../db/pool"));
const PrepaidQueueService_1 = require("./PrepaidQueueService");
const AutoMigrationService_1 = require("./AutoMigrationService");
class PrepaidPackageService {
          ...
          ORDER BY connection_type, price ASC`);
                ...
                if (descError.message && (descError.message.includes("description") || descError.message.includes("connection_type"))) {
                    ...
                    try {
                        ...
                        const fixed = await AutoMigrationService_1.AutoMigrationService.fixPrepaidPackagesTable();
                        if (fixed) {
     ...
     */
    static async getActivePackagesByType(connectionType) {
        try {
            const [rows] = await pool_1.default.query(`SELECT 
          id, name, description, connection_type,
          mikrotik_profile_name, parent_download_queue, parent_upload_queue,
        ...
        ORDER BY price ASC`, [connectionType]);
            return rows;
        }
        catch (error) {
    ...
    static async updatePackage(packageId, packageData) {
        try {
            ...
            await pool_1.default.query(`UPDATE prepaid_packages SET
          name = ?,
          description = ?,
          connection_type = ?,
          mikrotik_profile_name = ?,
    ...
    static async deletePackage(packageId) {
        try {
            ...
            const [subscriptions] = await pool_1.default.query(`SELECT COUNT(*) as count FROM prepaid_subscriptions 
         WHERE package_id = ? AND status = 'active'`, [packageId]);
            ...
            // Soft delete
            await pool_1.default.query('UPDATE prepaid_packages SET is_active = 0 WHERE id = ?', [packageId]);
     ...
     */
    static async getParentQueuesFromMikrotik() {
        try {
            ...
            const queueService = new PrepaidQueueService_1.PrepaidQueueService(mikrotikConfig);
    ...
    static validatePackageData(packageData) {
        ...
        // PPPoE validation
        if (packageData.connection_type === 'pppoe' &&
     ...
     */
    static async getCustomerConnectionType(customerId) {
        try {
            const [rows] = await pool_1.default.query('SELECT connection_type, pppoe_username FROM customers WHERE id = ?', [customerId]);
...
exports.PrepaidPackageService = PrepaidPackageService;
exports.default = PrepaidPackageService;
//# sourceMappingURL=PrepaidPackageService.js.map