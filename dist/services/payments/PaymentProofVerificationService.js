...
exports.PaymentProofVerificationService = void 0;
...
class PaymentProofVerificationService {
     ...
     */
    static async verify(buffer, expectedAmount, customerName, customerPhone) {
        const { AISettingsService } = await Promise.resolve().then(() => __importStar(require('../payment/AISettingsService')));
        const settings = await AISettingsService.getSettings();
        const proofHash = crypto.createHash('sha256').update(buffer).digest('hex');
        const [dupRows] = await pool_1.databasePool.query(`SELECT id, invoice_number FROM invoices WHERE proof_image_hash = ? AND status = 'paid' LIMIT 1`, [proofHash]);
        if (dupRows.length > 0) {
            return {
                status: 'rejected',
        ...
        `;
        try {
            const responseText = await ChatBotService_1.ChatBotService.analyzeImage(buffer, 'image/jpeg', prompt);
            ...
            if (data.transaction_date) {
                ...
                if (diffDays <= 30)
                    isDateValid = true; // LOOSENED TO 30 DAYS FOR TESTING IF USER KEEPS GETTING REJECTED
            }
            else {
            ...
            let reviewReasons = [];
            if (!isAmountValid)
                reviewReasons.push(`Nominal beda (Bukti: ${extractedAmount}, Tagihan: ${expectedAmount})`);
            if (!isDateValid)
                reviewReasons.push(`Tanggal transaksi kadaluarsa/tidak terbaca (${data.transaction_date})`);
            if (!isNameValid)
                reviewReasons.push(`Nama pengirim tidak sesuai & tidak ada catatan nama pelanggan.`);
...
exports.PaymentProofVerificationService = PaymentProofVerificationService;
//# sourceMappingURL=PaymentProofVerificationService.js.map