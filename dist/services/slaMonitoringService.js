...
const incidentAIService_1 = __importDefault(require("./monitoring/incidentAIService"));
class SLAMonitoringService {
    constructor() {
        this.DOWNTIME_THRESHOLD_MINUTES = 30;
        this.TRANSIENT_THRESHOLD_MINUTES = 30;
    }
    /**
     * Detect and create new downtime incidents from connection logs
     * Called every 5 minutes by scheduler
     */
    async detectDowntimeIncidents() {
        ...
        try {
            ...
            `;
            const [rows] = await pool_1.default.query(query, [this.DOWNTIME_THRESHOLD_MINUTES]);
            ...
            for (const row of rows) {
                ...
                incidentAIService_1.default.analyzeIncident(incidentId).catch(error => {
     ...
     */
    async excludeTransientIncidents() {
        ...
        try {
            ...
            `;
            const [result] = await pool_1.default.query(query, [this.TRANSIENT_THRESHOLD_MINUTES]);
            ...
            for (const schedule of schedules) {
                ...
                const query = `
                    ...
                    WHERE customer_id IN (${placeholders})
                        ...
                        AND exclude_reason IS NULL
                `;
                const [result] = await pool_1.default.query(query, [schedule.id, ...affectedCustomers, schedule.start_time, schedule.end_time]);
     ...
     */
    async excludeIsolatedCustomerIncidents() {
     ...
     */
    async calculateMonthlySLA(monthYear) {
        ...
        try {
            ...
            for (const customer of customers) {
                await this.calculateCustomerMonthlySLA(customer.customer_id, targetMonth, totalMinutes, customer.sla_target);
            }
     ...
     */
    async calculateCustomerMonthlySLA(customerId, monthYear, totalMinutes, slaTarget) {
     ...
     */
    async calculateDiscount(customerId, actualSLA, targetSLA) {
        ...
        const [settings] = await pool_1.default.query(`
            SELECT 
                ss.discount_rate,
     ...
     */
    async getCustomerSLASummary(customerId, monthYear) {
        const targetMonth = monthYear || new Date(new Date().getFullYear(), new Date().getMonth(), 1);
     ...
     */
    async getCustomerActiveIncidents(customerId) {
        const [rows] = await pool_1.default.query(`
     ...
     */
    async approveDiscount(slaRecordId, approvedBy) {
        await pool_1.default.query(`
            UPDATE sla_records
            SET 
                discount_approved = 1,
...
exports.default = new SLAMonitoringService();
//# sourceMappingURL=slaMonitoringService.js.map