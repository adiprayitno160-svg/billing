...
async function getPackageById(id) {
    ...
    try {
        ...
        if (result && result.profile_id) {
            ...
            result.rate_limit_rx = result.rate_limit_rx || result.profile_rate_limit_rx || '0';
            result.rate_limit_tx = result.rate_limit_tx || result.profile_rate_limit_tx || '0';
            // Burst limits (Priority: Package > Profile)
            result.burst_limit_rx = result.burst_limit_rx || result.profile_burst_limit_rx || null;
            result.burst_limit_tx = result.burst_limit_tx || result.profile_burst_limit_tx || null;
            result.burst_threshold_rx = result.burst_threshold_rx || result.profile_burst_threshold_rx || null;
            result.burst_threshold_tx = result.burst_threshold_tx || result.profile_burst_threshold_tx || null;
            result.burst_time_rx = result.burst_time_rx || result.profile_burst_time_rx || null;
            result.burst_time_tx = result.burst_time_tx || result.profile_burst_time_tx || null;
        }
        return result;
    }
    finally {
...
async function createPackage(data) {
    ...
    try {
        ...
        if (!data.profile_id) {
            ...
            const [existingProfiles] = await conn.execute('SELECT id FROM pppoe_profiles WHERE name = ?', [data.name]);
            if (Array.isArray(existingProfiles) && existingProfiles.length > 0) {
                ...
                const [pResult] = await conn.execute(`
                    ...
                    VALUES (?, NOW(), NOW())
                `, [data.name]);
                data.profile_id = pResult.insertId;
                ...
                try {
                    const config = await getMikrotikConfig();
                    if (config) {
                        const { createPppProfile, findPppProfileIdByName } = await Promise.resolve().then(() => __importStar(require('./mikrotikService')));
        ...
        const [result] = await conn.execute(`
			...
			(name, profile_id, price, duration_days, auto_activation, status, description,
                ...
                max_clients, priority, limit_at_upload, limit_at_download, burst_enabled)
			VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
				`, [
            data.name,
        ...
        const parseRateToNum = (val) => {
            ...
            if (val.toLowerCase().includes('m'))
                return num * 1000000;
            if (val.toLowerCase().includes('g'))
                return num * 1000000000;
            return num * 1000000; // Default to Mbps if numeric only
        };
        const formatRateFromNum = (val) => {
            if (val >= 1000000)
        ...
        */
        ...
        try {
            ...
            if (config && data.profile_id) {
                ...
                if (profile) {
                    const { findPppProfileIdByName, updatePppProfile } = await Promise.resolve().then(() => __importStar(require('./mikrotikService')));
                    ...
                    if (mikrotikId) {
                        ...
                        const updateData = {
                            ...
                            'burst-limit-rx': data.burst_enabled ? (data.burst_limit_rx || '') : '',
                            'burst-limit-tx': data.burst_enabled ? (data.burst_limit_tx || '') : '',
                            'burst-threshold-rx': data.burst_enabled ? (data.burst_threshold_rx || '') : '',
                            'burst-threshold-tx': data.burst_enabled ? (data.burst_threshold_tx || '') : '',
                            'burst-time-rx': data.burst_enabled ? (data.burst_time_rx || '') : '',
                            'burst-time-tx': data.burst_enabled ? (data.burst_time_tx || '') : '',
...
async function updatePackage(id, data) {
    ...
    try {
        ...
        const [packageRows] = await conn.execute('SELECT * FROM pppoe_packages WHERE id = ?', [id]);
        ...
        await conn.execute(`
            ...
            data.is_enabled_7_days !== undefined ? data.is_enabled_7_days : null,
            data.is_enabled_14_days !== undefined ? data.is_enabled_14_days : null,
            data.is_enabled_30_days !== undefined ? data.is_enabled_30_days : null,
            data.max_clients !== undefined ? data.max_clients : null,
            ...
            data.burst_enabled !== undefined ? data.burst_enabled : null,
            id
        ]);
        ...
        const parseRateToNumUpdate = (val) => {
            if (!val)
                return 0;
            const num = parseFloat(val);
            if (val.toLowerCase().includes('k'))
        ...
        const formatRateFromNumUpdate = (val) => {
            if (val >= 1000000)
                return `${Math.floor(val / 1000000)}M`;
            if (val >= 1000)
                return `${Math.floor(val / 1000)}k`;
        ...
        try {
            const config = await getMikrotikConfig();
            if (config) {
                const { createSimpleQueue, updateSimpleQueue, deleteSimpleQueue, findSimpleQueueIdByName, findPppProfileIdByName, updatePppProfile } = await Promise.resolve().then(() => __importStar(require('./mikrotikService')));
                ...
                if (targetProfileId) {
                    ...
                    if (profile) {
                        ...
                        if (mikrotikId) {
                            ...
                            const isBurstEnabled = data.burst_enabled !== undefined ? data.burst_enabled : (currentPackage.burst_enabled || 0);
                            const updateData = {
                                'parent-queue': 'none', // DISABLED SHARED PARENT QUEUE by User Request
                                'rate-limit-rx': profileRxCap,
                                'rate-limit-tx': profileTxCap,
                                'burst-limit-rx': isBurstEnabled ? (data.burst_limit_rx !== undefined ? data.burst_limit_rx : currentPackage.burst_limit_rx || '') : '',
                                'burst-limit-tx': isBurstEnabled ? (data.burst_limit_tx !== undefined ? data.burst_limit_tx : currentPackage.burst_limit_tx || '') : '',
                                'burst-threshold-rx': isBurstEnabled ? (data.burst_threshold_rx !== undefined ? data.burst_threshold_rx : currentPackage.burst_threshold_rx || '') : '',
                                'burst-threshold-tx': isBurstEnabled ? (data.burst_threshold_tx !== undefined ? data.burst_threshold_tx : currentPackage.burst_threshold_tx || '') : '',
                                'burst-time-rx': isBurstEnabled ? (data.burst_time_rx !== undefined ? data.burst_time_rx : currentPackage.burst_time_rx || '') : '',
                                'burst-time-tx': isBurstEnabled ? (data.burst_time_tx !== undefined ? data.burst_time_tx : currentPackage.burst_time_tx || '') : '',
                            ...
                            if (finalMaxClients > 1 && (!updateData['limit-at-rx'] || !updateData['limit-at-tx'])) {
                                ...
                                if (rxB > 0)
                                    updateData['limit-at-rx'] = formatRateFromNumUpdate(rxB / finalMaxClients);
                                if (txB > 0)
                                    updateData['limit-at-tx'] = formatRateFromNumUpdate(txB / finalMaxClients);
                            }
                            await updatePppProfile(config, mikrotikId, updateData);
...
async function deletePackage(id) {
    ...
    try {
        ...
        try {
            const config = await getMikrotikConfig();
            if (config) {
                const { deleteSimpleQueue, findSimpleQueueIdByName, deletePppProfile, findPppProfileIdByName } = await Promise.resolve().then(() => __importStar(require('./mikrotikService')));
    ...
    try {
        ...
        if (profile) {
            try {
                const config = await getMikrotikConfig();
                if (config) {
                    const { findPppProfileIdByName, deletePppProfile } = await Promise.resolve().then(() => __importStar(require('./mikrotikService')));
