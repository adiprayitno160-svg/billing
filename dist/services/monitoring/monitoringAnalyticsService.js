...
class MonitoringAnalyticsService {
     ...
     */
    static async getTodayBandwidthStats() {
        try {
            const query = `
                SELECT 
     ...
     */
    static async getBandwidthTrend(days = 30) {
        try {
            const query = `
                SELECT 
                ...
                ORDER BY date DESC
            `;
            const [rows] = await pool_1.databasePool.query(query, [days]);
            return rows.map(row => ({
                date: row.date,
     ...
     */
    static async getTopCustomersByBandwidth(limit = 10, days = 7) {
        try {
            const query = `
                SELECT 
                    bl.customer_id,
                ...
                LIMIT ?
            `;
            const [rows] = await pool_1.databasePool.query(query, [days, limit]);
            return rows.map(row => ({
    ...
    static async getNetworkHealth() {
        try {
            ...
            const [customerStats] = await pool_1.databasePool.query(`
                SELECT 
            ...
            const [onlineStats] = await pool_1.databasePool.query(`
                SELECT 
            ...
            const [degradedStats] = await pool_1.databasePool.query(`
            ...
            `);
            // Get average latency
            const [latencyStats] = await pool_1.databasePool.query(`
                SELECT 
            ...
            const [uptimeStats] = await pool_1.databasePool.query(`
                SELECT 
     ...
     */
    static async getBandwidthByArea(days = 7) {
        try {
            const query = `
                SELECT 
     ...
     */
    static async getBandwidthSummary() {
        try {
...
exports.default = MonitoringAnalyticsService;
//# sourceMappingURL=monitoringAnalyticsService.js.map