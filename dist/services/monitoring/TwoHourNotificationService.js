...
exports.TwoHourNotificationService = void 0;
const pool_1 = require("../../db/pool");
const WhatsAppService_1 = require("../whatsapp/WhatsAppService");
class TwoHourNotificationService {
    constructor() {
     ...
     */
    async processLongTermOfflineCustomers() {
     ...
     * Find customers that have been offline for 2+ hours
     */
    async getLongTermOfflineCustomers() {
        try {
     ...
     */
    async handleOfflineCustomerNotification(customer) {
        try {
     ...
     */
    async sendOfflineNotification(customer, ticketInfo) {
        try {
            if (!customer.phone) {
     ...
     */
    async sendRecoveryNotification(customer, ticketNumber) {
        try {
            if (!customer.phone) {
     ...
     */
    async getOrCreateTicketForCustomer(customer) {
        try {
            ...
            const [existingTickets] = await pool_1.databasePool.query(`
            ...
            const [newTicket] = await pool_1.databasePool.query(`
    ...
    async shouldSendNotification(customerId, eventType) {
        try {
            ...
            const [lastNotification] = await pool_1.databasePool.query(`
                SELECT notified_at 
     ...
     */
    async processRecoveredCustomers() {
        ...
        try {
            ...
            for (const customer of recoveredCustomers) {
                ...
                const [recentTickets] = await pool_1.databasePool.query(`
     ...
     * Find customers that were offline for 2+ hours but are now online
     */
    async getRecentlyRecoveredCustomers() {
        try {
            const query = `
                SELECT 
                    c.id,
...
exports.TwoHourNotificationService = TwoHourNotificationService;
//# sourceMappingURL=TwoHourNotificationService.js.map