...
class AdvancedMonitoringService {
    ...
    static async getCustomersForMap(forceRefresh = false) {
        ...
        let odc_olt_links = [];
        try {
            const [odcs] = await pool_1.databasePool.query(`
                SELECT 
    ...
    static async refreshCache() {
        ...
        try {
            ...
            const [customerStatusCache] = await pool_1.databasePool.query(`
                SELECT 
                    customer_id,
                    status,
                    response_time_ms,
            ...
            customerStatusCache.forEach((status) => {
    ...
    static async runOptimizedMonitoringCycle() {
        ...
        try {
            ...
            if (staticIPCustomers.length > 0) {
                ...
                let arpList = [];
                if (mikrotikConfig) {
                    arpList = await (0, mikrotikService_1.getArpList)(mikrotikConfig);
                }
