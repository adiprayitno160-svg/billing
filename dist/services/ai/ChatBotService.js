...
class ChatBotService {
    static async initialize() {
        ...
        let apiKey = process.env.GEMINI_API_KEY || (await AISettingsService_1.AISettingsService.getAPIKey());
        if (!apiKey)
        ...
        // Sanity check for model name (fix common database issues and deprecated models)
        if (modelName === 'gemini-pro' || modelName === 'gemini-1.0-pro')
            modelName = 'gemini-1.5-pro';
        if (!modelName || modelName.includes('gemini-pro'))
            modelName = 'gemini-1.5-flash'; // Fallback to flash for safety
     ...
     */
    static async ask(userQuery, customerData) {
        ...
        try {
            ...
            relevantDocs = await KnowledgeBaseService_1.KnowledgeBaseService.search(userQuery);
            let context = '';
     ...
     */
    static async analyzeImage(imageBuffer, mimeType, promptText) {
        if (this.isDisabled()) {
...
ChatBotService.genAI = null;
ChatBotService.model = null;
// Circuit‑breaker state
ChatBotService.failureCount = 0;
ChatBotService.FAILURE_THRESHOLD = 3; // attempts before disabling
ChatBotService.COOLDOWN_MS = 5 * 60 * 1000; // 5 minutes
ChatBotService.disabledUntil = null;
