...
class UserService {
    ...
    async getAllUsers() {
        try {
            const [rows] = await pool_1.databasePool.execute('SELECT id, username, email, phone, full_name, role, is_active, created_at, updated_at FROM users ORDER BY created_at DESC');
            return rows;
        }
        catch (error) {
    ...
    async getUserById(id) {
        try {
            const [rows] = await pool_1.databasePool.execute('SELECT id, username, email, phone, full_name, role, is_active, session_id, created_at, updated_at FROM users WHERE id = ?', [id]);
            const users = rows;
    ...
    async getUserByUsername(username) {
        try {
            const [rows] = await pool_1.databasePool.execute('SELECT id, username, email, phone, full_name, role, is_active, created_at, updated_at FROM users WHERE username = ?', [username]);
            const users = rows;
            return users.length > 0 ? users[0] : null;
        }
        catch (error) {
    ...
    async getUserByEmail(email) {
        try {
            const [rows] = await pool_1.databasePool.execute('SELECT id, username, email, phone, full_name, role, is_active, created_at, updated_at FROM users WHERE email = ?', [email]);
            const users = rows;
            return users.length > 0 ? users[0] : null;
        }
        catch (error) {
    ...
    async createUser(userData) {
        try {
            ...
            const [result] = await pool_1.databasePool.execute('INSERT INTO users (username, email, phone, password, role, full_name, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, NOW(), NOW())', [userData.username, userData.email, userData.phone || null, hashedPassword, userData.role, userData.full_name]);
    ...
    async updateUser(id, userData) {
        try {
            ...
            const params = [userData.username, userData.email, userData.phone || null, userData.role, userData.full_name];
    ...
    async updateSessionId(userId, sessionId) {
        try {
            await pool_1.databasePool.execute('UPDATE users SET session_id = ?, updated_at = NOW() WHERE id = ?', [sessionId, userId]);
        }
        catch (error) {
