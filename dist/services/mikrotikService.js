...
exports.deleteQueueTreeByName = deleteQueueTreeByName;
...
exports.getInterfaceTrafficMonitoring = getInterfaceTrafficMonitoring;
exports.getQueueTreeStats = getQueueTreeStats;
exports.getIpAddresses = getIpAddresses;
exports.getSystemResources = getSystemResources;
exports.getInterfacesForMonitoring = getInterfacesForMonitoring;
...
async function testMikrotikConnection(cfg) {
    try {
        ...
        console.log('Queue names:', queues.map((q) => q.name));
        api.close();
...
async function getInterfaces(cfg) {
    ...
    try {
        ...
        if (rows.length === 0) {
            ...
            const names = rows.map((r) => r['name'] || r.name || 'unnamed').filter((n) => n !== 'unnamed');
            if (names.length > 0) {
        ...
        }).filter((ifc) => ifc.name && ifc.name.trim() !== ''); // Filter out interfaces without name
...
async function addIpAddress(cfg, params) {
    ...
    try {
        ...
        const interfaceExists = Array.isArray(interfaces) && interfaces.some((iface) => iface.name === params.interface);
        console.log(`Interface '${params.interface}' exists:`, interfaceExists);
        if (!interfaceExists) {
            throw new Error(`Interface '${params.interface}' tidak ditemukan di MikroTik. Interface yang tersedia: ${interfaces.map((i) => i.name).join(', ')}`);
        }
        ...
        const existingAddress = Array.isArray(existingAddresses) && existingAddresses.find((addr) => addr.address === params.address);
        if (existingAddress) {
        ...
        const addedAddress = Array.isArray(addresses) && addresses.find((addr) => addr.address === params.address && addr.interface === params.interface);
        ...
        if (addedAddress) {
            ...
            const similarAddresses = Array.isArray(addresses) && addresses.filter((addr) => addr.interface === params.interface);
            console.log('Addresses on same interface:', similarAddresses);
            // Cek apakah ada IP dengan format yang mirip
            const similarFormat = Array.isArray(addresses) && addresses.filter((addr) => addr.address && addr.address.includes('192.168.1'));
...
async function addMangleRulesForClient(cfg, params) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
...
async function createClientQueues(cfg, params) {
...
async function createQueueTree(cfg, data) {
    ...
    try {
        ...
        const existingQueue = existingQueues.find((q) => q.name === data.name);
        if (existingQueue) {
        ...
        if (data.packetMarks)
            queueData['packet-mark'] = data.packetMarks;
        if (data.limitAt)
            queueData['limit-at'] = data.limitAt;
        if (data.burstLimit)
            queueData['burst-limit'] = data.burstLimit;
        if (data.burstThreshold)
            queueData['burst-threshold'] = data.burstThreshold;
        if (data.burstTime)
            queueData['burst-time'] = data.burstTime;
        if (data.priority)
            queueData.priority = data.priority;
        if (data.disabled !== undefined)
            queueData.disabled = data.disabled;
        const params = [];
...
async function findQueueTreeIdByName(cfg, name) {
    ...
    try {
        await api.connect();
        const results = await api.write('/queue/tree/print', []);
        const found = (Array.isArray(results) ? results : []).find((q) => q.name === name);
...
async function deleteQueueTreeByName(cfg, name) {
...
async function removeIpAddress(cfg, address) {
    ...
    try {
        await api.connect();
        const list = await api.write('/ip/address/print');
        const row = (Array.isArray(list) ? list : []).find((r) => r.address === address);
...
// Mangle helpers
async function removeMangleRulesForClient(cfg, params) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
    ...
    try {
        await api.connect();
        const list = await api.write('/ip/firewall/mangle/print');
 ...
 */
async function findPppoeSecretIdByName(cfg, username) {
...
async function createPppoeSecret(cfg, data) {
    ...
    try {
        ...
        try {
            ...
            const verifyApi = new routeros_api_1.RouterOSAPI({
                host: cfg.host,
                port: port,
                user: cfg.username,
                password: cfg.password,
 ...
 */
async function getInterfaceTrafficMonitoring(cfg, interfaceName) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
 ...
 */
async function getQueueTreeStats(cfg) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
 ...
 */
async function getIpAddresses(cfg) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
 ...
 */
async function getInterfacesForMonitoring(cfg) {
    const api = new routeros_api_1.RouterOSAPI({
        host: cfg.host,
        port: cfg.port,
        user: cfg.username,
