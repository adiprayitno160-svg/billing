...
async function createStaticIpPackage(data) {
    ...
    try {
        ...
        await (0, mikrotikService_1.createQueueTree)(config, {
...
async function updateStaticIpPackage(id, data) {
    ...
    try {
        ...
        const queueTrees = await (0, mikrotikService_1.getQueueTrees)(config);
        ...
        if (uploadQueue) {
            await (0, mikrotikService_1.updateQueueTree)(config, uploadQueue['.id'], {
        ...
        if (downloadQueue) {
            await (0, mikrotikService_1.updateQueueTree)(config, downloadQueue['.id'], {
...
async function deleteStaticIpPackage(id) {
    ...
    try {
        ...
        if (uploadQueue) {
            ...
            await (0, mikrotikService_1.deleteQueueTree)(config, uploadQueue['.id']);
        }
        if (downloadQueue) {
            console.log('Deleting download queue:', downloadQueueName);
            await (0, mikrotikService_1.deleteQueueTree)(config, downloadQueue['.id']);
        }
