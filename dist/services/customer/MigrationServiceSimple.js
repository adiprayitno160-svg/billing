...
const MigrationLogger_1 = __importDefault(require("./MigrationLogger"));
class MigrationServiceSimple {
     ...
     */
    async getCustomerIP(customerId, customerName) {
        // Method 1: Active static_ip_clients
        let [rows] = await pool_1.default.query(`SELECT ip_address FROM static_ip_clients WHERE customer_id = ? AND status = 'active' LIMIT 1`, [customerId]);
        ...
        [rows] = await pool_1.default.query(`SELECT ip_address FROM static_ip_clients WHERE customer_id = ? ORDER BY created_at DESC LIMIT 1`, [customerId]);
        if (rows.length > 0 && rows[0] && rows[0].ip_address) {
     ...
     */
    calculateCustomerIP(cidrAddress) {
        try {
    ...
    async migrateToPrepaid(customerId, adminId) {
        ...
        try {
            const [customerCheck] = await pool_1.default.query('SELECT name FROM customers WHERE id = ?', [customerId]);
        ...
        migrationId = await MigrationLogger_1.default.startMigration('toPrepaid', customerId, customerName || 'Unknown', adminId);
        ...
        try {
            ...
            await MigrationLogger_1.default.dbOperation('Transaction started', customerId, customerName);
            // Step 4: Get customer data
            const [customerRows] = await connection.query('SELECT * FROM customers WHERE id = ?', [customerId]);
            if (customerRows.length === 0) {
                await connection.rollback();
                await MigrationLogger_1.default.error('Customer tidak ditemukan', undefined, { customerId, migrationId });
                return { success: false, message: 'Customer tidak ditemukan', error: 'Customer not found' };
            }
            const customer = customerRows[0];
            customerName = customer.name;
            await MigrationLogger_1.default.step('VALIDATION', `Customer found: ${customer.name}`, customerId, customerName, {
            ...
            if (customer.billing_mode === 'prepaid') {
                const [portalCheck] = await connection.query('SELECT portal_id FROM portal_customers WHERE customer_id = ?', [customerId]);
                if (portalCheck.length > 0) {
                    await connection.rollback();
                    await MigrationLogger_1.default.warn('Customer sudah prepaid dengan portal access', { customerId, customerName, portalId: portalCheck[0].portal_id, migrationId });
                    return {
                        success: false,
            ...
            if (customer.connection_type === 'pppoe' && !customer.pppoe_username) {
                await connection.rollback();
                await MigrationLogger_1.default.error('Customer PPPoE tidak memiliki username', undefined, { customerId, customerName, migrationId });
            ...
            if (portalRows.length > 0) {
                ...
                portalPin = undefined;
                await MigrationLogger_1.default.dbOperation('Portal access exists', customerId, customerName, { portalId, migrationId });
            }
            else {
           ...
           VALUES (?, ?, ?, 'active', NOW())`, [customerId, portalId, hashedPin]);
                await MigrationLogger_1.default.dbOperation('Portal access created', customerId, customerName, { portalId, migrationId });
            }
            // Step 8: Update customer billing mode
            await connection.query(`UPDATE customers SET billing_mode = 'prepaid', is_isolated = 1, updated_at = NOW() WHERE id = ?`, [customerId]);
            await MigrationLogger_1.default.dbOperation('Customer billing mode updated to prepaid', customerId, customerName, { migrationId });
            ...
            try {
                ...
                await LatePaymentTrackingService.resetCounter(customerId, adminId || 0, 'Auto-reset: Migrated to prepaid', 'Migration Service');
            }
            catch (error) {
            ...
            await connection.query(`UPDATE subscriptions SET status = 'cancelled' WHERE customer_id = ? AND status = 'active'`, [customerId]);
            await MigrationLogger_1.default.dbOperation('Postpaid subscriptions cancelled', customerId, customerName, { migrationId });
            // Step 10: Log migration
            await connection.query(`INSERT INTO migration_history (customer_id, from_mode, to_mode, migrated_by, portal_id, notes, created_at)
         VALUES (?, 'postpaid', 'prepaid', ?, ?, 'Migrasi ke prepaid', NOW())`, [customerId, adminId || null, portalId]);
            await MigrationLogger_1.default.dbOperation('Migration history logged', customerId, customerName, { portalId, migrationId });
            // Step 11: Commit transaction
            await connection.commit();
            await MigrationLogger_1.default.dbOperation('Transaction committed - Database migration SUCCESS', customerId, customerName, { migrationId });
            ...
            try {
                ...
                // Get active setting
                let [mikrotikSettings] = await pool_1.default.query(`SELECT id, host, port, username, password, is_active 
           ...
           LIMIT 1`);
                ...
                if (mikrotikSettings.length === 0 && allSettings.length > 0) {
                    ...
                    [mikrotikSettings] = await pool_1.default.query(`SELECT id, host, port, username, password, is_active 
             FROM mikrotik_settings 
             WHERE id = ?`, [allSettings[0].id]);
                }
                if (mikrotikSettings.length === 0) {
                    mikrotikResult = '⚠️ Mikrotik not configured';
                    await MigrationLogger_1.default.warn('Mikrotik not configured - no active settings found', { customerId, customerName, migrationId });
                }
                else {
                    ...
                    if (mikrotikHost === '192.168.5.1' || mikrotikHost.startsWith('192.168.5.') || mikrotikHost.endsWith('.1')) {
                        ...
                        if (correctIP && correctIP !== mikrotikHost && settings.id) {
                            try {
                                await pool_1.default.query('UPDATE mikrotik_settings SET host = ? WHERE id = ?', [correctIP, settings.id]);
                    ...
                    await MigrationLogger_1.default.info(`Using Mikrotik configuration for migration`, {
                        customerId,
                        customerName,
                    ...
                    if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
                        // Handle PPPoE
                        await MigrationLogger_1.default.mikrotikOperation('Starting PPPoE migration', customerId, customerName, { username: customer.pppoe_username, portalId, migrationId });
                        ...
                        try {
                            ...
                            await MigrationLogger_1.default.mikrotikOperation('PPPoE user disconnected', customerId, customerName, { username: customer.pppoe_username, migrationId });
                        }
                        catch (e) {
                            const error = e instanceof Error ? e : new Error(String(e));
                            await MigrationLogger_1.default.warn('PPPoE disconnect error (non-critical)', { customerId, customerName, username: customer.pppoe_username, migrationId, error: error.message });
                        }
                        ...
                        if (updateSuccess) {
                            ...
                            await MigrationLogger_1.default.mikrotikOperation('PPPoE profile updated to prepaid-no-package', customerId, customerName, { username: customer.pppoe_username, profile: 'prepaid-no-package', migrationId });
                        }
                        else {
                            mikrotikResult = '⚠️ Failed to update PPPoE profile';
                            await MigrationLogger_1.default.warn('Failed to update PPPoE profile', { customerId, customerName, username: customer.pppoe_username, migrationId });
                        }
                    }
                        ...
                        console.log(`[Migration] =====================================================\n`);
                        await MigrationLogger_1.default.mikrotikOperation('Starting Static IP migration', customerId, customerName, { migrationId, mikrotikHost: mikrotikHostFinal });
                        ...
                        if (!ipResult.ip) {
                            ...
                            await MigrationLogger_1.default.warn('No IP found for static IP customer', { customerId, customerName, ipSource: ipResult.source, migrationId });
                        }
                        else {
                            const customerIP = this.calculateCustomerIP(ipResult.ip);
                            await MigrationLogger_1.default.debug(`IP calculation: ${ipResult.ip} -> ${customerIP}`, { customerId, customerName, originalIP: ipResult.ip, calculatedIP: customerIP, source: ipResult.source, migrationId });
                            // Validate IP
                            const ipRegex = /^(\d{1,3}\.){3}\d{1,3}$/;
                            if (!ipRegex.test(customerIP)) {
                                mikrotikResult = `⚠️ Invalid IP format: ${customerIP}`;
                                await MigrationLogger_1.default.error(`Invalid IP format: ${customerIP}`, undefined, { customerId, customerName, ip: customerIP, migrationId });
                            }
                            else {
                                ...
                                if (lastOctet === 1 || lastOctet === 254 || customerIP === mikrotikHostFinal) {
                                    ...
                                    await MigrationLogger_1.default.warn(`IP is gateway/router IP: ${customerIP}`, { customerId, customerName, ip: customerIP, lastOctet, mikrotikHost: mikrotikHostFinal, migrationId });
                                }
                                else {
                                    ...
                                    try {
                                        ...
                                        await MigrationLogger_1.default.debug(`Cleaned up IP from prepaid-active`, { customerId, customerName, ip: customerIP, migrationId });
                                        // Add to prepaid-no-package
                                        const addSuccess = await addressListService.addToAddressList('prepaid-no-package', customerIP, `Prepaid - ${customer.name} - Portal: ${portalId}`);
                                        if (addSuccess) {
                                            await MigrationLogger_1.default.mikrotikOperation(`IP added to prepaid-no-package address list`, customerId, customerName, { ip: customerIP, addressList: 'prepaid-no-package', migrationId });
                                            // Verify
                                            ...
                                            if (verified) {
                                                ...
                                                await MigrationLogger_1.default.mikrotikOperation(`IP verified in address list - REDIRECT ACTIVE`, customerId, customerName, { ip: customerIP, addressList: 'prepaid-no-package', verified: true, migrationId });
                                            }
                                            else {
                                                mikrotikResult = `⚠️ IP ${customerIP} added but verification pending`;
                                                await MigrationLogger_1.default.warn(`IP added but verification failed`, { customerId, customerName, ip: customerIP, migrationId });
                                            }
                                        }
                                        else {
                                            mikrotikResult = `❌ Failed to add IP ${customerIP} to address list`;
                                            await MigrationLogger_1.default.error(`Failed to add IP to address list`, undefined, { customerId, customerName, ip: customerIP, addressList: 'prepaid-no-package', migrationId });
                                        }
                                    }
                                    catch (mikrotikError) {
                                        ...
                                        await MigrationLogger_1.default.error(`Mikrotik setup error`, error, { customerId, customerName, ip: customerIP, migrationId });
                                    }
                                }
                            }
                        }
                ...
                await MigrationLogger_1.default.error('Mikrotik setup error (non-critical)', error, { customerId, customerName, migrationId });
            }
            // End migration with success
            await MigrationLogger_1.default.endMigration(migrationId, 'toPrepaid', customerId, customerName, true, { message: mikrotikResult, portal_id: portalId });
            return {
                success: true,
            ...
            try {
                await connection.rollback();
                await MigrationLogger_1.default.dbOperation('Transaction rolled back due to error', customerId, customerName, { migrationId });
            }
            catch (rollbackError) {
                const rollbackErr = rollbackError instanceof Error ? rollbackError : new Error(String(rollbackError));
                await MigrationLogger_1.default.error('Rollback error', rollbackErr, { customerId, customerName, migrationId });
            }
            const errorObj = error instanceof Error ? error : new Error(String(error));
            const errorMessage = errorObj.message;
            await MigrationLogger_1.default.endMigration(migrationId, 'toPrepaid', customerId, customerName, false, { error: errorMessage });
            return {
                success: false,
    ...
    async migrateToPostpaid(customerId, adminId) {
        ...
        migrationId = await MigrationLogger_1.default.startMigration('toPostpaid', customerId, customerName || 'Unknown', adminId);
        // Step 1: Ensure table exists
        await this.ensureMigrationHistoryTable();
        // Step 2: Get connection
        const connection = await pool_1.default.getConnection();
        try {
            ...
            if (customer.billing_mode === 'postpaid') {
                await connection.rollback();
                await MigrationLogger_1.default.warn('Customer sudah postpaid', { customerId, customerName, migrationId });
                return {
                    success: false,
            ...
            await connection.query(`UPDATE subscriptions SET status = 'cancelled' WHERE customer_id = ? AND status = 'active'`, [customerId]);
            await MigrationLogger_1.default.dbOperation('Prepaid subscriptions cancelled', customerId, customerName, { migrationId });
            // Step 7: Update customer billing mode
            await connection.query(`UPDATE customers SET billing_mode = 'postpaid', is_isolated = 0, updated_at = NOW() WHERE id = ?`, [customerId]);
            await MigrationLogger_1.default.dbOperation('Customer billing mode updated to postpaid', customerId, customerName, { migrationId });
            ...
            await connection.query(`INSERT INTO migration_history (customer_id, from_mode, to_mode, migrated_by, portal_id, notes, created_at)
         VALUES (?, 'prepaid', 'postpaid', ?, ?, 'Migrasi ke postpaid', NOW())`, [customerId, adminId || null, portalId]);
            await MigrationLogger_1.default.dbOperation('Migration history logged', customerId, customerName, { portalId, migrationId });
           ...
           LIMIT 1`);
             ...
             WHERE id = ?`, [allSettings[0].id]);
                ...
                if (mikrotikSettings.length === 0) {
                    ...
                    if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
                        ...
                        await MigrationLogger_1.default.mikrotikOperation('Starting PPPoE migration to postpaid', customerId, customerName, { username: customer.pppoe_username, migrationId });
                        const mikrotikService = new MikrotikService_1.MikrotikService({
                            host: mikrotikHostFinal,
                            username: mikrotikUsernameFinal,
                            password: mikrotikPasswordFinal,
                        ...
                        const [profileRows] = await pool_1.default.query(`SELECT s.*, pp.profile_id, pprof.name as profile_name
               ...
               LIMIT 1`, [customerId]);
                        ...
                        if (profileRows.length > 0 && profileRows[0].profile_name) {
                            ...
                            const [activeRows] = await pool_1.default.query(`SELECT s.*, pp.profile_id, pprof.name as profile_name
                 FROM subscriptions s
                 LEFT JOIN pppoe_packages pp ON s.package_id = pp.id
                 LEFT JOIN pppoe_profiles pprof ON pp.profile_id = pprof.id
                 ...
                 LIMIT 1`, [customerId]);
                            if (activeRows.length > 0 && activeRows[0].profile_name) {
                                ...
                                const [pppoeProfileRows] = await pool_1.default.query(`SELECT name as profile_name FROM pppoe_profiles WHERE is_default = 1 LIMIT 1`);
                        ...
                        if (updateSuccess) {
                            ...
                            await MigrationLogger_1.default.mikrotikOperation(`PPPoE profile restored to ${profileName}`, customerId, customerName, { username: customer.pppoe_username, profile: profileName, migrationId });
                        }
                        else {
                            mikrotikResult = `⚠️ Failed to update PPPoE profile`;
                            await MigrationLogger_1.default.warn('Failed to update PPPoE profile', { customerId, customerName, username: customer.pppoe_username, migrationId });
                        ...
                        await MigrationLogger_1.default.mikrotikOperation('Starting Static IP cleanup for postpaid', customerId, customerName, { migrationId });
                        const ipResult = await this.getCustomerIP(customerId, customer.name);
                        if (!ipResult.ip) {
                            ...
                            try {
                                ...
                                await MigrationLogger_1.default.mikrotikOperation(`IP removed from prepaid address lists`, customerId, customerName, { ip: customerIP, migrationId });
                            }
                            catch (mikrotikError) {
                                const error = mikrotikError instanceof Error ? mikrotikError : new Error(String(mikrotikError));
                                const errorMsg = error.message;
                                mikrotikResult = `⚠️ Mikrotik error: ${errorMsg.substring(0, 100)}`;
                                await MigrationLogger_1.default.error('Mikrotik cleanup error', error, { customerId, customerName, ip: customerIP, migrationId });
                            }
                        }
                    }
                }
            ...
            // End migration with success
            await MigrationLogger_1.default.endMigration(migrationId, 'toPostpaid', customerId, customerName, true, { message: mikrotikResult });
            return {
                success: true,
                message: `Migrasi berhasil. ${mikrotikResult}`
            };
            ...
            const errorObj = error instanceof Error ? error : new Error(String(error));
            const errorMessage = errorObj.message;
            await MigrationLogger_1.default.endMigration(migrationId, 'toPostpaid', customerId, customerName, false, { error: errorMessage });
            return {
                success: false,
                message: 'Migrasi gagal',
                error: errorMessage
...
exports.default = new MigrationServiceSimple();
//# sourceMappingURL=MigrationServiceSimple.js.map