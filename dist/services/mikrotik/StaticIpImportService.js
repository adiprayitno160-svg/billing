...
exports.StaticIpImportService = void 0;
...
class StaticIpImportService {
     ...
     */
    isSameNetwork(customerIp, gatewayWithCidr) {
        if (!customerIp || !gatewayWithCidr)
            return false;
     ...
     */
    formatBandwidth(limit) {
        if (!limit || limit === '0')
            return '0';
        if (/[KMGkmg]/.test(limit))
            return limit;
        if (limit.includes('/')) {
    ...
    async scanCandidates() {
        ...
        try {
            ...
            const queues = await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/queue/tree/print', []);
            // 2. Ambil semua Mangle Rules
            const mangles = await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/ip/firewall/mangle/print', []);
            ...
            const packetMarkToIpMap = new Map();
            ...
            for (const queue of queues) {
                ...
                if (!packetMark || packetMark === 'no-mark')
                    continue;
                ...
                let customerIp = null;
                let mangleId = undefined;
                let gatewayIp = null;
                let gatewayIpId = null;
     ...
     */
    async renameQueue(queueId, newName, newLimit) {
        const config = await this.getPoolConfig();
        try {
            ...
            await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/queue/tree/set', params);
            return true;
        }
        catch (error) {
     ...
     */
    async tagMangle(mangleId, comment) {
        const config = await this.getPoolConfig();
        try {
            await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/ip/firewall/mangle/set', [
                `.id=${mangleId}`,
                `=comment=${comment}`
            ]);
            return true;
     ...
     */
    async disableIpAddress(ipAddressId) {
        const config = await this.getPoolConfig();
        try {
     ...
     */
    async enableIpAddress(ipAddressId) {
        const config = await this.getPoolConfig();
        try {
            await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/ip/address/set', [
                `.id=${ipAddressId}`,
     ...
     */
    async findIpAddressId(gatewayIp) {
        const config = await this.getPoolConfig();
        try {
            const ipAddresses = await MikroTikConnectionPool_1.mikrotikPool.execute(config, '/ip/address/print', []);
            if (!Array.isArray(ipAddresses))
...
exports.StaticIpImportService = StaticIpImportService;
//# sourceMappingURL=StaticIpImportService.js.map