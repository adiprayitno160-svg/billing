...
const WiFiManagementService_1 = require("../genieacs/WiFiManagementService");
class WhatsAppBotService {
     ...
     */
    static async validateCustomer(phone) {
        const customer = await this.getCustomerByPhone(phone);
        if (!customer) {
            await this.sendMessage(phone, '⛔ *AKSES DITOLAK*\n\n' +
     ...
     */
    static async handleMediaMessage(message, phone) {
        try {
            ...
            const verificationResult = await PaymentVerificationService_1.PaymentVerificationService.verifyPaymentProofAuto(media, customer.id);
            if (verificationResult.success) {
                await this.sendMessage(phone, '✅ *Pembayaran Berhasil Diverifikasi!*\n\n' +
                ...
                await this.sendMessage(phone, '❌ *Verifikasi Gagal*\n\n' +
            ...
            await this.sendMessage(phone, '❌ Terjadi kesalahan saat memproses bukti transfer. Silakan coba lagi atau hubungi customer service.');
        }
    }
    /**
     * Handle command
     */
    static async handleCommand(message, phone, command) {
        ...
        if (cmd === '/start' || cmd === '/menu' || cmd === '/help') {
            ...
            if (parts.length === 2 && parts[0] && parts[1]) {
                ...
                await this.sendMessage(phone, '❌ *Format salah!*\n\n' +
     ...
     * Handle menu command
     */
    static async handleMenuCommand(message, phone, command) {
        const cmd = command.toLowerCase().trim();
     ...
     * Show main menu
     */
    static async showMainMenu(phone) {
...
Kirim foto bukti transfer langsung untuk verifikasi otomatis!`;
     ...
     */
    static async showInvoices(phone) {
        try {
            // Validate customer access first
            const customer = await this.validateCustomer(phone);
            if (!customer)
            ...
            const [invoices] = await pool_1.databasePool.query(`SELECT 
            ...
            await this.sendMessage(phone, '❌ Gagal memuat tagihan.\n\n' +
                'Silakan coba lagi atau hubungi customer service.\n' +
                'Error: ' + (error.message || 'Unknown error'));
        }
    }
    static async showHelp(phone) {
        // Validate customer access first
        const customer = await this.validateCustomer(phone);
        if (!customer)
            return;
...
Hubungi customer service untuk bantuan lebih lanjut.`;
     ...
     * Show WiFi menu
     */
    static async showWiFiMenu(phone) {
        // Validate customer access first
        const customer = await this.validateCustomer(phone);
        if (!customer)
            return;
...
Ketik /menu untuk kembali ke menu utama.`;
    ...
    static async rebootOnt(phone) {
        ...
        if (result.success) {
            await this.sendMessage(phone, '✅ *Reboot Berhasil!*\n\n' +
     ...
     */
    static async changeWiFiSSID(phone, newSSID) {
        try {
            if (!newSSID || newSSID.length === 0) {
                await this.sendMessage(phone, '❌ *SSID tidak boleh kosong!*\n\n' +
            ...
            if (!deviceId) {
                await this.sendMessage(phone, '❌ *Device tidak ditemukan*\n\n' +
            ...
            if (result.success) {
                await this.sendMessage(phone, `✅ *SSID WiFi Berhasil Diubah!*\n\n` +
                    `SSID Baru: *${newSSID}*\n\n` +
                    `Perubahan akan diterapkan dalam beberapa saat.\n` +
                    `Silakan sambungkan ulang perangkat Anda dengan SSID baru.`);
            }
            else {
                await this.sendMessage(phone, `❌ *Gagal Mengubah SSID*\n\n` +
                    `Error: ${result.message}\n\n` +
                    `Silakan coba lagi atau hubungi customer service.`);
            }
        }
            ...
            await this.sendMessage(phone, '❌ Terjadi kesalahan saat mengubah SSID WiFi.\n' +
                'Silakan coba lagi atau hubungi customer service.');
        }
    }
    /**
    ...
    static async changeWiFiPassword(phone, newPassword) {
        try {
            if (!newPassword || newPassword.length < 8) {
                await this.sendMessage(phone, '❌ *Password tidak valid!*\n\n' +
            ...
            if (result.success) {
                await this.sendMessage(phone, `✅ *Password WiFi Berhasil Diubah!*\n\n` +
                ...
                await this.sendMessage(phone, `❌ *Gagal Mengubah Password*\n\n` +
                    `Error: ${result.message}\n\n` +
                    `Silakan coba lagi atau hubungi customer service.`);
            }
        }
            ...
            await this.sendMessage(phone, '❌ Terjadi kesalahan saat mengubah password WiFi.\n' +
                'Silakan coba lagi atau hubungi customer service.');
        }
    }
    /**
     ...
     */
    static async changeWiFiBoth(phone, newSSID, newPassword) {
        try {
            if (!newSSID || newSSID.length === 0) {
                await this.sendMessage(phone, '❌ *SSID tidak boleh kosong!*');
                return;
            ...
            if (result.success) {
                await this.sendMessage(phone, `✅ *WiFi Berhasil Diubah!*\n\n` +
                    `SSID Baru: *${newSSID}*\n` +
                    `Password Baru: *${newPassword}*\n\n` +
                ...
                await this.sendMessage(phone, `❌ *Gagal Mengubah WiFi*\n\n` +
                    `Error: ${result.message}\n\n` +
                    `Silakan coba lagi atau hubungi customer service.`);
            }
        }
            ...
            await this.sendMessage(phone, '❌ Terjadi kesalahan saat mengubah WiFi.\n' +
                'Silakan coba lagi atau hubungi customer service.');
        }
    }
    /**
    ...
    static async getCustomerByPhone(phone) {
        try {
            ...
            const [customersExact] = await pool_1.databasePool.query('SELECT * FROM customers WHERE phone = ? LIMIT 1', [normalizedPhone]);
            ...
            if (normalizedPhone.startsWith('62')) {
                ...
                const [customersZero] = await pool_1.databasePool.query('SELECT * FROM customers WHERE phone = ? OR phone = ? LIMIT 1', [phoneWithZero, normalizedPhone]);
                ...
                const [customers62] = await pool_1.databasePool.query('SELECT * FROM customers WHERE phone = ? OR phone = ? LIMIT 1', [phoneWith62, normalizedPhone]);
    ...
    static async sendMessage(phone, message) {
        try {
            await WhatsAppService_1.WhatsAppService.sendMessage(phone, message);
        }
        catch (error) {
...
WhatsAppBotService.COMMAND_PREFIX = '/';
