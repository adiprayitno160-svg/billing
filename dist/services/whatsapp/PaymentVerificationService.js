...
class PaymentVerificationService {
    ...
    static async verifyPaymentProofAuto(media, customerId) {
        try {
            ...
            const [customerRows] = await pool_1.databasePool.query('SELECT id, name, billing_mode, phone FROM customers WHERE id = ?', [customerId]);
    ...
    static async verifyPostpaidPayment(customerId, media, transferAmount, extractedData) {
        try {
            // 1. Get customer info
            const [customerRows] = await pool_1.databasePool.query('SELECT name, phone FROM customers WHERE id = ?', [customerId]);
            ...
            if (geminiEnabled) {
                try {
                    ...
                    const geminiResult = await GeminiService_1.GeminiService.analyzePaymentProof(imageBuffer, expectedAmount, customerName, 'invoice');
                    // RELAXED RULE: If names match (even partially) or amount is exact, OR it is a valid proof with high confidence
                    const isPaymentProof = geminiResult.isPaymentProof;
                    const amountMatches = geminiResult.amountMatches || geminiResult.validation?.metadata?.amountMatch === 'match';
                    ...
                    if (!geminiResult.isValid || geminiResult.confidence < 0.6) {
                        ...
                        const isTechnicalError = geminiResult.validation?.riskReasons?.some(r => r.includes('failed') || r.includes('API') || r.includes('FetchError'));
                        if (isTechnicalError) {
            ...
            const paymentId = await this.recordInvoicePayment(bestMatch.id, customerId, paymentAmount, 'transfer', `Verifikasi otomatis via WhatsApp - ${extractedData.bankAccount || 'Unknown'}`, extractedData.transferDate || new Date());
            // Trigger notification
            UnifiedNotificationService_1.UnifiedNotificationService.notifyPaymentReceived(paymentId).catch(err => console.error('[PaymentVerification] Failed to trigger notification:', err));
    ...
    static async verifyPrepaidPayment(customerId, media, transferAmount, extractedData) {
        try {
            ...
            const [paymentRequests] = await pool_1.databasePool.query(`SELECT * FROM payment_requests
                 WHERE customer_id = ?
                 AND status = 'pending'
                 AND expires_at > NOW()
                 AND unique_code = ?
                 ORDER BY created_at DESC
                 LIMIT 1`, [customerId, uniqueCode]);
            let paymentRequest = null;
            if (paymentRequests.length === 0) {
                // Try to find by amount match (without unique code)
                const [requestsByAmount] = await pool_1.databasePool.query(`SELECT * FROM payment_requests
                     WHERE customer_id = ?
                     AND status = 'pending'
                     AND expires_at > NOW()
                     AND ABS(total_amount - ?) <= 1000
                     ORDER BY created_at DESC
                     LIMIT 1`, [customerId, transferAmount]);
            ...
            if (geminiEnabled) {
                try {
                    ...
                    if (!geminiResult.isValid || geminiResult.confidence < 0.6) {
                        // RELAXED RULE: If it has the EXACT amount match (e.g. 150,225) we bypass strict AI risks 
                        // because guessing the unique 3-digit code is highly unlikely.
                        if (amountDiff === 0) {
                            console.log(`[PaymentVerification] 🟢 Relaxed Validation: Image failed AI strict check BUT exact unique amount (Rp ${transferAmount}) matches perfectly. Proceeding!`);
                            geminiResult.isValid = true; // Force valid
                        }
                        else if (geminiResult.validation?.riskLevel === 'high') {
                            return {
                                success: false,
            ...
            const processingResult = await PrepaidService.confirmPayment(paymentRequest.id, null, // verified_by (auto verification)
    ...
    static async recordInvoicePayment(invoiceId, customerId, amount, paymentMethod, notes, paymentDate) {
        ...
        try {
            ...
            const [result] = await connection.query(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, created_at)
            ...
            await connection.query(`UPDATE invoices 
                 SET paid_amount = ?, remaining_amount = ?, status = ?, 
                     last_payment_date = ?, 
                     updated_at = NOW()
                 WHERE id = ?`, [newPaid, newRemaining, newStatus, paymentDate, invoiceId]);
            ...
            if (newStatus === 'paid') {
                ...
                try {
                    ...
                    const [custRows] = await connection.query('SELECT pppoe_username, connection_type, ip_address FROM customers WHERE id = ?', [customerId]);
