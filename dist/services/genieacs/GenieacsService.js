...
class GenieacsService {
     ...
     */
    static async getConfigFromDb() {
        try {
            const [rows] = await pool_1.databasePool.query(`SELECT setting_key, setting_value FROM system_settings 
     ...
     */
    static async getInstanceFromDb() {
        if (!GenieacsService.instance) {
    ...
    async getDevices(limit = 100, skip = 0, projection = [], query = null) {
     ...
     */
    async getDevicesBySerial(serialNumber) {
    ...
    */
    async configureWanIP(deviceId, options) {
        try {
            const encodedId = encodeURIComponent(deviceId);
            const { wanDeviceIndex = 1, connectionDeviceIndex = 1, ipConnectionIndex = 1, connectionType, addressingType, ipAddress, subnetMask, gateway, dnsServers, vlanId, enable = true } = options;
            // Clear task queue
            try {
            ...
            if (addressingType === 'Static') {
                if (ipAddress)
                    parameterValues.push([`${basePath}.ExternalIPAddress`, ipAddress, 'xsd:string']);
                if (subnetMask)
                    parameterValues.push([`${basePath}.SubnetMask`, subnetMask, 'xsd:string']);
                if (gateway)
                    parameterValues.push([`${basePath}.DefaultGateway`, gateway, 'xsd:string']);
                if (dnsServers)
                    parameterValues.push([`${basePath}.DNSServers`, dnsServers, 'xsd:string']);
            }
    ...
    async getDeviceCount(query = null) {
        try {
            const params = {};
            if (query) {
                params.query = JSON.stringify(query);
     ...
     */
    async updatePPPoECredentials(deviceId, username, password) {
        try {
            const encodedId = encodeURIComponent(deviceId);
            // Bersihkan antrean
            try {
     ...
     */
    async configureWanPPP(deviceId, options) {
        try {
            const encodedId = encodeURIComponent(deviceId);
            const { wanDeviceIndex = 1, connectionDeviceIndex = 1, pppConnectionIndex = 1, username, password, vlanId, enable = true } = options;
            // Clear task queue
            try {
                const tasks = await this.getDeviceTasks(deviceId);
                for (const t of tasks) {
     ...
     */
    async addDeviceTag(deviceId, tagName) {
        try {
            const encodedId = encodeURIComponent(deviceId);
     ...
     */
    async removeDeviceTag(deviceId, tagName) {
        try {
            const encodedId = encodeURIComponent(deviceId);
            const encodedTag = encodeURIComponent(tagName);
