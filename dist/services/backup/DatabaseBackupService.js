...
class DatabaseBackupService {
    ...
    async getMysqldumpPath() {
        try {
            ...
            const [rows] = await pool_1.databasePool.query('SELECT setting_value FROM app_settings WHERE setting_key = ?', ['backup_mysqldump_path']);
            ...
            if (process.platform === 'win32') {
                ...
                if (fs_1.default.existsSync(laragonMysqlBase)) {
                    const dirs = fs_1.default.readdirSync(laragonMysqlBase);
                    const mysqlDirs = dirs.filter(d => d.startsWith('mysql-') && fs_1.default.statSync(path_1.default.join(laragonMysqlBase, d)).isDirectory());
                    if (mysqlDirs.length > 0) {
                        for (const dir of mysqlDirs) {
                            const candidate = path_1.default.join(laragonMysqlBase, dir, 'bin', 'mysqldump.exe');
                ...
                try {
                    const { stdout } = await new Promise((resolve, reject) => {
                        (0, child_process_1.exec)('which mysqldump', (err, stdout, stderr) => err ? reject(err) : resolve({ stdout, stderr }));
                    });
                    if (stdout) {
     ...
     * Save configuration
     */
    async saveConfiguration(mysqldumpPath) {
        await pool_1.databasePool.query('INSERT INTO app_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value = ?', ['backup_mysqldump_path', mysqldumpPath, mysqldumpPath]);
    }
    /**
    ...
    async dumpDatabase() {
        ...
        const tempCnfPath = path_1.default.join(this.backupDir, `.temp-dump-${Date.now()}.cnf`);
...
`;
        try {
            ...
            fs_1.default.writeFileSync(tempCnfPath, cnfContent, { mode: 0o600 });
        }
        catch (e) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const child = (0, child_process_1.spawn)(mysqldumpCmd, args);
            ...
            child.on('error', (err) => {
                if (fs_1.default.existsSync(tempCnfPath))
                    fs_1.default.unlinkSync(tempCnfPath);
            ...
            child.on('close', async (code) => {
                ...
                if (code === 0) {
                    ...
                    if (fs_1.default.existsSync(filePath))
                        fs_1.default.unlinkSync(filePath); // Delete partial file
    ...
    async detectMysqlBinary() {
        ...
        try {
            const { stdout } = await new Promise((resolve, reject) => {
                (0, child_process_1.exec)('which mysql', (err, stdout, stderr) => err ? reject(err) : resolve({ stdout, stderr }));
            });
            if (stdout) {
                const trimmed = stdout.trim();
    ...
    async restoreDatabase(filePath) {
        ...
        try {
            fs_1.default.chmodSync(filePath, 0o644);
        }
        catch (e) {
        ...
        const cnfPath = path_1.default.join(this.backupDir, `.temp-restore-${timestamp}.cnf`);
...
`;
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                const child = (0, child_process_1.spawn)(mysqlCmd, [
            ...
            if (fs_1.default.existsSync(cnfPath)) {
                fs_1.default.unlinkSync(cnfPath);
            }
        }
    }
    /**
    ...
    async fullSystemBackup() {
        ...
        const sqlFileName = path_1.default.basename(sqlPath);
        ...
        return new Promise((resolve, reject) => {
            ...
            const relativeDest = path_1.default.relative(process.cwd(), backupFilePath).split(path_1.default.sep).join('/');
            ...
            const processCmd = (0, child_process_1.spawn)('tar', [...tarArgs, '.'], {
                cwd: process.cwd(),
                shell: true
            });
     ...
     */
    async rotateLocalBackups(maxFiles = 5) {
        try {
            if (!fs_1.default.existsSync(this.backupDir))
                return;
            const files = fs_1.default.readdirSync(this.backupDir)
                ...
                time: fs_1.default.statSync(path_1.default.join(this.backupDir, f)).birthtime.getTime()
            }))
