...
exports.fixMissingColumns = fixMissingColumns;
...
async function getDatabaseStatus() {
    ...
    try {
        ...
        const version = Array.isArray(versionResult) ? versionResult[0].version : 'Unknown';
        // Get table count
        ...
        `);
        const totalTables = Array.isArray(tablesResult) ? tablesResult[0].count : 0;
        ...
        const [rowsResult] = await conn.execute(`
            ...
            WHERE table_schema = DATABASE()
        `);
        const totalRows = Array.isArray(rowsResult) ? rowsResult[0].total_rows || 0 : 0;
        return {
            connected: true,
            version,
            uptime: 'Active',
...
async function checkDatabaseSchema() {
    ...
    try {
        ...
        try {
            ...
            const aiColumns = columns.map(col => col.Field);
...
async function runMigration(migrationType) {
    const conn = await pool_1.databasePool.getConnection();
    try {
        await conn.beginTransaction();
