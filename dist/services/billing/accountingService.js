...
class AccountingService {
    ...
    static async createJournalEntry(entry, lines) {
        ...
        try {
            ...
            // IDEMPOTENCY CHECK: Prevent double accounting by checking reference
            if (entry.reference) {
                const [existing] = await connection.query('SELECT id FROM journal_entries WHERE reference = ?', [entry.reference]);
                if (existing.length > 0) {
                    console.log(`[AccountingService] Skipping duplicate entry for reference: ${entry.reference}`);
