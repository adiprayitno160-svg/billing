...
class IsolationService {
    ...
    static async isolateCustomer(isolationData) {
        ...
        try {
            ...
            if (isolationData.action === 'isolate') {
                await connection.execute('UPDATE customers SET is_isolated = TRUE WHERE id = ?', [isolationData.customer_id]);
            }
            else {
                await connection.execute('UPDATE customers SET is_isolated = FALSE WHERE id = ?', [isolationData.customer_id]);
            }
            await connection.commit();
            // Send notification to customer using template system
            try {
                const [customerRows] = await connection.query('SELECT name, phone, customer_code FROM customers WHERE id = ?', [isolationData.customer_id]);
                if (customerRows.length > 0 && customerRows[0].phone) {
                    ...
                    if (isolationData.action === 'isolate') {
                        ...
                        const [invoiceRows] = await connection.query(`SELECT invoice_number, total_amount, due_date, period 
                             ...
                             ORDER BY due_date DESC LIMIT 1`, [isolationData.customer_id]);
    ...
    static async autoIsolatePreviousMonthUnpaid() {
        ...
        console.log(`[Auto Isolation] Found ${result.length} customers with unpaid invoices`);
        for (const customer of result) {
            try {
