    ...
    static async isolateCustomer(isolationData, existingConnection) {
        ...
        try {
            ...
            if (isolationData.action === 'restore') {
                const [unpaidCheck] = await connection.query("SELECT period FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled') AND (remaining_amount > 0 OR status != 'paid') ORDER BY period ASC", [isolationData.customer_id]);
