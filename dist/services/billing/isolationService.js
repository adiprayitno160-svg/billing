...
class IsolationService {
     ...
     */
    static async ensureIsolationTemplateExists() {
        try {
            const template = await NotificationTemplateService_1.NotificationTemplateService.getTemplate('service_blocked_system', 'whatsapp');
            if (!template) {
                console.log('[Isolation] Creating default system isolation template...');
                await NotificationTemplateService_1.NotificationTemplateService.createTemplate(ISOLATION_SYSTEM_TEMPLATE);
            }
        }
        catch (e) {
     ...
     */
    static async executeMikrotikIsolation(customer, action, existingConnection) {
        ...
        try {
            ...
            if (connectionType === 'pppoe') {
                ...
                if (Array.isArray(secrets) && secrets.length > 0) {
                    ...
                    targetSecret = allSecrets.find((s) => s.name?.toLowerCase() === pppoeUsername.toLowerCase() ||
                ...
                // ============== STATIC IP ISOLATION ==============
                const [staticIpRows] = await connection.execute(`SELECT ip_address, gateway_ip, gateway_ip_id, interface FROM static_ip_clients WHERE customer_id = ? ORDER BY id DESC LIMIT 1`, [customer.id]);
                ...
                if (!gwId) {
                    ...
                    try {
                        ...
                        if (Array.isArray(ips)) {
                            const found = ips.find((ip) => (targetGwIp && (ip.address === targetGwIp || ip.address.split('/')[0] === targetGwIp)) ||
                ...
                if (gwId) {
                    try {
                        ...
                        if (!staticIpClient.gateway_ip_id) {
                            await connection.execute('UPDATE static_ip_clients SET gateway_ip_id = ? WHERE customer_id = ?', [gwId, customer.id]);
                        }
                    }
                    catch (e) {
     ...
     */
    static async isolateCustomer(isolationData, existingConnection) {
        ...
        try {
            ...
            if (isolationData.action === 'restore') {
                const [unpaidCheck] = await connection.query(`SELECT period FROM invoices 
                     WHERE customer_id = ? 
                     AND status NOT IN ('paid', 'cancelled', 'hutang') 
                     AND remaining_amount > 0 
                     AND (status = 'overdue' OR status = 'janji_bayar' OR ((status = 'sent' OR status = 'partial') AND due_date <= CURDATE()))
                     ORDER BY period ASC`, [isolationData.customer_id]);
                const hasUnpaidInvoices = unpaidCheck.length > 0;
                const periods = unpaidCheck.map(inv => invoiceService_1.InvoiceService.getMonthName(inv.period)).join(', ');
                if (hasUnpaidInvoices && (isolationData.performed_by === 'system' || isolationData.performed_by === '0')) {
                    ...
                    await UnifiedNotificationService_1.UnifiedNotificationService.broadcastToAdmins(adminMsg).catch(e => console.error('Failed to notify admin about remaining debt:', e));
                    ...
                    await UnifiedNotificationService_1.UnifiedNotificationService.broadcastToAdmins(adminMsg).catch(e => console.error('Failed to notify admin about override:', e));
            ...
            if (customer.connection_type === 'pppoe') {
                ...
                mikrotikUsername = staticRows[0]?.ip_address || '';
            }
            ...
            if (isolationData.action === 'isolate') {
                ...
                if (customer.is_isolated || customer.status === 'inactive') {
                    ...
                    console.log(`[Isolation] ✅ Customer ${customer.name} status set to ACTIVE`);
                }
                else {
                    await connection.execute('UPDATE customers SET is_isolated = FALSE, isolated_at = NULL WHERE id = ?', [customer.id]);
                    console.log(`[Isolation] ✅ Customer ${customer.name} is_isolated reset, status remains ${customer.status}`);
                }
            }
            if (isNewConnection)
            ...
            if (customer.phone && !isolationData.skipNotification) {
                try {
                    ...
                    let notifyType = isolationData.action === 'isolate' ? 'service_blocked' : 'service_unblocked';
                    ...
                    const checkConn = existingConnection || pool_1.databasePool;
                    const [existingNotif] = await checkConn.query(`SELECT 1 FROM unified_notifications_queue 
                         WHERE customer_id = ? AND notification_type = ? 
                         AND DATE(created_at) = CURDATE() LIMIT 1`, [customer.id, notifyType]);
    ...
    static async sendIsolationWarnings(daysBefore = 3) {
        ...
        try {
            ...
            `;
            const [customers] = await connection.query(query, [warningDateStr]);
                ...
                try {
                    await UnifiedNotificationService_1.UnifiedNotificationService.queueNotification({
                        ...
                        variables: {
                            ...
                            amount: NotificationTemplateService_1.NotificationTemplateService.formatCurrency(parseFloat(customer.remaining_amount)),
                            ...
                            due_date: NotificationTemplateService_1.NotificationTemplateService.formatDate(new Date(customer.due_date)),
            ...
            for (const customer of customers) {
                try {
                    await UnifiedNotificationService_1.UnifiedNotificationService.queueNotification({
                        customer_id: customer.id,
                        invoice_id: customer.invoice_id,
                        notification_type: 'isolation_h1_warning',
                        channels: ['whatsapp'],
                        variables: {
                            customer_name: customer.name,
                            customer_code: customer.customer_code,
    ...
    static async autoIsolateOverdueCustomers() {
        ...
        for (const customer of customers) {
            try {
                const periods = customer.periods ? customer.periods.split(',').map((p) => {
        ...
        if (isolated > 0) {
            try {
                await UnifiedNotificationService_1.UnifiedNotificationService.broadcastToAdmins(`🚨 *LAPORAN ISOLIR OTOMATIS*\n\n` +
     ...
     */
    static async isolateIfOverdue(customerId) {
        ...
        const query = `
            ...
            LIMIT 1
        `;
        const [invoices] = await pool_1.databasePool.query(query, [customerId]);
    ...
    static async autoIsolatePreviousMonthUnpaid() {
        ...
        `;
        const [customers] = await pool_1.databasePool.execute(query, [prevPeriod]);
        let isolated = 0;
        let failed = 0;
        for (const customer of customers) {
            try {
     ...
     */
    static async autoIsolateDeferredExpired() {
        const query = `
        ...
        for (const customer of customers) {
            try {
                ...
                await pool_1.databasePool.execute(`UPDATE payment_deferments SET status = 'completed' WHERE customer_id = ? AND status IN ('pending', 'approved') AND deferred_until_date < CURDATE()`, [customer.id]);
                // Reset is_deferred flag
                await pool_1.databasePool.execute('UPDATE customers SET is_deferred = FALSE WHERE id = ?', [customer.id]);
                const success = await this.isolateCustomer({
                    customer_id: customer.id,
                    action: 'isolate',
    ...
    static async restoreIfQualified(customerId, existingConnection) {
        ...
        try {
            ...
            const [unpaidResult] = await connection.query(`SELECT id, invoice_number, status, remaining_amount, due_date 
                 ...
                 AND remaining_amount > 0 
                 AND (status = 'overdue' OR status = 'janji_bayar' OR status = 'hutang' OR ((status = 'sent' OR status = 'partial') AND due_date <= CURDATE()))`, [customerId]);
            if (unpaidResult.length === 0) {
                ...
                console.log(`[Isolation] ⏳ Customer ${customer.name} (#${customerId}) still has ${unpaidResult.length} unpaid invoice(s): ${unpaidResult.map((i) => i.invoice_number).join(', ')}`);
            }
            return false;
        }
        catch (error) {
     ...
     */
    static async bulkRestoreAllSilent(performedBy = 'admin') {
        const [customers] = await pool_1.databasePool.execute('SELECT id, name FROM customers WHERE is_isolated = TRUE');
        let restored = 0;
        let failed = 0;
    ...
    static async bulkIsolateByOdc(odcId, reason) {
        const [customers] = await pool_1.databasePool.execute('SELECT id FROM customers WHERE odc_id = ? AND is_isolated = FALSE AND status = "active"', [odcId]);
        let isolated = 0;
        let failed = 0;
        for (const customer of customers) {
            try {
     ...
     */
    static async removeIsolationDeferment(customerId, performedBy = 'admin') {
        const connection = await pool_1.databasePool.getConnection();
        try {
            await connection.beginTransaction();
            // 1. Update active deferments to cancelled
            await connection.execute(`UPDATE payment_deferments 
                 SET status = 'cancelled', updated_at = NOW() 
                 WHERE customer_id = ? AND status IN ('pending', 'approved')`, [customerId]);
            // 2. Clear flag in customers table
            await connection.execute('UPDATE customers SET is_deferred = FALSE WHERE id = ?', [customerId]);
            // 3. Log the action
            await connection.execute(`INSERT INTO customer_logs (customer_id, action, description, created_by) 
                 VALUES (?, 'deferment_cancelled', 'Penagguhan (Whitelist) dibatalkan secara manual.', ?)`, [customerId, performedBy]);
            await connection.commit();
            return true;
        }
        catch (error) {
     ...
     */
    static async startupCatchUpIsolation() {
     ...
     */
    static async massIsolateSpecificPeriod(period) {
        const query = `
            ...
            AND c.status = 'active'
        `;
        const [results] = await pool_1.databasePool.execute(query, [period]);
        let isolated = 0;
        let failed = 0;
        console.log(`[Isolation] 🚨 MASS ISOLATION START for period ${period}. Targets: ${results.length}`);
        for (const row of results) {
            try {
                const monthName = invoiceService_1.InvoiceService.getMonthName(row.period);
                const success = await this.isolateCustomer({
        ...
        if (isolated > 0 || failed > 0) {
            try {
                await UnifiedNotificationService_1.UnifiedNotificationService.broadcastToAdmins(`🚨 *LAPORAN ISOLASI MASSAL*\n\n` +
     ...
     */
    static async autoExpireUnconfirmedPayments() {
        let expired = 0;
        let isolated = 0;
        let failed = 0;
        try {
            // Find pending confirmations older than 1 day
            const [pendingConfirmations] = await pool_1.databasePool.query(`
            ...
            for (const conf of pendingConfirmations) {
                try {
                    ...
                    await pool_1.databasePool.query(`UPDATE payment_confirmations SET status = 'expired', updated_at = NOW() WHERE id = ?`, [conf.id]);
                    // 2. Set isolation_enabled = 1 for this customer (so auto-isolate cron picks them up)
                    await pool_1.databasePool.query(`UPDATE customers SET isolation_enabled = 1, updated_at = NOW() WHERE id = ? AND isolation_enabled = 0`, [conf.customer_id]);
                    expired++;
                    ...
                    try {
                        ...
                        if (waService && conf.phone) {
                            ...
                            await waService.sendMessage(phone + '@s.whatsapp.net', `⚠️ *PERHATIAN — ${conf.customer_name}*\n\n` +
            ...
            if (expired > 0) {
                try {
                    const customerNames = pendingConfirmations.map((c) => c.customer_name).join(', ');
                    await UnifiedNotificationService_1.UnifiedNotificationService.broadcastToAdmins(`⏰ *KONFIRMASI KADALUARSA*\n\n` +
...
//# sourceMappingURL=isolationService.js.map