    ...
    static async isolateCustomer(isolationData, existingConnection) {
        ...
        try {
            ...
            if (isolationData.action === 'restore') {
                const [unpaidCheck] = await connection.query("SELECT period FROM invoices WHERE customer_id = ? AND status NOT IN ('paid', 'cancelled', 'hutang') AND (remaining_amount > 0 OR status != 'paid') ORDER BY period ASC", [isolationData.customer_id]);
    ...
    static async restoreIfQualified(customerId, existingConnection) {
        ...
        try {
            ...
            const [unpaidResult] = await connection.query(`SELECT id, invoice_number, status, remaining_amount, due_date 
                 ...
                 AND (status = 'overdue' OR status = 'partial' OR status = 'janji_bayar' OR status = 'hutang' OR (status = 'sent' AND due_date <= CURDATE()))`, [customerId]);
