...
const ArrearsAnalysisPrompts_1 = require("../ai/ArrearsAnalysisPrompts");
class AutoMigrationService {
    ...
    static async processCustomer(customer, connection) {
        ...
        if (count >= 2) {
            ...
            const prompt = ArrearsAnalysisPrompts_1.ArrearsAnalysisPrompts.getMigrationActionPrompt(customer, arreasStats);
        ...
        // --- SKENARIO 2: PERINGATAN (1x) ---
            ...
            if (logs.length === 0) {
                ...
                const prompt = ArrearsAnalysisPrompts_1.ArrearsAnalysisPrompts.getEarlyWarningPrompt(customer, arreasStats, []);
                const result = await this.generateAIResponse(prompt);
                if (result && result.whatsappMessage) {
                    // 2. Kirim WA
                    if (customer.phone) {
                        await WhatsAppService_1.whatsappService.sendMessage(customer.phone, result.whatsappMessage);
                        // 3. Log Notifikasi
    ...
    static async executeMigration(connection, customer, aiResult) {
        ...
        try {
            ...
            if (customer.phone && aiResult.whatsappMessage) {
                await WhatsAppService_1.whatsappService.sendMessage(customer.phone, aiResult.whatsappMessage);
                // Log Notifikasi
                await connection.query(`
                    INSERT INTO customer_notifications_log 
                    (customer_id, channel, notification_type, message, status, recipient)
    ...
    static async generateAIResponse(promptText) {
        try {
...
AutoMigrationService.genAI = null;
AutoMigrationService.model = null;
