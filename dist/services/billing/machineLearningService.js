...
class MachineLearningService {
     ...
     */
    static async predictPaymentProbability(customerId) {
        try {
            ...
            const [credit] = await pool_1.databasePool.query("SELECT score FROM customer_credit_scores WHERE customer_id = ?", [customerId]);
            ...
            if (totalInvoices > 0) {
                ...
                if (onTimeRate < 0.5)
                    reasons.push('Riwayat pembayaran sering terlambat.');
            }
            else {
            ...
            baseProb += scoreIncr;
            if (score > 800)
                reasons.push('Skor kredit sangat baik.');
            if (score < 500)
