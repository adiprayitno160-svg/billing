...
class LatePaymentTrackingService {
    ...
    static async trackPayment(invoiceId, paymentId, paymentDate, dueDate) {
        ...
        try {
            ...
            const [invoiceRows] = await connection.query(`SELECT i.*, c.id as customer_id, c.name as customer_name, c.phone
         FROM invoices i
      ...
      `);
            if (tableCheck.length > 0) {
                ...
                await connection.query(`INSERT INTO customer_late_payment_tracking 
     ...
     */
    static async calculateLatePaymentCount(customerId, months) {
        const connection = await pool_1.default.getConnection();
        try {
      ...
      `);
        ...
        `);
                if (tableCheck.length > 0) {
                    ...
                    const [countRows] = await connection.query(`SELECT COUNT(*) as count
             FROM customer_late_payment_tracking
             WHERE customer_id = ? 
               AND is_late = TRUE
               AND payment_date >= ?`, [customerId, cutoffDate]);
            ...
            await connection.query(`UPDATE customers SET late_payment_count = ? WHERE id = ?`, [count, customerId]);
            return count;
        }
        catch (error) {
     ...
     */
    static async checkAndResetCounter(customerId) {
        const connection = await pool_1.default.getConnection();
        try {
            // Get reset threshold from settings
            const [resetRows] = await connection.query(`SELECT setting_value FROM system_settings WHERE setting_key = 'consecutive_on_time_reset'`);
      ...
      `);
            ...
            // Get customer
            const [customerRows] = await connection.query(`SELECT COALESCE(consecutive_on_time_payments, 0) as consecutive_on_time_payments, 
         ...
         FROM customers WHERE id = ?`, [customerId]);
            ...
            if (consecutiveOnTime >= resetThreshold && currentCount > 0) {
                await this.resetCounter(customerId, 0, `Auto-reset: ${consecutiveOnTime} consecutive on-time payments`, 0);
                // Reset consecutive counter
                await connection.query(`UPDATE customers SET consecutive_on_time_payments = 0 WHERE id = ?`, [customerId]);
     ...
     */
    static async getLatePaymentHistory(customerId, limit = 50) {
        try {
     ...
     */
    static async sendLatePaymentWarning(customerId, count) {
        try {
            // Check if warning is enabled
            const settingKey = count === 3 ? 'late_payment_warning_at_3' : 'late_payment_warning_at_4';
            const [enabledRows] = await pool_1.default.query(`SELECT setting_value FROM system_settings WHERE setting_key = ?`, [settingKey]);
            ...
            const customer = customerRows[0];
            // Get threshold
            const [thresholdRows] = await pool_1.default.query(`SELECT setting_value FROM system_settings WHERE setting_key = 'late_payment_threshold'`);
...
Terima kasih.`;
     ...
     */
    static async resetCounter(customerId, adminId = 0, reason = 'Manual reset', adminName) {
        const connection = await pool_1.default.getConnection();
        try {
            await connection.beginTransaction();
      ...
      `);
            ...
            // Get current count
            const [customerRows] = await connection.query(`SELECT COALESCE(late_payment_count, 0) as late_payment_count FROM customers WHERE id = ?`, [customerId]);
            if (customerRows.length === 0) {
      ...
      `);
            const hasLateCount = allColumns.some((col) => col.COLUMN_NAME === 'late_payment_count');
            const hasConsecutive = allColumns.some((col) => col.COLUMN_NAME === 'consecutive_on_time_payments');
            const hasLastLateDate = allColumns.some((col) => col.COLUMN_NAME === 'last_late_payment_date');
            const updateParts = [];
            if (hasLateCount)
                updateParts.push('late_payment_count = 0');
            if (hasConsecutive)
                updateParts.push('consecutive_on_time_payments = 0');
            if (hasLastLateDate)
                updateParts.push('last_late_payment_date = NULL');
           ...
           WHERE id = ?`, [customerId]);
            ...
            // Log audit
            await this.logAudit(customerId, 'reset', oldCount, 0, reason, adminId, adminName || 'System');
            await connection.commit();
     ...
     */
    static async adjustCounter(customerId, adjustment, adminId, reason, adminName) {
        const connection = await pool_1.default.getConnection();
        try {
            await connection.beginTransaction();
            // Check if column exists
      ...
      `);
            ...
            // Update count
            await connection.query(`UPDATE customers SET late_payment_count = ? WHERE id = ?`, [newCount, customerId]);
            // Log audit
            await this.logAudit(customerId, 'manual_adjust', oldCount, newCount, reason, adminId, adminName);
            await connection.commit();
    ...
    static async getCustomerLatePaymentStats(customerId) {
      ...
      `);
            const hasLatePaymentColumns = columns.some((col) => col.COLUMN_NAME === 'late_payment_count');
            const hasLastLatePaymentDate = columns.some((col) => col.COLUMN_NAME === 'last_late_payment_date');
            const hasConsecutiveOnTime = columns.some((col) => col.COLUMN_NAME === 'consecutive_on_time_payments');
            const [rows] = await pool_1.default.query(`SELECT 
        ...
        `);
                if (tableCheck.length > 0) {
                    const [periodRows] = await pool_1.default.query(`SELECT setting_value FROM system_settings WHERE setting_key = 'late_payment_rolling_months'`);
                    const months = periodRows.length > 0 ? parseInt(periodRows[0].setting_value) : 12;
                    const cutoffDate = new Date();
                    cutoffDate.setMonth(cutoffDate.getMonth() - months);
                    const [totalRows] = await pool_1.default.query(`SELECT COUNT(*) as total
             FROM customer_late_payment_tracking
     ...
     */
    static async batchResetCounter(customerIds, adminId, reason, adminName) {
        let successCount = 0;
     ...
     * Log audit trail
     */
    static async logAudit(customerId, action, oldCount, newCount, reason, performedBy = 0, performedByName = 'System') {
        try {
            await pool_1.default.query(`INSERT INTO late_payment_audit_log 
         (customer_id, action, old_count, new_count, reason, performed_by, performed_by_name)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [customerId, action, oldCount, newCount, reason, performedBy || null, performedByName]);
        }
        catch (error) {
    ...
    static async dailyRecalculation() {
        ...
        try {
            ...
            const [customers] = await pool_1.default.query(`SELECT id FROM customers`);
