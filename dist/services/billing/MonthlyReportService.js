...
class MonthlyReportService {
     ...
     */
    static async generateAndSendMonthlyReport() {
        try {
            console.log('[MonthlyReport] Starting monthly report generation...');
            // 1. Get Head of Finance phone number
            const [settings] = await pool_1.default.query("SELECT setting_value FROM system_settings WHERE setting_key = 'head_finance_phone' LIMIT 1");
            ...
            const [paidInvoices] = await pool_1.default.query(`
                SELECT 
                    p.payment_date,
            ...
            const [unpaidInvoices] = await pool_1.default.query(`
                SELECT 
                    i.invoice_number,
                    i.period,
                    i.total_amount,
            ...
            // 6. Send via WhatsApp
            const waService = WhatsAppService_1.WhatsAppService.getInstance();
    ...
    static async generatePDF(filePath, monthName, paid, unpaid) {
...
//# sourceMappingURL=MonthlyReportService.js.map