...
exports.InvoiceDetailService = void 0;
const pool_1 = require("../../db/pool");
class InvoiceDetailService {
    /**
     * Get invoice detail dengan informasi lengkap
     */
    static async getInvoiceDetail(invoiceId) {
        try {
            const query = `
                SELECT 
                    i.*,
                ...
                WHERE i.id = ?
            `;
            const [rows] = await pool_1.databasePool.execute(query, [invoiceId]);
     ...
     */
    static async getDebtTracking(customerId) {
        try {
            const query = `
                SELECT 
                    dt.*,
            ...
            `;
            const [rows] = await pool_1.databasePool.execute(query, [customerId]);
            return rows;
        }
        catch (error) {
     ...
     */
    static async createPaymentSession(invoiceId, paymentAmount, paymentMethod) {
     ...
     */
    static async processPaymentFromDetail(sessionToken, paymentData) {
        const connection = await pool_1.databasePool.getConnection();
        try {
            await connection.beginTransaction();
            // Get session data
            const [sessionRows] = await connection.execute('SELECT * FROM invoice_payment_sessions WHERE session_token = ? AND status = "pending" AND expires_at > NOW()', [sessionToken]);
            ...
            await connection.execute('UPDATE invoice_payment_sessions SET status = "completed" WHERE session_token = ?', [sessionToken]);
    ...
    static async updateInvoicePaymentStatus(invoiceId) {
        ...
        try {
            ...
            // Get invoice details
            const [invoiceRows] = await connection.execute('SELECT total_amount, due_date, status FROM invoices WHERE id = ?', [invoiceId]);
            ...
            await connection.execute(`UPDATE invoices 
                 ...
                 WHERE id = ?`, [totalPaid, remainingAmount, newStatus, invoiceId]);
        }
        finally {
            connection.release();
        }
     ...
     */
    static async createDebtTracking(debtData) {
        const connection = await pool_1.databasePool.getConnection();
        try {
            const query = `
     ...
     */
    static async getPaymentSession(sessionToken) {
        try {
            const query = `
                SELECT 
                    ips.*,
            ...
            `;
            const [rows] = await pool_1.databasePool.execute(query, [sessionToken]);
            return rows[0] || null;
        }
        catch (error) {
    ...
    static async cleanExpiredSessions() {
        try {
            await pool_1.databasePool.execute('UPDATE invoice_payment_sessions SET status = "expired" WHERE expires_at < NOW() AND status = "pending"');
        }
        catch (error) {
...
exports.InvoiceDetailService = InvoiceDetailService;
