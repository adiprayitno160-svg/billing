    ...
    static async generateMonthlyInvoices(period, customerIds, forceAll = false) {
        ...
        try {
            ...
            for (const subscription of subscriptions) {
                if (processedCustomerIds.has(subscription.customer_id))
                    continue;
                try {
                    const [existing] = await connection.query('SELECT id FROM invoices WHERE customer_id = ? AND period = ?', [subscription.customer_id, period]);
                    ...
                    if (carryOverAmount > 0 && oldInvoiceIds.length > 0) {
                        ...
                        try {
                            ...
                            for (const inv of existingNotesResult) {
                                ...
                                await connection.query(`UPDATE invoices SET status = 'carried_over', remaining_amount = 0, notes = ? WHERE id = ?`, [newNote, inv.id]);
                            }
            ...
            for (const customer of customerResult) {
                ...
                try {
                    ...
                    const [existing] = await connection.query('SELECT id FROM invoices WHERE customer_id = ? AND period = ?', [customer.customer_id, period]);
                    if (existing.length > 0) {
                    ...
                    if (subtotal <= 0) {
                        try {
                            const [lastSub] = await connection.query(`SELECT package_name, price FROM subscriptions 
                                 WHERE customer_id = ? 
                                 ORDER BY updated_at DESC LIMIT 1`, [customer.customer_id]);
