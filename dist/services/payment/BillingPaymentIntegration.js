...
class BillingPaymentIntegration {
    ...
    async processPaymentSuccess(transactionId) {
        ...
        try {
            ...
            const [transactionRows] = await connection.execute(`SELECT pt.*, i.id as invoice_id, i.customer_id, i.subscription_id, i.total_amount, i.paid_amount, i.remaining_amount, i.invoice_number,
         ...
         WHERE pt.external_id = ?`, [transactionId]);
            ...
            await connection.execute('UPDATE invoices SET status = ?, paid_amount = ?, remaining_amount = ?, paid_at = NOW(), updated_at = NOW() WHERE id = ?', [newStatus, newPaid, newRemaining, transaction.invoice_id]);
        ...
        `, [transaction.customer_id, excessAmount, `Kelebihan pembayaran gateway (${transaction.gateway_code}) invoice ${transaction.invoice_number}`, transaction.invoice_id.toString()]);
            ...
            const [paymentResult] = await connection.execute(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, status, created_at)
         ...
         VALUES (?, ?, ?, 'payment_success', ?, 'completed', NOW())`, [transaction.invoice_id, transaction.customer_id, transactionId, transaction.amount]);
            ...
            if (paymentId) {
                try {
                    ...
                    UnifiedNotificationService.notifyPaymentReceived(paymentId).catch(err => console.error('[BillingPaymentIntegration] Notification error:', err));
                }
                catch (e) {
