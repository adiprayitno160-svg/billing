         ...
         VALUES (?, ?, ?, ?)`, [
           ...
           WHERE external_id = ?`, [
                ...
                if (webhookData.status === 'completed' || webhookData.status === 'paid') {
                    const [transaction] = await connection.execute(`SELECT pt.invoice_id, pt.amount, p.id as payment_id, i.due_date, i.total_amount
                 ...
                 WHERE pt.external_id = ?`, [webhookData.transactionId]);
                    if (transaction && transaction.length > 0) {
                        ...
                        if (!paymentId) {
                            ...
                            const [payResult] = await connection.execute(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, gateway_transaction_id, created_at)
                   VALUES (?, ?, ?, NOW(), ?, ?, NOW())`, [
                                invoiceId,
                                gatewayCode,
                                transData.amount,
