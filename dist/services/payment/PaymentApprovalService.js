...
class PaymentApprovalService {
     ...
     */
    static async autoApprovePayment(verificationId, invoiceId, amount, customerId, userId = null) {
        const connection = await pool_1.databasePool.getConnection();
        try {
            await connection.beginTransaction();
            // Get customer billing mode
            const [customerRows] = await connection.execute('SELECT billing_mode, name FROM customers WHERE id = ?', [customerId]);
            ...
            const [invoiceRows] = await connection.execute('SELECT remaining_amount, invoice_number FROM invoices WHERE id = ?', [invoiceId]);
            ...
            `;
            const [paymentResult] = await connection.execute(paymentQuery, [invoiceId, amount, `WA-${Date.now()}`, `Auto-approved via AI OCR${excessAmount > 0 ? ` (Kelebihan Rp ${excessAmount})` : ''}`]);
            ...
            if (paymentId) {
                try {
                    ...
                    UnifiedNotificationService.notifyPaymentReceived(paymentId).catch(e => console.error('Background notification error in auto-approve:', e));
                }
                catch (e) {
