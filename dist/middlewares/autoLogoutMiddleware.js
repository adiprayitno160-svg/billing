...
async function autoLogoutMiddleware(req, res, next) {
    try {
        ...
        if (autoLogoutEnabled && userId) {
            ...
            const [timeoutRows] = await pool_1.default.query("SELECT setting_value FROM system_settings WHERE setting_key = 'auto_logout_timeout' LIMIT 1");
            ...
            if (Array.isArray(timeoutRows) && timeoutRows.length > 0) {
                ...
                await pool_1.default.query("INSERT INTO system_settings (setting_key, setting_value) VALUES ('auto_logout_timeout', '7200')");
            }
