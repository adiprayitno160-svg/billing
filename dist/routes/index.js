...
const oltController_1 = require("../controllers/ftth/oltController");
const odcController_1 = require("../controllers/ftth/odcController");
const odpController_1 = require("../controllers/ftth/odpController");
const AreaController_1 = require("../controllers/ftth/AreaController");
const OntViewController_1 = require("../controllers/ftth/OntViewController");
const pppoeController_1 = require("../controllers/pppoeController");
const pppoeService_1 = require("../services/pppoeService");
const pppoeService_2 = require("../services/pppoeService");
const mikrotikService_1 = require("../services/mikrotikService");
const staticIpPackageController_1 = require("../controllers/staticIpPackageController");
const staticIpClientController_1 = require("../controllers/staticIpClientController");
const staticIpImportRoutes_1 = __importDefault(require("./staticIpImportRoutes"));
const activation_1 = __importDefault(require("./pppoe/activation"));
const reportingController_1 = require("../controllers/reportingController");
const payment_1 = __importDefault(require("./payment"));
const auth_1 = __importDefault(require("./auth"));
const kasir_1 = __importDefault(require("./kasir"));
...
const networkMonitoring_1 = __importDefault(require("./networkMonitoring"));
const sla_1 = __importDefault(require("./sla"));
const maintenance_1 = __importDefault(require("./maintenance"));
const settings_1 = __importDefault(require("./settings"));
const whatsapp_1 = __importDefault(require("./whatsapp"));
const prepaid_1 = __importDefault(require("./prepaid"));
const prepaidDashboard_1 = __importDefault(require("./prepaidDashboard"));
...
const IpCalculatorController_1 = require("../controllers/network/IpCalculatorController");
// EMER ROUTE MOVED
...
const JobTypeController_1 = require("../controllers/technician/JobTypeController");
...
const RegistrationRequestController_1 = require("../controllers/customers/RegistrationRequestController");
const packageLimit_1 = require("../utils/packageLimit");
...
router.get('/api/trigger-bulk-mar', async (req, res) => {
    try {
        ...
        const { InvoiceService } = await Promise.resolve().then(() => __importStar(require('../services/billing/invoiceService')));
        const ids = await InvoiceService.generateMonthlyInvoices('2026-03', undefined, true);
        console.log(`[DEBUG] MANUAL TRIGGER BULK MAR FINISHED: ${ids.length} created`);
        res.json({ success: true, count: ids.length, ids });
    }
    catch (e) {
        console.error('[DEBUG] MANUAL TRIGGER BULK MAR ERROR:', e);
        res.status(500).json({ success: false, error: e.message });
    }
});
...
router.get('/technician', authMiddleware_1.isAuthenticated, TechnicianController_1.TechnicianController.dashboard);
router.use('/technician', technician_1.default);
router.use('/api/technician', technician_1.default);
// EMERGENCY DEPLOY TEST
router.get('/api/deploy-test', (req, res) => {
    console.log('[DEBUG] DEPLOY TEST HIT AT', new Date().toISOString());
    res.json({ success: true, message: 'Deploy Test OK', time: new Date().toISOString() });
});
router.get('/api/v2/system/restart-now-secret-v7', (req, res) => {
    console.log('[DEBUG] SYSTEM RESTART TRIGGERED');
    const { exec } = require('child_process');
    exec('pm2 reload billing-app', (err, stdout, stderr) => {
        res.send(`RESTARTING... STDOUT: ${stdout} STDERR: ${stderr} ERR: ${err}`);
    });
});
router.use('/', auth_1.default);
// Registration Request Routes
router.get('/customers/registration-requests', authMiddleware_1.isAuthenticated, RegistrationRequestController_1.RegistrationRequestController.index);
router.post('/customers/registration-requests/:id/approve', authMiddleware_1.isAuthenticated, RegistrationRequestController_1.RegistrationRequestController.approve);
router.post('/customers/registration-requests/:id/reject', authMiddleware_1.isAuthenticated, RegistrationRequestController_1.RegistrationRequestController.reject);
// NOTIFICATION ROUTES - Mount notification page router
// Mount notification page router at /notification BEFORE middleware
router.use('/notification', notification_1.pageRouter);
...
router.get('/api/debug/ping/:ip', (req, res) => {
    ...
    if (!/^[0-9\.]+$/.test(ip))
        return res.send('Invalid IP format');
...
router.all('/packages/static-ip/sync-all', staticIpPackageController_1.postStaticIpPackageSyncAll);
    ...
    try {
        ...
        try {
            ...
            const [notifications] = await connection.query(`SELECT 
                 ...
                 LIMIT 10`);
            // Cek template
            const [templates] = await connection.query(`SELECT * FROM notification_templates 
...
router.get('/api/dashboard/whatsapp', async (req, res) => {
    try {
        ...
        const [recentMessages] = await pool_1.databasePool.query(`
...
router.get('/customers/registration-requests', authMiddleware_1.isAuthenticated, customerController_1.viewRegistrationRequests);
router.get('/api/interface-stats', dashboardController_1.getInterfaceStats);
router.get('/api/mikrotik/info', settingsController_1.getMikrotikInfoApi);
// API endpoint for real-time monitoring stats
router.get('/api/dashboard/trouble-customers', async (req, res) => {
    try {
        const { getTroubleCustomers } = await Promise.resolve().then(() => __importStar(require('../controllers/dashboardController')));
...
router.get('/api/dashboard/offline-customers', async (req, res) => {
    try {
        ...
        const { getPppoeActiveConnections } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
...
router.get('/api/mikrotik/customer/:customerId/traffic', async (req, res) => {
    try {
        ...
        // Get MikroTik config
        const [mikrotikRows] = await pool_1.databasePool.query('SELECT * FROM mikrotik_settings ORDER BY id DESC LIMIT 1');
        ...
        if (customer.connection_type === 'pppoe' && customer.pppoe_username) {
            ...
            if (sessions && sessions.length > 0) {
                ...
                if (weeks)
                    uptimeSeconds += parseInt(weeks[1]) * 7 * 24 * 3600;
                if (days)
                    uptimeSeconds += parseInt(days[1]) * 24 * 3600;
                if (hours)
                    uptimeSeconds += parseInt(hours[1]) * 3600;
                if (minutes)
                    uptimeSeconds += parseInt(minutes[1]) * 60;
                if (secs)
                    uptimeSeconds += parseInt(secs[1]);
            ...
            if (targetIp) {
                ...
                const customerQueue = queues.find((q) => q['packet-mark']?.includes(targetIp.split('/')[0]) ||
...
router.use('/api/prepaid', prepaid_1.default);
// Prepaid Dashboard routes (UI pages)
router.use('/prepaid', prepaidDashboard_1.default);
// Accounting routes
console.log('[ROUTE REGISTRATION] Registering /accounting routes...');
router.use('/accounting', accounting_1.default);
console.log('[ROUTE REGISTRATION] Accounting routes registered successfully');
// Monitoring routes
router.use('/monitoring', networkMonitoring_1.default);
router.use('/monitoring', monitoring_1.default);
// SLA Monitoring routes (submenu of monitoring)
router.use('/monitoring/sla', sla_1.default);
// Maintenance Schedule routes (submenu of monitoring)
router.use('/monitoring/maintenance', maintenance_1.default);
// WhatsApp routes
router.use('/whatsapp', whatsapp_1.default);
// GenieACS routes
router.use('/genieacs', genieacs_1.default);
// WiFi Admin routes
router.use('/wifi-admin', wifi_admin_1.default);
// Static IP Import routes
router.use('/', staticIpImportRoutes_1.default);
...
try {
    ...
    router.get('/settings/job-types', authMiddleware_1.isAuthenticated, JobTypeController_1.JobTypeController.index);
    router.post('/api/settings/job-types', authMiddleware_1.isAuthenticated, JobTypeController_1.JobTypeController.create);
    router.put('/api/settings/job-types/:id', authMiddleware_1.isAuthenticated, JobTypeController_1.JobTypeController.update);
    router.delete('/api/settings/job-types/:id', authMiddleware_1.isAuthenticated, JobTypeController_1.JobTypeController.delete);
    // 2. Admin Technician: Salary Approval & Payment
    // Fixes 404 on /admin/technician/salary/approval
    router.get('/admin/technician/salary/approval', authMiddleware_1.isAuthenticated, TechnicianSalaryController_1.TechnicianSalaryController.viewMonthlyRecap);
    router.post('/admin/technician/salary/approve', authMiddleware_1.isAuthenticated, TechnicianSalaryController_1.TechnicianSalaryController.approveSalary);
    router.get('/admin/technician/payments/summary', authMiddleware_1.isAuthenticated, TechnicianSalaryController_1.TechnicianSalaryController.viewPaymentSummary);
    router.get('/admin/technician/payments/slip/:id', authMiddleware_1.isAuthenticated, TechnicianSalaryController_1.TechnicianSalaryController.printSalarySlip);
...
router.get('/api/mikrotik/secrets/search', async (req, res) => {
    ...
    try {
        ...
        const config = await (0, pppoeService_2.getMikrotikConfig)();
        if (!config) {
        ...
        let secrets;
        try {
            secrets = await (0, mikrotikService_1.getPppoeSecrets)(config);
        ...
        const [existingCustomers] = await pool_1.databasePool.query('SELECT pppoe_username FROM customers WHERE pppoe_username IS NOT NULL');
...
router.get('/api/mikrotik/pppoe/active-unregistered', authMiddleware_1.isAuthenticated, customerController_1.getActivePppoeConnections);
...
router.post('/api/pppoe/secret/create', async (req, res) => {
    ...
    try {
        ...
        const { getProfileById, getPackageById } = await Promise.resolve().then(() => __importStar(require('../services/pppoeService')));
...
router.use('/api/notification', notification_1.apiRouter);
console.log('[ROUTE REGISTRATION] /api/notification routes registered');
// Pengaturan -> MikroTik
router.get('/settings/mikrotik', settingsController_1.getMikrotikSettingsForm);
router.post('/settings/mikrotik', settingsController_1.postMikrotikSettings);
router.post('/settings/mikrotik/test', settingsController_1.postMikrotikTest);
...
router.get('/ftth/olt', oltController_1.getOltList);
router.get('/ftth/olt/add', (req, res) => res.render('ftth/olt_add', { title: 'Tambah OLT' }));
router.get('/ftth/olt/:id', oltController_1.getOltEdit);
router.post('/ftth/olt', oltController_1.postOltCreate);
router.post('/ftth/olt/:id', oltController_1.postOltUpdate);
router.post('/ftth/olt/:id/delete', oltController_1.postOltDelete);
// FTTH ODC
router.get('/ftth/odc', odcController_1.getOdcList);
router.get('/ftth/odc/add', odcController_1.getOdcAdd);
router.get('/ftth/odc/:id', odcController_1.getOdcEdit);
router.post('/ftth/odc', odcController_1.postOdcCreate);
router.post('/ftth/odc/:id', odcController_1.postOdcUpdate);
router.post('/ftth/odc/:id/delete', odcController_1.postOdcDelete);
// FTTH ODP
router.get('/ftth/odp', odpController_1.getOdpList);
router.get('/ftth/odp/add', odpController_1.getOdpAdd);
router.get('/ftth/odp/:id', odpController_1.getOdpEdit);
router.post('/ftth/odp', odpController_1.postOdpCreate);
router.post('/ftth/odp/:id', odpController_1.postOdpUpdate);
router.post('/ftth/odp/:id/delete', odpController_1.postOdpDelete);
// API: Search ODC
router.get('/api/ftth/odc/search', async (req, res) => {
    try {
...
// API: Search ODP
router.get('/api/ftth/odp/search', async (req, res) => {
    try {
        const query = String(req.query.q || '').trim();
...
// FTTH Areas
router.get('/ftth/areas', AreaController_1.AreaController.index);
router.get('/ftth/areas/create', AreaController_1.AreaController.create);
router.post('/ftth/areas', AreaController_1.AreaController.store);
router.get('/ftth/areas/:id/edit', AreaController_1.AreaController.edit);
router.post('/ftth/areas/:id/update', AreaController_1.AreaController.update);
router.post('/ftth/areas/:id/delete', AreaController_1.AreaController.delete);
// FTTH ONT
router.get('/ftth/ont', OntViewController_1.OntViewController.index);
router.get('/ftth/ont/create', OntViewController_1.OntViewController.create);
router.post('/ftth/ont', OntViewController_1.OntViewController.store);
router.get('/ftth/ont/:id/edit', OntViewController_1.OntViewController.edit);
router.post('/ftth/ont/:id/update', OntViewController_1.OntViewController.update);
router.post('/ftth/ont/:id/delete', OntViewController_1.OntViewController.delete);
// Paket Internet -> PPPOE
// Profile routes
// IMPORTANT: Routes dengan path spesifik HARUS ditempatkan SEBELUM routes dengan parameter dinamis
router.get('/packages/pppoe/profiles', pppoeController_1.getProfileList);
router.post('/packages/pppoe/profiles/sync', pppoeController_1.postSyncProfiles);
router.get('/packages/pppoe/profiles/new', pppoeController_1.getProfileForm);
router.get('/packages/pppoe/profiles/test-update-100', async (req, res) => {
    try {
        const { RouterOSAPI } = await Promise.resolve().then(() => __importStar(require('routeros-api')));
        ...
        const allProfiles = await (0, pppoeService_1.listProfiles)();
        const profile100 = allProfiles.find(p => p.name.toLowerCase().includes('100') ||
        ...
        const mikrotikProfiles = await (0, mikrotikService_1.getPppProfiles)(config);
        ...
        const verifyProfiles = await (0, mikrotikService_1.getPppProfiles)(config);
...
router.get('/packages/pppoe/profiles/:id/edit', pppoeController_1.getProfileEdit);
router.post('/packages/pppoe/profiles/create', pppoeController_1.postProfileCreate);
router.post('/packages/pppoe/profiles/:id/update', pppoeController_1.postProfileUpdate);
router.delete('/packages/pppoe/profiles/:id', pppoeController_1.postProfileDelete);
// Test endpoint untuk sinkronisasi profil
router.get('/api/test/profile-sync/:id', async (req, res) => {
    try {
        ...
        const profile = await (0, pppoeService_1.getProfileById)(profileId);
        if (!profile) {
        ...
        const dbRateLimitRx = profile.rate_limit_rx || '0';
        const dbRateLimitTx = profile.rate_limit_tx || '0';
...
router.get('/test/force-update-profile-100', async (req, res) => {
    const { RouterOSAPI } = await Promise.resolve().then(() => __importStar(require('routeros-api')));
    try {
        ...
        const results = {
            profile: {
                ...
                database_rate_limit_rx: profile100.rate_limit_rx,
                database_rate_limit_tx: profile100.rate_limit_tx
            },
            mikrotik: {
...
// Package routes
router.get('/packages/pppoe/packages', pppoeController_1.getPackageList);
router.get('/packages/pppoe/packages/create', pppoeController_1.getPackageForm);
router.get('/packages/pppoe/packages/:id/edit', pppoeController_1.getPackageEdit);
router.post('/packages/pppoe/packages/create', pppoeController_1.postPackageCreate);
router.post('/packages/pppoe/packages/:id/update', pppoeController_1.postPackageUpdate);
router.post('/packages/pppoe/packages/:id/delete', pppoeController_1.postPackageDelete);
// Paket Internet -> Static IP
router.use('/', staticIpImportRoutes_1.default); // Routes: /settings/static-ip/import, /api/static-ip/import/*
// PPPoE Activation Routes
router.use('/api/pppoe/activation', activation_1.default);
router.use('/pppoe/activation', activation_1.default);
router.get('/packages/static-ip', staticIpPackageController_1.getStaticIpPackageList);
router.get('/packages/static-ip/add', staticIpPackageController_1.getStaticIpPackageAdd);
router.post('/packages/static-ip', staticIpPackageController_1.postStaticIpPackageCreate);
router.get('/packages/static-ip/:id/edit', staticIpPackageController_1.getStaticIpPackageEdit);
router.post('/packages/static-ip/:id/delete', staticIpPackageController_1.postStaticIpPackageDelete);
router.post('/packages/static-ip/:id/update', staticIpPackageController_1.postStaticIpPackageUpdate);
router.post('/packages/static-ip/:id/delete-queues', staticIpPackageController_1.postStaticIpPackageDeleteQueues);
router.post('/packages/static-ip/:id/copy', staticIpPackageController_1.postStaticIpPackageCopy);
router.delete('/packages/static-ip/:id/api-delete', staticIpPackageController_1.apiDeletePackage);
// Routes untuk mengganti paket IP statis pelanggan
router.get('/customers/:customerId/change-static-ip-package', staticIpClientController_1.getChangePackageForm);
router.post('/customers/:customerId/change-static-ip-package', staticIpClientController_1.postChangePackage);
// Test route first
...
router.post('/packages/static-ip/:id/create-queues', (req, res) => {
    ...
    try {
        (0, staticIpPackageController_1.postStaticIpPackageCreateQueues)(req, res, () => { });
    }
    catch (error) {
...
router.get('/packages/static-ip/:packageId/clients', staticIpClientController_1.getStaticIpClientList);
router.get('/packages/static-ip/:packageId/clients/add', staticIpClientController_1.getStaticIpClientAdd);
router.post('/packages/static-ip/:packageId/clients', staticIpClientController_1.postStaticIpClientCreate);
router.post('/packages/static-ip/:packageId/clients/:clientId/delete', staticIpClientController_1.postStaticIpClientDelete);
router.get('/packages/static-ip/:packageId/clients/:clientId/edit', staticIpClientController_1.getStaticIpClientEdit);
router.post('/packages/static-ip/:packageId/clients/:clientId/edit', staticIpClientController_1.postStaticIpClientUpdate);
// Test endpoint untuk debug MikroTik
router.get('/test-mikrotik-ip', staticIpClientController_1.testMikrotikIpAdd);
// Auto debug system untuk IP static
router.get('/auto-debug-ip-static', staticIpClientController_1.autoDebugIpStatic);
// IP Calculator routes
router.get('/network/ip-checker', (req, res) => IpCalculatorController_1.IpCalculatorController.renderPage(req, res));
router.post('/network/ip-checker/scan', (req, res) => IpCalculatorController_1.IpCalculatorController.scanSubnet(req, res));
router.post('/network/ip-checker/scan30', (req, res) => IpCalculatorController_1.IpCalculatorController.scanSubnet30(req, res));
router.post('/network/ip-checker/tabs', (req, res) => IpCalculatorController_1.IpCalculatorController.saveTab(req, res));
router.delete('/network/ip-checker/tabs', (req, res) => IpCalculatorController_1.IpCalculatorController.deleteTab(req, res));
router.post('/network/ip-checker/update-comment', (req, res) => IpCalculatorController_1.IpCalculatorController.updateComment(req, res));
router.post('/network/ip-checker/delete-comment', (req, res) => IpCalculatorController_1.IpCalculatorController.deleteComment(req, res));
...
router.get('/api/debug/interfaces', async (req, res) => {
    try {
        ...
        const { getInterfaces } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
...
router.post('/api/customers/sync-genieacs', customerController_1.syncAllCustomersToGenieacs);
router.get('/customers/list', customerController_1.getCustomerList);
router.get('/customers/', customerController_1.getCustomerList);
router.post('/api/customers/:id/sync-pppoe', authMiddleware_1.isAuthenticated, customerController_1.syncCustomerPppoe);
router.get('/customers', customerController_1.getCustomerList);
router.get('/customers/export', excelController_1.exportCustomersToExcel);
router.get('/customers/template', excelController_1.getImportTemplate);
router.post('/customers/:id/compensation', customerController_1.addCompensation);
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            try {
                ...
                await pool_1.databasePool.execute('INSERT INTO customers (customer_code, name, phone, email, address, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())', [code, name, phone, email, address, 'active']);
                results.success++;
            }
            catch (err) {
                results.failed++;
...
router.post('/customers/import', (req, res, next) => {
    ...
    upload.single('excelFile')(req, res, (err) => {
        ...
        (0, excelController_1.importCustomersFromExcel)(req, res).catch(err => {
...
router.get('/customers/new-pppoe', async (req, res) => {
    try {
        ...
        let packages = await (0, pppoeService_3.listPackages)();
        // Tampilkan semua paket PPPoE
        console.log('Packages loaded:', packages.length);
        const profiles = await (0, pppoeService_3.listProfiles)();
        ...
        try {
            ...
            if (req.query.request_id) {
                try {
                    ...
                    console.error('Err fetching registration:', e);
                }
            }
...
router.post('/customers/new-pppoe', async (req, res) => {
    ...
    try {
        ...
        console.log('Request method:', req.method);
        console.log('Request URL:', req.url);
        const { customer_code, client_name, username, password, package_id, address, phone_number, latitude, longitude, olt_id, odc_id, odp_id, is_wireless, // Add is_wireless
        enable_billing, billing_mode, ppn_mode, rental_mode, rental_cost, // Add rental_cost
        serial_number, initial_validity_days, activation_date, custom_payment_deadline } = req.body;
        ...
        if (!package_id)
            throw new Error('Paket wajib dipilih');
        // ODP required only if NOT wireless mode
        if (!is_wireless && !odp_id)
            throw new Error('ODP wajib dipilih (kecuali Mode Wireless)');
        ...
        const isFull = await (0, packageLimit_1.isPppoePackageFull)(Number(package_id));
        if (isFull) {
        ...
        try {
            ...
            const [existingCodeRows] = await conn.execute('SELECT id, name FROM customers WHERE customer_code = ?', [customer_code.trim()]);
            if (Array.isArray(existingCodeRows) && existingCodeRows.length > 0) {
                const existingCustomer = existingCodeRows[0];
            ...
            const [existingUsernameRows] = await conn.execute('SELECT id, name FROM customers WHERE pppoe_username = ?', [username.trim()]);
            if (Array.isArray(existingUsernameRows) && existingUsernameRows.length > 0) {
                const existingCustomer = existingUsernameRows[0];
            ...
            // Task 3: Custom Jatuh Tempo default = tanggal aktivasi
            let final_activation_date = activation_date || new Date().toISOString().split('T')[0];
            let final_custom_payment_deadline = custom_payment_deadline;
            if (!final_custom_payment_deadline && final_activation_date) {
                final_custom_payment_deadline = new Date(final_activation_date).getDate();
                console.log(`[Deadline] Auto-set to activation day: ${final_custom_payment_deadline}`);
            }
            ...
            if ((!latitude || !longitude) && odp_id) {
                ...
                const [odpRows] = await conn.query('SELECT latitude, longitude FROM ftth_odp WHERE id = ?', [odp_id]);
            ...
            console.log('Inserting customer with data:', {
                ...
                activation_date: final_activation_date,
                custom_payment_deadline: final_custom_payment_deadline
            });
            ...
            try {
                ...
                const { getMikrotikConfig } = await Promise.resolve().then(() => __importStar(require('../services/pppoeService')));
                const { createPppoeSecret, findPppoeSecretIdByName, updatePppoeSecret } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
                ...
                if (!config) {
                    ...
                    try {
                        ...
                        const { getPackageById } = await Promise.resolve().then(() => __importStar(require('../services/pppoeService')));
                    ...
                    try {
                        const { getPppProfiles } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
                        ...
                        if (!profileExists) {
                            ...
                            const similarProfile = profileNames.find(p => p.toLowerCase().includes('gratis') ||
                    ...
                    if (existingSecretId) {
                        ...
                        try {
                            ...
                            if (createError.message && (createError.message.includes('already have') ||
            ...
            try {
                ...
                if (config && package_id) {
                    ...
                    if (pkg && pkg.max_clients && pkg.max_clients > 1) {
                        ...
                        const mikrotikService = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
            ...
            if (enableBilling && package_id) {
                try {
                    ...
                    const [subs] = await pool_1.databasePool.query('SELECT id, price, start_date, package_name FROM subscriptions WHERE customer_id = ? ORDER BY id DESC LIMIT 1', [customerId]);
                    if (subs.length > 0) {
                        const sub = subs[0];
            ...
            try {
                ...
                const [packageRows] = await pool_1.databasePool.query('SELECT name FROM pppoe_packages WHERE id = ?', [package_id]);
                ...
                const result = await CustomerNotificationService_1.default.notifyNewCustomer({
                    ...
                    createdBy: req.session.user?.username || undefined
                });
...
router.get('/test/debug-create-secret', async (req, res) => {
    console.log('\n\n═══════════════════════════════════════════════════════════');
    console.log('=== DEBUG CREATE SECRET ===');
    console.log('═══════════════════════════════════════════════════════════');
    // Default test values
    const testName = req.query.name || `test_user_${Date.now()}`;
    const testPassword = req.query.password || '12345';
    const testProfile = req.query.profile || 'default';
    const { getMikrotikConfig } = await Promise.resolve().then(() => __importStar(require('../services/pppoeService')));
    const { createPppoeSecret, findPppoeSecretIdByName } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
    const config = await getMikrotikConfig();
    if (!config) {
...
router.post('/customers/bulk-delete', async (req, res, next) => {
    try {
        const { bulkDeleteCustomers } = await Promise.resolve().then(() => __importStar(require('../controllers/customerController')));
        return bulkDeleteCustomers(req, res);
    }
    catch (e) {
        next(e);
    }
...
router.get('/customers/edit-static-ip/:id', async (req, res) => {
    ...
    try {
        ...
        let client = await (0, staticIpClientService_1.getStaticIpClientByCustomerId)(clientId);
        ...
        if (!client) {
            ...
            const conn = await pool_1.databasePool.getConnection();
            const [custRows] = await conn.execute('SELECT * FROM customers WHERE id = ?', [clientId]);
            conn.release();
            if (custRows.length > 0) {
                ...
                const [custCodeRows] = await conn.execute('SELECT * FROM customers WHERE customer_code = ?', [clientId]);
                if (custCodeRows.length > 0) {
                    ...
                    if (ts.length === 14 && /^\d+$/.test(ts)) {
                        ...
                        const [custTimeRows] = await conn.execute(`
        ...
        let pkg = await (0, staticIpPackageService_1.getStaticIpPackageById)(client.package_id);
        ...
        const allPackages = await (0, staticIpPackageService_1.listStaticIpPackages)();
...
router.post('/customers/edit-static-ip/:id', async (req, res) => {
    try {
        ...
        const { client_name, ip_address, interface: iface, package_id, address, phone_number, latitude, longitude, olt_id, odc_id, odp_id } = req.body;
        if (!client_name)
            throw new Error('Nama pelanggan wajib diisi');
        if (!ip_address)
            throw new Error('IP statis wajib diisi');
        ...
        if (!cidrRegex.test(ip_address))
            throw new Error('Format IP CIDR tidak valid');
        let formattedIp = ip_address;
        if (!formattedIp.includes('/')) {
            formattedIp += '/32';
        }
        // Ambil data binding static ip lama (bisa null jika data rusak)
        const oldClient = await (0, staticIpClientService_1.getStaticIpClientByCustomerId)(customerId);
        ...
        const pkg = await (0, staticIpPackageService_1.getStaticIpPackageById)(targetPackageId);
        if (!pkg)
        ...
        if (cfg) {
            ...
            if (oldClient) {
                if (oldClient.ip_address) {
                    await (0, mikrotikService_2.removeIpAddress)(cfg, oldClient.ip_address);
                }
                ...
                const downloadMark = ipOnly;
                const uploadMark = `UP-${ipOnly}`;
                await (0, mikrotikService_2.removeMangleRulesForClient)(cfg, { peerIp: ipOnly, downloadMark, uploadMark });
                await (0, mikrotikService_2.deleteClientQueuesByClientName)(cfg, oldClient.client_name);
            }
            // 2) Tambahkan resource baru (Selalu jalankan untuk apply config baru)
            if (iface) {
                await (0, mikrotikService_2.addIpAddress)(cfg, {
                    interface: iface,
                    address: formattedIp,
                    comment: client_name
                });
            ...
            const newDownloadMark = newPeerIp;
            const newUploadMark = `UP-${newPeerIp}`;
            await (0, mikrotikService_2.addMangleRulesForClient)(cfg, { peerIp: newPeerIp, downloadMark: newDownloadMark, uploadMark: newUploadMark });
            ...
            await (0, mikrotikService_2.createQueueTree)(cfg, {
                name: client_name,
                parent: packageDownloadQueue,
                packetMarks: newDownloadMark,
                maxLimit: mlDownload,
                queue: pkg.child_queue_type_download || 'pcq',
            ...
            await (0, mikrotikService_2.createQueueTree)(cfg, {
                ...
                maxLimit: mlUpload,
                queue: pkg.child_queue_type_upload || 'pcq',
                priority: pkg.child_priority_upload || '8'
            });
        }
        // Update database (Sync both tables)
        const connPool = await pool_1.databasePool.getConnection();
        try {
            ...
            if (oldClient) {
                await (0, staticIpClientService_1.updateClient)(oldClient.id, {
                    client_name,
                ...
                const [custRows] = await connPool.execute('SELECT customer_code FROM customers WHERE id = ?', [customerId]);
...
const mikrotikService_2 = require("../services/mikrotikService");
const staticIpClientService_1 = require("../services/staticIpClientService");
const pppoeService_3 = require("../services/pppoeService");
...
router.get('/api/packages/:connectionType', async (req, res) => {
    try {
        ...
        if (connectionType === 'pppoe') {
            const packages = await (0, pppoeService_3.listPackages)();
            res.json(packages);
        }
    ...
    try {
        ...
        try {
            secrets = await Promise.race([
                (0, mikrotikService_1.getPppoeSecrets)(cfg),
                timeoutPromise
            ]);
        }
        catch (err) {
        ...
        const [usedRows] = await pool_1.databasePool.execute('SELECT DISTINCT pppoe_username FROM customers WHERE pppoe_username IS NOT NULL AND pppoe_username != ""');
...
router.get('/packages/static-ip/:packageId/clients/add', staticIpClientController_1.getStaticIpClientAdd);
router.post('/packages/static-ip/:packageId/clients/add', staticIpClientController_1.postStaticIpClientCreate);
// Edit/Delete Client Operations
router.get('/packages/static-ip/clients/:id/edit', staticIpClientController_1.getStaticIpClientEdit);
router.post('/packages/static-ip/clients/:id/edit', staticIpClientController_1.postStaticIpClientUpdate);
router.post('/packages/static-ip/clients/:id/delete', staticIpClientController_1.postStaticIpClientDelete);
// Note: Route umum /customers/edit-static-ip/:id juga menghandle update
const processingStaticIpLimits = new Set();
router.post('/customers/new-static-ip', async (req, res) => {
    let processLockKey = null;
    ...
    try {
        ...
        console.log('Request body:', req.body);
        const { client_name, customer_code, ip_address, package_id, interface: iface, address, phone_number, latitude, longitude, olt_id, odc_id, odp_id, billing_mode, ppn_mode, rental_mode, serial_number, enable_billing, is_wireless, // Add is_wireless
        activation_date, custom_payment_deadline } = req.body;
        console.log('Parsed data:', { client_name, customer_code, ip_address, package_id, interface: iface, is_wireless: is_wireless ? 'YES' : 'NO' });
        if (!client_name)
            throw new Error('Nama pelanggan wajib diisi');
        if (!ip_address)
            throw new Error('IP statis wajib diisi (contoh: 192.168.1.1/30)');
        if (!package_id)
            throw new Error('Paket wajib dipilih');
        // Validate package limit
        const isFull = await (0, packageLimit_1.isStaticIpPackageFull)(Number(package_id));
        if (isFull) {
        ...
        try {
            ...
            const [existingIpRows] = await conn.execute('SELECT c.name FROM static_ip_clients sic JOIN customers c ON sic.customer_id = c.id WHERE sic.ip_address = ?', [ip_address.trim()]);
        ...
        const connVal = await pool_1.databasePool.getConnection();
        try {
            ...
            const [existingNameRows] = await connVal.execute("SELECT id, name FROM customers WHERE REPLACE(name, ' ', '') LIKE REPLACE(?, ' ', '')", [client_name.trim()]);
            if (Array.isArray(existingNameRows) && existingNameRows.length > 0) {
                const existing = existingNameRows[0];
        ...
        const full = await (0, staticIpClientService_1.isPackageFull)(pkgId);
        if (full)
            throw new Error('Paket sudah penuh');
        ...
        if (prefix === 30) {
            ...
            if (ipInt === firstHost)
                peerIp = intToIp(secondHost);
            else if (ipInt === secondHost)
                peerIp = intToIp(firstHost);
            else
                peerIp = intToIp(secondHost); // fallback
        }
        const { customerId } = await (0, staticIpClientService_1.addClientToPackage)(pkgId, {
            client_name,
        ...
        const cfg = await (0, pppoeService_2.getMikrotikConfig)();
        const pkg = await (0, staticIpPackageService_1.getStaticIpPackageById)(pkgId);
        ...
        if (cfg && pkg) {
            try {
                ...
                const { qtype_download, qtype_upload, priority_download, priority_upload, limitat_download, limitat_upload, maxlimit_download, maxlimit_upload, burst_enabled, burst_limit_download, burst_limit_upload, burst_threshold_download, burst_threshold_upload, burst_time_download, burst_time_upload } = req.body;
                await (0, staticIpPackageService_1.syncClientQueues)(customerId, pkgId, ip_address_with_cidr, client_name, {
                    overrides: {
        ...
        if (enable_billing !== '0' && package_id) {
            try {
                ...
                if (autoGenFirstInvoice) {
                    ...
                    const [subs] = await pool_1.databasePool.query('SELECT id, price, package_name FROM subscriptions WHERE customer_id = ? ORDER BY id DESC LIMIT 1', [customerId]);
                    if (subs.length > 0) {
                        const sub = subs[0];
        ...
        try {
            // const CustomerNotificationService = (await import('../services/customer/CustomerNotificationService')).default;
            const [packageRows] = await pool_1.databasePool.query('SELECT name FROM static_ip_packages WHERE id = ?', [package_id]);
            const packageName = packageRows.length > 0 ? packageRows[0].name : undefined;
            // Get customer code
            const [customerRows] = await pool_1.databasePool.query('SELECT customer_code, name, phone, address FROM customers WHERE id = ?', [customerId]);
        ...
        if (processLockKey)
            processingStaticIpLimits.delete(processLockKey);
...
router.get('/api/test/queue/test-connection', async (req, res) => {
    try {
        const { testMikrotikConnection } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
        const config = {
            host: '192.168.5.1',
            port: 8728,
            username: 'adii',
...
router.get('/api/test/queue/create-direct', async (req, res) => {
    try {
        ...
        const { createQueueTree } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
        const config = {
            host: '192.168.5.1',
            port: 8728,
            username: 'adii',
...
router.get('/api/test/queue/custom-name-test', async (req, res) => {
    try {
        ...
        const { createQueueTree, getQueueTrees } = await Promise.resolve().then(() => __importStar(require('../services/mikrotikService')));
        const config = {
            host: '192.168.5.1',
            port: 8728,
            username: 'adii',
        ...
        const createdQueue = queues.find((q) => q.name === customName);
        ...
        res.json({
            ...
            data: {
                ...
                verification: {
                    ...
                    allQueues: queues.map((q) => ({ name: q.name, parent: q.parent, comment: q.comment }))
                }
            }
        });
    }
...
router.get('/api/test/queue/auto-test', async (req, res) => {
    try {
        ...
        const createdQueues = queues.filter((q) => q.name && q.name.includes('autotest'));
        api.close();
        res.json({
            success: true,
            ...
            data: {
                testName,
                results,
                createdQueues: createdQueues.map((q) => ({ name: q.name, parent: q.parent, comment: q.comment }))
            }
        });
    }
    catch (error) {
...
router.get('/api/test/queue/loop-test', async (req, res) => {
    try {
        ...
        console.log('Checking created queues...');
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('looptest'));
        api.close();
        res.json({
            success: true,
            ...
            data: {
                testName,
                results,
                createdQueues: createdQueues.map((q) => ({
                    name: q.name,
                    parent: q.parent,
                    comment: q.comment,
...
router.get('/api/test/queue/find-format', async (req, res) => {
    try {
        ...
        console.log('Checking created queues...');
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('find'));
        api.close();
        res.json({
            success: true,
...
router.get('/api/test/queue/auto-find', async (req, res) => {
    try {
        ...
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('autofind'));
        console.log('Created queues:', createdQueues.map((q) => q.name));
        api.close();
        res.json({
            success: true,
...
router.get('/api/test/queue/routeros-v6', async (req, res) => {
    try {
        ...
        console.log('\n=== Checking created queues ===');
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('ROUTEROSV6'));
        console.log('Created queues:', createdQueues.map((q) => q.name));
        api.close();
        res.json({
            success: true,
...
router.get('/api/test/queue/mikrotik-official', async (req, res) => {
    try {
        ...
        console.log('\n=== Checking created queues ===');
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('TEST123'));
        console.log('Created queues:', createdQueues.map((q) => q.name));
        api.close();
        res.json({
            success: true,
...
router.get('/api/test/queue/simple-fix', async (req, res) => {
    try {
        ...
        console.log('\n=== Checking created queues ===');
        const queues = await api.write('/queue/tree/print');
        const createdQueues = queues.filter((q) => q.name && q.name.includes('SIMPLE'));
        console.log('Created queues:', createdQueues.map((q) => q.name));
        api.close();
        res.json({
            success: true,
...
router.get('/api/test/queue/loop-until-success', async (req, res) => {
    try {
        ...
        const formatTemplates = [
            // Object formats
            { name: 'Object 1', command: (name) => ({ name, parent: 'DOWNLOAD ALL' }) },
            { name: 'Object 2', command: (name) => ({ parent: 'DOWNLOAD ALL', name }) },
            { name: 'Object 3', command: (name) => ({ name, parent: 'UPLOAD ALL' }) },
            { name: 'Object 4', command: (name) => ({ parent: 'UPLOAD ALL', name }) },
            { name: 'Object 5', command: (name) => ({ name, parent: 'global' }) },
            { name: 'Object 6', command: (name) => ({ parent: 'global', name }) },
            // Array formats
            { name: 'Array 1', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL'] },
            { name: 'Array 2', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`] },
            { name: 'Array 3', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=UPLOAD ALL'] },
            { name: 'Array 4', command: (name) => ['/queue/tree/add', 'parent=UPLOAD ALL', `name=${name}`] },
            { name: 'Array 5', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=global'] },
            { name: 'Array 6', command: (name) => ['/queue/tree/add', 'parent=global', `name=${name}`] },
            // Format dengan quotes
            { name: 'Object 7', command: (name) => ({ name: `"${name}"`, parent: 'DOWNLOAD ALL' }) },
            { name: 'Object 8', command: (name) => ({ parent: '"DOWNLOAD ALL"', name }) },
            { name: 'Array 7', command: (name) => ['/queue/tree/add', `name="${name}"`, 'parent=DOWNLOAD ALL'] },
            { name: 'Array 8', command: (name) => ['/queue/tree/add', 'parent="DOWNLOAD ALL"', `name=${name}`] },
            // Format dengan parameter tambahan
            { name: 'Object 9', command: (name) => ({ name, parent: 'DOWNLOAD ALL', 'max-limit': '5M' }) },
            { name: 'Object 10', command: (name) => ({ parent: 'DOWNLOAD ALL', name, 'max-limit': '5M' }) },
            { name: 'Array 9', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', 'max-limit=5M'] },
            { name: 'Array 10', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, 'max-limit=5M'] },
            // Format dengan comment
            { name: 'Object 11', command: (name) => ({ name, parent: 'DOWNLOAD ALL', comment: 'Loop test' }) },
            { name: 'Object 12', command: (name) => ({ parent: 'DOWNLOAD ALL', name, comment: 'Loop test' }) },
            { name: 'Array 11', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', 'comment=Loop test'] },
            { name: 'Array 12', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, 'comment=Loop test'] },
            // Format dengan semua parameter
            { name: 'Object 13', command: (name) => ({ name, parent: 'DOWNLOAD ALL', 'max-limit': '5M', comment: 'Loop test' }) },
            { name: 'Object 14', command: (name) => ({ parent: 'DOWNLOAD ALL', name, 'max-limit': '5M', comment: 'Loop test' }) },
            { name: 'Array 13', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', 'max-limit=5M', 'comment=Loop test'] },
            { name: 'Array 14', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, 'max-limit=5M', 'comment=Loop test'] },
            // Format khusus RouterOS
            { name: 'Object 15', command: (name) => ({ name, parent: 'DOWNLOAD ALL', priority: '8' }) },
            { name: 'Object 16', command: (name) => ({ parent: 'DOWNLOAD ALL', name, priority: '8' }) },
            { name: 'Array 15', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', 'priority=8'] },
            { name: 'Array 16', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, 'priority=8'] },
            // Format dengan parameter khusus
            { name: 'Object 17', command: (name) => ({ name, parent: 'DOWNLOAD ALL', 'packet-mark': `mark_${name}` }) },
            { name: 'Object 18', command: (name) => ({ parent: 'DOWNLOAD ALL', name, 'packet-mark': `mark_${name}` }) },
            { name: 'Array 17', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', `packet-mark=mark_${name}`] },
            { name: 'Array 18', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, `packet-mark=mark_${name}`] },
            // Format dengan limit-at
            { name: 'Object 19', command: (name) => ({ name, parent: 'DOWNLOAD ALL', 'limit-at': '0' }) },
            { name: 'Object 20', command: (name) => ({ parent: 'DOWNLOAD ALL', name, 'limit-at': '0' }) },
            { name: 'Array 19', command: (name) => ['/queue/tree/add', `name=${name}`, 'parent=DOWNLOAD ALL', 'limit-at=0'] },
            { name: 'Array 20', command: (name) => ['/queue/tree/add', 'parent=DOWNLOAD ALL', `name=${name}`, 'limit-at=0'] }
        ];
        ...
        while (attempt <= maxAttempts) {
            ...
            for (let i = 0; i < formatTemplates.length; i++) {
                ...
                try {
                    ...
                    const queues = await api.write('/queue/tree/print');
                    const createdQueue = queues.find((q) => q.name === currentTestName);
                    if (createdQueue) {
                        ...
                        console.log('❌ Queue not found with custom name. Available queues:', queues.map((q) => q.name));
                    }
                }
                catch (error) {
    ...
    try {
        ...
        for (let i = 0; i < allFormats.length; i++) {
            ...
            try {
                ...
                console.log('Checking if queue was created with custom name...');
                const queues = await api.write('/queue/tree/print');
                const createdQueue = queues.find((q) => q.name === testName);
                if (createdQueue) {
                    console.log('🎉 SUCCESS! Queue created with custom name:', createdQueue);
...
// About routes
router.get('/about', aboutController_1.getAboutPage);
router.get('/about/check-updates', aboutController_1.checkUpdates);
router.get('/about/check-hotfix', aboutController_1.checkHotfix);
router.post('/about/update', aboutController_1.updateAppVersion);
router.post('/about/apply-hotfix', aboutController_1.applyHotfixUpdate);
router.post('/about/update-settings', aboutController_1.updateSettings);
router.get('/about/update-history', aboutController_1.getUpdateHistoryPage);
// Database Management routes
router.get('/database/management', databaseController_1.getDatabaseManagement);
router.post('/database/fix', databaseController_1.fixDatabaseIssues);
router.post('/database/migrate', databaseController_1.runDatabaseMigration);
router.post('/database/migrate/late-payment-tracking', databaseController_1.runLatePaymentTrackingMigration);
router.get('/database/logs', databaseController_1.getDatabaseLogs);
// Backup & Restore routes
// Backup & Restore routes
router.get('/backup', (req, res) => res.redirect('/settings/backup'));
router.get('/settings/backup', backupController_1.BackupController.index);
router.post('/settings/backup/config', backupController_1.BackupController.saveConfig);
router.post('/settings/backup/upload-key', upload.single('keyFile'), backupController_1.BackupController.uploadKey);
router.post('/settings/backup/run', backupController_1.BackupController.runBackup);
router.get('/settings/backup/list', backupController_1.BackupController.listBackups);
router.get('/settings/backup/download/:filename', backupController_1.BackupController.downloadBackup);
router.post('/settings/backup/run-local', backupController_1.BackupController.runLocalBackup);
router.post('/settings/backup/restore/:filename', backupController_1.BackupController.restoreBackup);
router.post('/settings/backup/restore-upload', upload.single('sqlFile'), backupController_1.BackupController.restoreFromUpload);
// Monitoring
...
const reportingController = new reportingController_1.ReportingController(pool_1.databasePool);
const bulkOperationsController = new bulkOperationsController_1.BulkOperationsController(pool_1.databasePool, wsService);
...
// Kasir Routes
router.use('/kasir', kasir_1.default);
// Address List Routes
router.use('/address-list', addressList_1.default);
...
router.get('/api/ftth/olt-slot/:oltId/ports', async (req, res) => {
    try {
        ...
        const [rows] = await pool_1.databasePool.query('SELECT port_name, status, ont_count, max_onts FROM olt_port_info WHERE olt_id = ? ORDER BY port_number', [oltId]);
        res.json({
            success: true,
            data: rows
        });
...
router.get('/customers/:id/edit', (req, res, next) => {
    ...
    (0, customerController_1.getCustomerEdit)(req, res);
});
// Toggle customer status
router.post('/customers/:id/toggle-status', authMiddleware.requireAuth.bind(authMiddleware), async (req, res) => {
    try {
        const { toggleCustomerStatus } = await Promise.resolve().then(() => __importStar(require('../controllers/customerController')));
...
router.delete('/customers/:id', authMiddleware.requireAuth.bind(authMiddleware), async (req, res) => {
    try {
        const { deleteCustomer } = await Promise.resolve().then(() => __importStar(require('../controllers/customerController')));
...
router.post('/customers/:id', customerController_1.updateCustomer);
router.put('/customers/:id', customerController_1.updateCustomer);
router.patch('/customers/:id', customerController_1.updateCustomer);
router.post('/customers/:id/welcome-notification', customerController_1.sendWelcomeNotificationManual);
router.get('/customers/:id', (req, res, next) => {
    console.log('[ROUTE] GET /customers/:id - Customer ID:', req.params.id);
    (0, customerController_1.getCustomerDetail)(req, res);
});
...
router.get('/test-mikrotik-address-lists', customerController_1.testMikrotikAddressLists);
...
router.get('/test/reset-to-postpaid/:customerId', async (req, res) => {
    try {
        ...
        const [customers] = await pool_1.databasePool.query('SELECT id, name, connection_type FROM customers WHERE id = ?', [customerId]);
        ...
        if (customer.connection_type === 'static_ip') {
            const [staticIPs] = await pool_1.databasePool.query('SELECT ip_address FROM static_ip_clients WHERE customer_id = ? LIMIT 1', [customerId]);
        ...
        if (ipAddress) {
            ...
            try {
                const getMikrotikConfig = (await Promise.resolve().then(() => __importStar(require('../utils/mikrotikConfigHelper')))).getMikrotikConfig;
                const settings = await getMikrotikConfig();
                if (settings) {
                    const MikrotikAddressListService = (await Promise.resolve().then(() => __importStar(require('../services/mikrotik/MikrotikAddressListService')))).default;
        ...
        // Update database
        await pool_1.databasePool.query('UPDATE customers SET billing_mode = ? WHERE id = ?', ['postpaid', customerId]);
        await pool_1.databasePool.query('UPDATE customers SET is_isolated = ? WHERE id = ?', [0, customerId]);
...
router.get('/ftth/odc/add', odcController_1.getOdcAdd);
router.post('/ftth/odc', odcController_1.postOdcCreate);
router.get('/ftth/odc/:id', odcController_1.getOdcEdit); // Edit page
router.post('/ftth/odc/:id', odcController_1.postOdcUpdate); // Update action
router.post('/ftth/odc/:id/delete', odcController_1.postOdcDelete);
