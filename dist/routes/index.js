...
router.post('/customers/new-pppoe', async (req, res) => {
    ...
    try {
        ...
        console.log('Request method:', req.method);
        console.log('Request URL:', req.url);
        const { customer_code, client_name, username, password, package_id, address, phone_number, latitude, longitude, olt_id, odc_id, odp_id, enable_billing } = req.body;
        ...
        try {
            ...
            await conn.execute('UPDATE customers SET pppoe_username = ? WHERE id = ?', [customerId.toString(), customerId]);
            ...
            try {
                const CustomerNotificationService = (await Promise.resolve().then(() => __importStar(require('../services/customer/CustomerNotificationService')))).default;
...
router.get('/test/debug-create-secret', async (req, res) => {
    try {
        const testName = req.query.name || 'testuser';
        const testPassword = req.query.password || 'test123';
        const testProfile = req.query.profile || 'gratis';
        console.log('\n\n═══════════════════════════════════════════════════════════');
...
router.get('/customers/edit-static-ip/:id', async (req, res) => {
    try {
        ...
        const pkg = await (0, staticIpPackageService_2.getStaticIpPackageById)(client.package_id);
        if (!pkg) {
...
router.post('/customers/edit-static-ip/:id', async (req, res) => {
    try {
        ...
        const pkg = await (0, staticIpPackageService_2.getStaticIpPackageById)(oldClient.package_id);
        const cfg = await (0, pppoeService_2.getMikrotikConfig)();
        if (cfg && pkg && oldClient) {
            ...
            await (0, mikrotikService_2.removeMangleRulesForClient)(cfg, { peerIp, downloadMark, uploadMark });
            // Hapus queues lama
...
const mikrotikService_3 = require("../services/mikrotikService");
...
router.get('/api/pppoe/secrets', async (req, res) => {
    try {
        ...
        const secrets = await (0, mikrotikService_3.getPppoeSecrets)(cfg);
...
router.post('/customers/new-static-ip', async (req, res) => {
    ...
    try {
        ...
        console.log('Request body:', req.body);
        const { client_name, customer_code, ip_address, package_id, interface: iface, address, phone_number, latitude, longitude, olt_id, odc_id, odp_id, enable_billing } = req.body;
        ...
        const cfg = await (0, pppoeService_2.getMikrotikConfig)();
        const pkg = await (0, staticIpPackageService_2.getStaticIpPackageById)(pkgId);
        ...
        if (cfg && pkg) {
            ...
            const downloadMark = peerIp;
            const uploadMark = `UP-${peerIp}`;
            await (0, mikrotikService_2.addMangleRulesForClient)(cfg, { peerIp, downloadMark, uploadMark });
            ...
            const mlDownload = maxlimit_download || (pkg.child_download_limit || pkg.shared_download_limit || pkg.max_limit_download);
            const mlUpload = maxlimit_upload || (pkg.child_upload_limit || pkg.shared_upload_limit || pkg.max_limit_upload);
        ...
        if (enableBilling && package_id) {
            try {
                ...
                if (pkg) {
                    ...
                    const conn2 = await pool_1.databasePool.getConnection();
                    try {
                        // Insert subscription
...
router.get('/customers/:id/edit', (req, res, next) => {
    ...
    (0, customerController_1.getCustomerEdit)(req, res, next);
});
...
router.get('/customers/:id', (req, res, next) => {
    ...
    (0, customerController_1.getCustomerDetail)(req, res, next);
});
