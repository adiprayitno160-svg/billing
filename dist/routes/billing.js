...
router.get('/tagihan/:invoiceId/pay', async (req, res) => {
    try {
        ...
        try {
            ...
            const [invoices] = await conn.query('SELECT i.*, c.id as customer_id, c.connection_type, c.custom_sla_target FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id WHERE i.id = ?', [req.params.invoiceId]);
            if (invoices.length === 0) {
            ...
            if (invoice.customer_id && invoice.period) {
                ...
                `, [invoice.customer_id, startDate, endDate]);
