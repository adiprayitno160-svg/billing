...
router.get('/rekap/tunggakan', async (req, res) => {
    try {
        ...
        try {
            ...
            const [arrears] = await conn.query(query, params);
...
router.get('/tagihan/:id/print', async (req, res) => {
    try {
        ...
        try {
            ...
            const [companyRows] = await conn.query("SELECT * FROM settings WHERE setting_key IN ('company_name', 'company_phone', 'company_address', 'company_website', 'company_logo') ORDER BY setting_key");
            const companyInfo = {};
...
router.post('/tagihan/:id/settle-carryover', async (req, res) => {
    ...
    try {
        ...
        const [invoiceRows] = await conn.query('SELECT i.*, c.name as customer_name FROM invoices i JOIN customers c ON i.customer_id = c.id WHERE i.id = ?', [invoiceId]);
        ...
        const [carryOverItems] = await conn.query(`SELECT * FROM invoice_items WHERE invoice_id = ? 
             AND (description LIKE 'Tunggakan Tagihan Bulan%' OR description LIKE 'Kekurangan Tagihan Bulan%')`, [invoiceId]);
        ...
        const [sourceInvoices] = await conn.query(`SELECT id, period, remaining_amount, total_amount, notes FROM invoices 
             WHERE customer_id = ? AND status = 'carried_over' AND period < ?
             ORDER BY period ASC`, [invoice.customer_id, invoice.period]);
        ...
        for (const srcInv of sourceInvoices) {
            ...
            await conn.execute(`INSERT INTO payments (invoice_id, payment_method, amount, payment_date, notes, created_at) 
                 VALUES (?, ?, ?, ?, ?, NOW())`, [srcInv.id, paymentMethodStr, parseFloat(srcInv.total_amount), paymentDateStr,
                notes || `Dibayar via invoice ${invoice.invoice_number} (Lunasi Tunggakan)`]);
            // Update source invoice to paid
            await conn.execute(`UPDATE invoices SET status = 'paid', paid_amount = total_amount, remaining_amount = 0, 
                 last_payment_date = ?, updated_at = NOW() WHERE id = ?`, [paymentDateStr, srcInv.id]);
            // Resolve debt tracking for source invoice
            await conn.execute(`UPDATE debt_tracking SET status = 'resolved', resolved_at = NOW(), updated_at = NOW() 
                 WHERE invoice_id = ? AND status = 'active'`, [srcInv.id]);
        }
        ...
        const updatedNotes = invoice.notes
            ...
            : settlementNote;
        await conn.execute(`UPDATE invoices SET total_amount = ?, remaining_amount = ?, notes = ?, updated_at = NOW() WHERE id = ?`, [newTotalAmount, newRemainingAmount, updatedNotes, invoiceId]);
        // 7. Clear carry_over_invoices records
        await conn.execute(`UPDATE carry_over_invoices SET status = 'settled' WHERE customer_id = ? AND status IN ('pending', 'applied')`, [invoice.customer_id]);
        await conn.commit();
        res.json({
            success: true,
